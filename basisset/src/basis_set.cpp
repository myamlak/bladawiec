...
// Parses one double strictly; trailing junk is an error.
//
// std::from_chars is the strict, locale-independent spelling of exactly this
// parse, and the integer overload below uses it. Its floating-point overloads
// are not portable in the same way: libc++ marks them deleted when the
// deployment target predates their availability, and deletes rather than
// omits them, so nothing here can test for their presence - the macOS runners
// are such a target, and this call did not compile there at all. strtod is
// the portable equivalent of the same parse: the token has already been split
// on whitespace, so "the parser consumed all of it" is still the whole check,
// and with no setlocale call anywhere in the program the C locale in force
// makes its decimal point the '.' the input files use. It accepts a leading
// '+', the hex-float forms its general format has no room for, and an
// exponent so small it underflows the type; all three are refused here
// explicitly instead of being accepted on macOS alone. An underflow is
// refused only when it rounded to zero, because a subnormal is a value
// from_chars accepts.
qcx::Result<double> ParseDouble(const std::string& token,
    ...
    const bool whole = !token.empty() && end == begin + token.size();
    const bool underflowedToZero = errno == ERANGE && value == 0.0;
    const bool parsed = whole && token.front() != '+' &&
                        token.find_first_of("xX") == std::string::npos && !underflowedToZero;
#else
    const auto result = std::from_chars(begin, begin + token.size(), value);
    const bool parsed = result.ec == std::errc{} && result.ptr == begin + token.size();
#endif
...
qcx::Result<void> BasisSet::AppendNwchemText(std::string_view text) {
    ...
    // and the next successful append built on it (basisset-2). Elements
    ...
    for (std::size_t i = 1; i < lines.size(); ++i)
    {
        ...
        if (state == State::kInEcp)
        {
            ...
            // per-shell headers of the ECP are ignored (to be populated once
            // the integral machinery consumes them).
