...
TEST(ScfCheckpointTest, CheckpointUhfRoundTrip) {
    ...
    // 8.6e-7..3.3e-6); the Fix-1 per-batch parallel Fock layout
...
// st-2: a DIIS history matrix that disagrees with the density's n must be
...
// st-3: a non-square density dataset must be refused with a clean
...
// st-3: a flipped density element must be refused by the state checksum -
...
// st-3: a checkpoint without the state_checksum attribute (written by an
