...
/// misses recompute through an injected engine callback and append. Mirrors
/// niedoida's Request{quartet, done, results} shape as a drop-in decorator
/// over the integrals batch API.
...
/// Drop-in decorator over the batch API (mirrors niedoida's
/// Request{quartet, done, results} shape): reads hit the store's manifest;
/// misses recompute through an injected engine callback and append. Opt-in
/// by construction - the direct/LinK Fock builders never use it (8.7: LinK
/// keeps recomputing by design). A corrupt store fails with kIOError.
///
...
public:
    ...
    /// injected. Stored fp32 blocks carry their stored bounds; the
    ...
    /// The cumulative hit/miss counters and wall times (8.7).
    /// \returns The stats.
