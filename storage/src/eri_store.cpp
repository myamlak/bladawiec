...
// checksum, verified on serve exactly like the values chunk (st-1).
...
//
// Exhaustive BY CONSTRUCTION (row 22), and this was the one labeler of
...
// Creates the empty /derived staging slots (the 8.6 contract; each carries
...
LoadImpl(const EriStore::State& state, const std::vector<CanonicalQuartetInfo>& request) {
    ...
    for (const std::size_t chunkId : chunkOrder)
    {
        ...
        if constexpr (kCertified)
        {
            ...
            // (st-1). An unverified bound would silently under-report the
