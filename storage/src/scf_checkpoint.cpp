...
// layout, verified on load - st-3). The file may be a fresh file or an
...
// The /scf/state checksum (st-3): FNV-1a-64 over the fixed-order byte
...
qcx::Result<void> SaveScfCheckpoint(const std::filesystem::path& path,
    ...
    // (st-2): WriteHistory declares the {k,n,n} dataspace from n
    ...
    // The state checksum (st-3): FNV-1a-64 over the fixed-order
...
qcx::Result<qcx::scf::ScfRestartState> LoadScfCheckpoint(const std::filesystem::path& path,
    ...
    // validation and the checksum comparison follow outside it
    // (st-3). A malformed checkpoint must surface as a clean
    ...
    // Shape validation (st-3): the density must be square {n,n}
    ...
    // The checksum (st-3): recompute over the read-back data in
