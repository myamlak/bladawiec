...
// The two specializations with real bodies today: the
// missing-combination case is a deliberate compile error, not tested here.
...
// mem-B1 coverage gap: BuildSparsityFromAdjacency passed neighbor indices
// through without column validation (only the tree builder checked its
// range), so an out-of-range packed index reached the payload-scatter loops
// and read or wrote past the payload arrays. Create now rejects any index
// >= rowCount up front.
...
TEST(SparsityPatternTest, ContractionPolicyDenseIsPlainProduct) {
    // The Dense x NoSymmetry specialization is the thin pass-through of
    // exactly a * b - what every existing Fock/RI call site
    // already computes directly - nothing more.
