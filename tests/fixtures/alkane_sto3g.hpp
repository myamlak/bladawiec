...
// The STO-3G alkane fixtures (the spatially extended molecules the QFMM
// acceptance suite is exercised on): C_nH_{2n+2} built programmatically for
// any carbon count at a PERFECTLY tetrahedral geometry - the backbone C-C-C
// angle is the tetrahedral angle (109.47 deg, so the planar trans zigzag's
// internal carbons host exact tetrahedral C-H directions out of the
// backbone plane) and the terminal carbons get the exact tetrahedral 3-H
// fan. The carbon text is the vendored data/basis/sto-3g/C.nwchem values
// (8 significant digits); hydrogen comes from h2_sto3g.hpp. Test-only
