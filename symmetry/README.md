...
detector, character tables, and general SALC construction. The module had no
production consumer until symmetry-blocked SCF diagonalization was wired through
`scf/`; the integral-side symmetry reduction (petite lists, pair classes) lives
in `integrals/` and `scf/`, not here. This module does **not** reduce integrals
and does not run SCF.
