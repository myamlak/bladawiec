...
std::string SerializeRunResultJson(const RunResult& result) {
    ...
    // file, which is the check the 2026-09-15 hunt was missing.
    ...
    // fabricated block.
    //
    // The far field's accuracy budget rides the same block rather than a
    // second one, because it is a fact about the same build and is read
    // beside the same model: the schema-37 members carry the preset budget
    // the far field was held to and the a-priori truncation bound's verdict
    // on it, so a run whose far field missed that budget states the miss
    // instead of leaving a consumer to infer it from an accuracy it cannot
    // see. They are written on every present block, with the arm's own flag
    // as the answer for the builds that did not arm it - the
    // `certified_bound` rule, and the reason none of the seven is omitted.
    if (result.qfmmModel.has_value())
    {
        const auto& model = *result.qfmmModel;
        document["qfmm_model"] =
            nlohmann::json{{"extent_model", model.extentModel},
                           {"separation_mode", model.separationMode},
                           {"separation_k", model.separationK},
                           {"theta", model.theta},
                           {"error_aware_admission", model.errorAwareAdmission},
                           {"far_field_budget", model.farFieldBudget},
                           {"per_interaction_budget", model.perInteractionBudget},
                           {"geometric_far_pair_count", model.geometricFarPairCount},
                           {"pairs_moved_to_near_field", model.pairsMovedToNearField},
                           {"fell_through_to_cap", model.fellThroughToCap},
                           {"worst_truncation_bound", model.worstTruncationBound}};
    }
