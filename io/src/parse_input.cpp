...
// this design exists to forbid (the pyscf-default and the
// Bohr-in-an-Angstrom-file mistakes were the same one).
...
bool BuilderConsumesAux(BuilderKind builder) noexcept {
    ...
    // driver refused every ri_jk request by name until that same wiring, so
...
qcx::Result<RunInput> ParseRunInput(std::string_view tomlText) {
    ...
    // the Bohr-in-an-Angstrom-file mistake this key exists to end.
