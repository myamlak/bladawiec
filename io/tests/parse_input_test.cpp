...
// null-vs-fabricated policy. The parser tests use one exact H2 example
// throughout; the serializer tests pin the JSON shape (keys, RHF
...
TEST(RunInputParseTest, RejectsOutOfContractRiTensorKnobValues) {
    // The strict side of the rung contract: a knob outside its
...
TEST(RunInputParseTest, MissingMethodTypeHintNamesTheWholeAcceptedVocabulary) {
    ...
    // missing key has to name all four too. This said "rhf | uhf" once,
    // which means a reader who omitted the key while wanting rks
...
// Bug-hunt io-2 coverage: a non-finite number must not silently serialize
...
// would have caught the Bohr-in-an-Angstrom-file mistake, where Bohr numbers
// were fed to the Angstrom-default key and a whole defect hunt chased a
// stretched molecule (and its mirror, pyscf's Angstrom default against
// qcx's Bohr convention).
//
