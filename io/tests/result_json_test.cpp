...
TEST(ResultJsonTest, SchemaVersionIsThirtySix) {
    ...
    EXPECT_EQ(qcx::io::RunResult::kSchemaVersion, 36);
}
...
TEST(ResultJsonTest, ModeRecordBlockEmittedWhenPresent) {
    ...
    // The class-path admission-gate fields (the pair-class root fix):
    // the table's Create-time charge and the disengagement
    // flag.
    ...
    // The UHF exchange half: a UHF run's Fock builds from
