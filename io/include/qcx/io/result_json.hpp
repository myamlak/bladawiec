...
/// engine seam's own record; io stays pure). RETAINED SHAPE, NO
...
/// read against (schema 31, the ri_jk track's disclosure plumbing).
///
...
/// cell's output and in prose (the track's plan paragraph and the I6 lane
/// record); the mapping beside it carried the BAR and no achieved value at all.
...
/// screened exchange at the same preset, measured in the ri_jk track's accuracy
...
/// exact quartets - the different-result-class problem this key answers by
...
struct RunSelection {
    ...
    /// file-version record the seam kept, never a fabricated default).
...
struct RunModeRecord {
    ...
    bool tensorExcluded = false; ///< Exclusion (ii, RI-J): the tensor term alone
                                 ///< cannot fit — the light rung is mandatory.
    std::size_t classTableBytes = 0; ///< The A3b pair-class table's Create-time structural
...
struct RunQfmmModel {
    ...
    /// of, and the default since 2026-09-15) or "kMidpointBound" (the
    ...
    /// nodes' charge radii plus a buffer - the default since 2026-09-15) or
    ...
    /// Whether the error arm was armed (schema 37): the far field's admission
    /// consulted the preset's error budget as well as the separation test, so
    /// the interactions it admitted are the ones the budget certifies rather
    /// than everything the geometry admitted. False on every build that did not
    /// ask for the arm, which is the default at every preset - and false is
    /// then a statement about the ARM, never about the budget: the members
    /// below record the bound's verdict on that build either way.
    ///
    /// Emitted on every block rather than omitted when false (the
    /// `certified_bound` rule): the budget members below carry their zero
    /// defaults on a build that did not arm it, and a reader must take this
    /// flag as the answer instead of inferring the budget's state from an
    /// absent key.
    bool errorAwareAdmission = false;
    /// The preset's far-field error budget, in hartree (schema 37): the whole
    /// budget the far field was held to, of which `per_interaction_budget` is
    /// one interaction's share.
    double farFieldBudget = 0.0;
    /// The error share each far interaction was allowed, in hartree (schema
    /// 37): `far_field_budget` split over the geometrically admissible pairs.
    /// It reads 0 when there are no far pairs, the absence of a share rather
    /// than a share of zero.
    double perInteractionBudget = 0.0;
    /// The far field's own interaction count (schema 37): the pairs the
    /// separation test admitted, and the denominator the share above was split
    /// over. It is the GEOMETRIC count - the far field the build would have had
    /// without the error arm - so it is unchanged on a run whose arm moved
    /// pairs away, which `pairs_moved_to_near_field` carries beside it.
    std::size_t geometricFarPairCount = 0;
    /// How many pairs the separation test admitted and the budget refused
    /// (schema 37), computed into the near field instead. Zero on a build that
    /// did not arm the error arm, and on an armed build whose budget admitted
    /// the whole geometric far field.
    std::size_t pairsMovedToNearField = 0;
    /// How many far interactions ran at the order cap with their a-priori
    /// truncation bound still above their share of the budget (schema 37): the
    /// fall-through that used to be silent. Zero while
    /// `error_aware_admission` is true, because that arm admits only what the
    /// bound certifies - so a nonzero count here names a build the bound does
    /// NOT certify. It is not the measured error: the bound is a
    /// worst-case-over-distributions bound rather than a prediction, so a
    /// nonzero count beside an inside-budget energy is the honest record of
    /// exactly that and not a contradiction between the two.
    std::size_t fellThroughToCap = 0;
    /// The worst interaction's truncation bound at the order cap, in hartree
    /// (schema 37): the largest bound over the far field, and the number
    /// `per_interaction_budget` is read against - divide it by that share for
    /// how far outside the budget the worst interaction sits. It reads 0 when
    /// there are no far pairs: the absence of a worst interaction, never a
    /// certified one.
...
struct RunResult {
    ...
    ///
    /// Schema 37 adds the far field's accuracy budget to the `qfmm_model`
    /// block (the seven RunQfmmModel members from `errorAwareAdmission` down):
    /// whether the error arm was armed, the preset budget the far field was
    /// held to, the share each far interaction was allowed, the geometric
    /// far-pair count the share was split over, the pairs the budget moved into
    /// the near field, the interactions that ran at the order cap with their
    /// bound above that share, and the worst such bound. The engine computed
    /// all seven already and the builder's own record carried them; only the
    /// serializer dropped them, so a run whose far field missed its budget was
    /// indistinguishable in the document from one that met it. Keys added, so
    ...
    /// that date is not comparable with one after it.
    ...
    /// model, the separation test with its buffer, the resolved theta, and
    /// (schema 37) the far field's accuracy budget with the a-priori bound's
    /// verdict on it.
