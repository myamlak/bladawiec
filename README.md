...
A C++23 quantum-chemistry framework: SCF (RHF/UHF/RKS/UKS) over a module DAG, with direct and
density-fitted Fock builds, a quadrature fast-multipole far field, exact and screened integral
engines, and a certified mixed-precision core.

C++ Core Guidelines; native Windows/MSVC as the primary build target, with Linux and macOS CI for
parity.

**API documentation:** <https://myamlak.github.io/bladawiec/>

**Reference values** in the regression suite are checked against published results and, where a
numerical anchor is needed, against independent reference programs. The implementation is written
from the published algorithms; no code is taken from another implementation.
...
- **[Input file reference](https://github.com/myamlak/bladawiec/blob/main/docs/input-guide.md)** — every section and key, with a worked example.
- **[Result reference](https://github.com/myamlak/bladawiec/blob/main/docs/output-guide.md)** — the report file and the JSON document.

## Documentation

API documentation is generated with Doxygen over the public headers and the pages in `docs/`:
...
See [`CONTRIBUTING.md`](https://github.com/myamlak/bladawiec/blob/main/CONTRIBUTING.md) for the house rules (naming, formatting, error
handling, tests) and the versioning policy, and `CITATION.bib` for the
published work the algorithms, kernels and vendored data derive from.
