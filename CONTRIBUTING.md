...
This repository accepts contributions from its collaborators. Read this whole
file before sending a change — a few of the rules here are unusual, and they
exist because this is a numerical library whose numbers are its contract.

If you are not a collaborator and want to report a bug or propose a change,
open an issue (see the Getting help section): pull requests from outside
the collaborator set are not accepted.
...
4. **State your evidence.** If you add an algorithm, a kernel or a fitted
   constant, say in the pull request where it came from and what verified
   it — a published paper, a reference implementation, a finite-difference
   check, an analytic derivation. An addition with no stated evidence will
   be asked for one.
5. **Regeneration is local-only.** Anything generated and committed —
...
6. **Small, reviewable changes.** One logical change per pull request.
...
- **Formatting:** `.clang-format` is not advisory — 4-space
...
- **Comments:** brief, self-contained and why-focused. Cite published work
  by its citation key from `CITATION.bib` — never an internal
  document.
- **Line endings:** LF everywhere, per `.gitattributes`.
...
- References to internal development: internal paths, decision numbers,
  stage or track names, or notes about how the work was organized.
...
BSD-3-Clause — see `LICENSE`. By contributing, you agree that your
...
`CMakeLists.txt`, the `Doxyfile` and `CITATION.cff`,
...
Pull requests are accepted from collaborators only; the maintainer reviews and
merges every change to `main`.
