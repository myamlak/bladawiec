...
/// the CRTP lands when UHF/DFT make the hook set real, and it reuses these
/// steps unchanged.
/// \{
...
struct RhfOptions {
    ...
    /// path for a while - the loop assigned its previous-iteration energy
    /// before the gate read it, so the leg compared the fresh energy
    /// against itself and every positive value passed. Both legs of the
    /// gate are live now, so a tighter value buys more iterations and a
    /// lower energy.
    double energyTolerance = 1e-8;
    double densityTolerance = 1e-6; ///< RMS density change (gates both DIIS and plain paths).
    ...
    /// These are logged for the calibration population - nothing acts on
...
struct HfResult {
    double totalEnergy = 0.0; ///< Electronic plus nuclear repulsion (Hartree) of the density this
                              ...
                              ///< so it is not the DIIS-extrapolated Fock's trace of
    ...
    Eigen::MatrixXd density; ///< Spin-summed closed-shell density D = 2 C_occ C_occ^T -
                             ...
                             ///< returned \c coefficients' first numOccupied columns (the
                             ///< converged-state contract): the exit builds this
    ...
    Eigen::MatrixXd coefficients; ///< MO coefficients C (n x n) of the density this result
                                  ...
                                  ///< trajectory's returned density. Columns ascending
    ...
    /// density leg alone decided every RHF stop.
    ...
    /// The returned state's FIXED-POINT residual: the Frobenius norm
    ...
    /// truncation would be indistinguishable from a correct run - an
    /// enforced choice is used, refused by name, or demoted with the
    /// disclosure.
    ///
...
/// Per-iteration Fock-matrix builder callback (the loop's builder seam).
/// scf cannot link integrals (module DAG), so the direct/RI Fock builders of
/// qcx-integrals are wired in by the caller (the tests and the driver).
///
...
/// both paths: the RHF energy leg was inert for a while, because the loop
/// assigned its previous-iteration energy before the gate read it, which
/// left the density leg deciding every RHF stop on its own. With no
/// restart seed (empty
...
/// builder (the loop's builder seam).
///
...
/// today, the driver). Convergence gating, the CDIIS path, and the
