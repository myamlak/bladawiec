...
/// shift-value sweep (recorded at execution).
/// \ingroup qcx-scf
...
/// robustness knobs land in Step D) and a shared base buys nothing today.
/// \ingroup qcx-scf
struct UhfOptions {
    ...
    /// both spins' Fock histories (niedoida's joint form). Replaces the two
    /// per-spin extrapolators when useDiis is also set; the two-phase cap
    /// is not ported to the joint form (the combination is rejected). Off
    /// by default - per-spin CDIIS stays the default.
    ...
    /// point (the placement). The shift is skipped on the first
    ...
    /// Optional per-spin starting densities (the Step-C initial guesses:
    /// GWH and SAD). Empty matrices mean the core guess P = 0; a non-empty
    ...
    /// Maximum-overlap occupation selection [Besley2009] (Step E): when
...
/// to use (the diagonalization blocking, requested by supplying the basis
...
///
/// The guard exists because the equivalence is conditional. "U^T (X^T F X) U
...
struct UhfResult {
    ...
    Eigen::MatrixXd densityAlpha; ///< Converged alpha density D_alpha = C_alpha,occ
                                  ...
                                  ///< returned coefficients (the converged-state contract), so the
                                  ///< two describe one state. Not the trajectory's final density.
    ...
    Eigen::MatrixXd coefficientsAlpha; ///< Alpha MO coefficients (n x n) of the returned alpha
                                       ...
                                       ///< orbital energy (the density-consistent rule; returned
    ...
    Eigen::VectorXd orbitalEnergiesAlpha; ///< Eigenvalues of the seed pair's Focks, ascending
                                          ///< to the
                                          ///< returned coefficients' orbital energies. Not the
    ...
    /// The returned alpha state's PROJECTOR residual (the converged-state contract): the Frobenius
    ...
    /// The returned alpha state's FIXED-POINT residual (the converged-state contract): the
    ...
    /// The alpha channel's symmetry-blocking disclosure (the guard; see
...
/// Runs UHF with a caller-provided two-Fock builder (Step F's seam - the
/// UHF analog of rhf.hpp's FockBuilderFn overload).
///
/// Same procedure as the supermatrix overload, but each iteration's
/// (fockAlpha, fockBeta) pair comes from \p fockBuilder (Step F's seam):
/// the direct/RI Fock builders of qcx-integrals plug in here (tests today,
/// the driver in Phase 1). Convergence gating, the per-spin CDIIS path,
...
/// non-trivial seed for Step-C guess tiers.
/// \param overlap Overlap matrix S (n x n).
/// \param coreHamiltonian Core Hamiltonian H = T + V (n x n).
...
/// DAG; the seam pattern), one entry per distinct element of the
...
/// coefficients (the Step-E MOM reference handoff).
/// \ingroup qcx-scf
struct SadGuess {
...
/// niedoida spherical-averaging procedure (AverageOccupations: fractional
/// occupations over the highest partially filled shell, core 1.0) and the
/// deliberate deviation: the atomic spin polarization is
/// PRESERVED (each fragment's P_alpha/P_beta scaled by the molecular
/// N_alpha/N_beta over the atomic totals), where niedoida spin-unpolarizes.
