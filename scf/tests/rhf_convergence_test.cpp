...
// A cross-check against the CCCBDB references exposed a real scf-core bug -
// qcx RHF on H2O/aug-cc-pVDZ (41 functions) failed the 100-iteration budget with a
...
// production path the validation runner uses), at both accuracy presets, with the
...
// H2O at the CCCBDB HF/aug-cc-pVDZ optimized geometry, converted from
// angstrom to bohr.
...
TEST(RhfConvergenceTest, CcpvdzControlConvergesToThePyscfPin) {
    ...
    // The cc-pVDZ control (the validation runner's reproduction baseline): the same shared
    ...
    // energy differences 8.6e-7..3.3e-6 (the 90d40fa s40
    // re-pin precedent). The engagement-gate removal (rhf.cpp: the
    ...
    // with the engine the same test code converges to -76.027052802530704
    ...
    // stay bit-identical under the engine.
