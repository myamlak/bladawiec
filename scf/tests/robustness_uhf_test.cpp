// Robustness-heuristics tests: the two-phase per-spin DIIS, the virtual-space
// level shift (with the shift's energy-bias accounting), the three-way
// convergence gate, and the joint-system DIIS
// re-evaluation, all behind the opt-in UhfOptions flags. The asserted rows use the
// standard pin style (energy 1e-7 / <S^2> 1e-6); the remaining harness rows
// (seed x accelerator x gate matrix, shift sweep, direct-path legs) are printed
// by benchmarks/scf_robustness_benchmark.cpp and recorded - the
...
// the joint-system re-evaluation is the mandated next move), and the joint-system
// rows join the comparison table (P0JointSystemDiisOutcome). The heavy O2 rows are
// fast-mode-gated like the
...
// composed-configuration probe of the exit criterion
...
// The O2/STO-3G SAD guess (the uhf_test.cpp helper; the core runs
...
// the section-1c assembly fock_sigma = coulomb + exchangeSigma - H (the
// double-H trap).
...
TEST(RobustnessUhfTest, O2TwoPhaseDiisConvergesToThePin) {
    ...
    // The unpolarized SAD start + two-phase per-spin DIIS: the
    ...
    // PRODUCTION DEFAULTS (owner ruling 2026-09-15): this row's pin is the
    ...
    // The band is the 2026-09-06 pin-downgrade (a documented re-pin): the
    // certified-lane per-range band's within-budget
    // ~1e-15-level value shifts flip this run's DIIS landing to a
    // near-degenerate-cluster member 2.21e-7 above the pin
    // (-147.63394656436637; the loose-gate cluster) - identical on the
    // corrected kernels, where the pre-band kernel pinned
...
TEST(RobustnessUhfTest, LevelShiftBlockForm) {
    // The block-form identity: for an S-orthonormal coefficient set
...
TEST(RobustnessUhfTest, LevelShiftConvergesToTheUnshiftedPin) {
    ...
    // The bias-absence pin: CDIIS + the level shift (1.0) on the
    ...
    // the shift's bias term 0.5·Delta·Tr[D(S−D)] is structurally absent; the
    ...
    // pair this test used to bind is RETIRED), and the two contracts below
    // are now split by what each of them can mean at it (2026-09-20). The
    // 2026-09-15 reading stands as the measurement it was: at the
    // production defaults this run's <S^2> landed at 2.0034083280071258,
    // 2.5297e-6 from the pin, 2.53x the 1e-6 band it then carried.
    //
    // The ENERGY keeps its 1e-7 band unweakened, because the bias-absence
    // claim is STRUCTURAL and the energy is stationary at the fixed point:
    // it is second-order in the density error, so the gate does not reach
    // it. Measured on this tree at the rung, the shifted run (28
    // iterations) and the unshifted run (34) land 4.95e-11 apart - a 2000x
    // margin on a contract that was never in question.
    //
    // <S^2> IS NOT an energy and does not inherit that. It is a bilinear
    // functional of the stopped density, so it is FIRST-order in the
    // density error and moves at the gate's own landing scale: measured
    // here, the unshifted run stops with a density RMS change of 2.46e-10
    // and lands 9.41e-8 from the pin, while the shifted run stops at
    // 1.07e-7 (a later iterate of the same trajectory) and lands 2.5297e-6
    // from it. A 1e-6 pin on a first-order property, at a gate whose
    // density leg is 1e-6, was never a contract the gate could keep - it
    // was a 1e-10-gate reading, like the bound it replaced.
    //
    // WHAT THE <S^2> ASSERTION STILL IS. The pin sits in this file's
    // triplet basin; the O2 two-solution landscape's other row, the P = 0
    // saddle, sits at kO2SaddleSpinSquared = 2.0127238687916638, 9.3e-3
    // away. The band is therefore restated at 1e-4 - the band this file's
    // own basin rows already use for the same quantity (the saddle rows,
    // the shift sweep, the joint-DIIS table) - and that is a LOOSENING
    // from 1e-6, stated plainly: the assertion is a BASIN-MEMBERSHIP
    // contract now, not a 1e-6 pin. It still asks the question this test
    // exists to ask - the level shift must not walk the trajectory onto
    // the saddle - with 93x margin against that separation and 40x against
    // the measured rung landing. The pin itself is NOT re-taken:
    // kO2PinnedSpinSquared is a physical value shared with the other O2 pin
    // tests, and a stopped value of a first-order functional is not a
    // better estimate of it than the tight-gate record was.
    qcx::scf::UhfOptions options;
    ...
    EXPECT_NEAR(result->spinSquared, kO2PinnedSpinSquared, 1e-4);
}

TEST(RobustnessUhfTest, RobustGateTruthTable) {
    // The gate's truth table: each leg's veto fires independently, the
    ...
    // equal-legs pair on purpose and are registered as a non-gate in
    // tools/gate_sites_not_gates.txt. The four constants below are derived from
...
TEST(RobustnessUhfTest, O2PinHoldsUnderTheRobustGate) {
    ...
    // The gate's pin test: CDIIS + the robust gate on the unpolarized SAD
    ...
    // PRODUCTION DEFAULTS (owner ruling 2026-09-15): measured on this tree
    ...
    // the true fixed point (the value lands in the measured record).
...
TEST(RobustnessUhfTest, P0SaddleLockingNotRescuedByComposedConfigurations) {
    ...
    // of the accelerators {plain, CDIIS-8, two-phase} x the
    ...
    // saddle placement). The criterion is not met: the failure is recorded
    // as the activation gate firing (mode (b)) and the joint-system DIIS
    // re-evaluation is the mandated next move.
    // A future rescue (that re-evaluation or another change) fails this test
    ...
    for (const int mode : {0, 1, 2}) // 0: plain, 1: CDIIS, 2: two-phase.
    {
        ...
        {
            // PRODUCTION DEFAULTS (owner ruling 2026-09-15): measured on
    ...
    // the red the gate conversion found. The leg joins the saddle-row
...
TEST(RobustnessUhfTest, JointSystemDiisConvergesToThePin) {
    ...
    // The joint form's destination pin on the easy seed: the joint-system DIIS
    // (one subspace over the combined alpha+beta error vectors, one
    // coefficient vector applied to both spins' Fock histories - the joint
    // form) on the unpolarized SAD start converges to
...
TEST(RobustnessUhfTest, P0JointSystemDiisOutcome) {
    ...
    // The per-spin-vs-joint comparison on the firing case: the P = 0
