// THE BLOCKING GUARD'S PIN, on the canonical broken-symmetry system: the
...
struct RunOutcome {
    ...
    // The run's own blocking-guard disclosure (what it requested, what the
    // guard decided, and the norms it decided on) - asserted below, because
...
void ReportCase(const CaseMeasurement& measured) {
    ...
    std::printf("  L1 OffBlockNorm(F_a,F_b) at the converged PLAIN solution: %.3e  %.3e"
                "   (the block argument needs the ~1e-12 noise level)\n",
...
TEST(UhfSymmetryConstraintTest, StretchedH2BrokenStartBlockedVersusPlain) {
    ...
    // The two paths from the same symmetry-adapted start agree: the guard's own
    ...
    // (1) The guard's precondition is violated on this solution: the converged
...
// the same symmetry-adapted solution, which is the case blocking covers.
TEST(UhfSymmetryConstraintTest, EquilibriumH2BrokenStartRelaxesToTheSameSolution) {
    ...
    // the agreement above would prove nothing about blocking), and one that refused
