...
//   - the labels of synthetic states built from the symmetrized basis U
...
// The synthetic SCF state on the symmetrized basis U: F = U diag(eps)
...
// The basis blocks mix full-group irreps whenever a computational row
...
// Every column of every block carries the same full-group label (exact
...
TEST(FullGroupLabelsTest, LabelVectorOffTheScfPath) {
    ...
    // point documents is not the rule in force there (the disclosure discipline).
...
TEST(FullGroupLabelsTest, FullereneIhSynthetic) {
    ...
    // Distinct energies everywhere; the Fock's eigenbasis is the basis
    ...
    for (const auto& label : result->labels)
    {
        // A column whose block MIXES two full-group irreps has no single
        // irrep of its own, and REPORTS UNKNOWN for it rather than naming
    ...
    // the eigenbasis recovers the columns; straddled subspaces are left
    // verbatim.
...
TEST(FullGroupLabelsTest, AlleneD2dS4AxisPrincipalStandardAndTilted) {
    ...
    // pin covers assignment flips; a regression confined to the S4-wins
    // rule alone does not flip allene's assignment (the fallback and the
    // residual lex ordering pick the same winner) - the
...
// D2-family principal rule. The allene fixture executes the S4-wins return
// but would survive its removal: allene's S4-axis C2 fixes all three
// carbons, so the fallback (the atom-fixed counts) elects the same
// principal. Here the S4 axis (z) carries NO atom - the C2 on it fixes
// nothing - while each C2' (the xy diagonals) passes through two Cl atoms
// and fixes them, so the fallback's counts elect a C2' as principal. Only
// the S4-wins rule (an improper order-4 class on the axis wins the tie)
// yields the physically-correct principal, the C2 on the S4 axis; a
// regression that drops that rule makes the class-size gate fire (the C2
// class has size 1, the C2' class size 2).
// ---------------------------------------------------------------------------
...
TEST(FullGroupLabelsTest, D2dSyntheticS4AxisPrincipalStandardAndTilted) {
    ...
    // The synthetic-state stage run for one orientation: labels plus the
    // realization the class-key assertions read. Running the stage IS the
    // rule pin HERE: a regression that deletes the S4-wins return
    ...
    const auto expectLabelsFromTable = [&table](const std::vector<std::string>& labels) {
        ...
        {
            // The blocks span A1+B1 and A2+B2 (see below), so a column of a
            // MIXED block has no single full-group irrep and reports
            // UNKNOWN for it. Any label that IS named must be a table irrep.
            if (label == std::string(kUnknownLabel))
            {
                continue;
            }
    ...
    // are NOT orientation-invariant: the blocks span A1+B1 (the 11-column
...
//
// The fixture is the deck the measurement covered: two H
