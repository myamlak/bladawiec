// THE BLOCKING GUARD ON THE RESTRICTED PATH: measured, and NOT ported.
//
// The unrestricted path carries a guard (uhf.cpp's diagonalizeFock):
// each iteration measures whether its Fock still commutes with the
...
// guard, and that gap was left open,
// naming RhfOptions::initialScfState as the exposure.
//
...
// The default GWH start the loop uses when no restart state is supplied, as a
...
// The scan's running tally. The defect signature looked for here is a
...
// on the rhf.cpp blocking-guard harness.
