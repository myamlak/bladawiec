// The RHF period-2 seed-fallback tests: the 96-record validation sweep found
...
// diis_test.cpp precedent) and (b) the 4 records as ctest fixtures
...
// The CO/6-311G** cycle members: the parity-pinned 2-cycle pair.
...
// The record geometries (H&H 1979 constants, angstrom -> bohr; the
// record manifest's cartesian_angstrom, verbatim).
...
// Runs the DEFAULT-path direct RHF (no initialScfState, so the loop takes
// its own GWH start) at kNormal on the direct Fock path and returns the
...
// The 4 two-cycle records as fixtures: psi4 references baked from the
// validation record at the 2e-6 budget. The pre-fix failure mode is
...
// average (measured) converges in budget. References: the psi4 energies
// of the validation record, full precision, at the 2e-6 budget.
