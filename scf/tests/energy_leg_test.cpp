...
//
// The deferral is closed: before the fix, RunScfLoop booked
// previousTotalEnergy before calling IsConverged, so the gate's energy leg
// compared the fresh energy against itself and [scf] energy_tolerance was
// inert on every RHF run - 1e-3, 1e-8 and 1e-10 gave bit-identical results
// while the walk reported converged: true.
//
...
//      former driver exactly, whose inert energy leg left the total
...
// (the former ordering did exactly that) it would read 0.0 and pass.
...
// iteration 19). The 106-BF table carries the same statistic at
...
// argument: with the former bookkeeping order restored in
// scf/src/rhf.cpp - the energy leg reading the fresh energy against itself -
// both rows land orders of magnitude outside 1e-8. The numbers are the
// injection probe's, reverted after the measurement.
//
...
/// The 1e-10/1e-10 pair is RETIRED under the owner ruling of 2026-09-18 (no
...
qcx::Result<WalkOutcome> Walk(const ScfInputs& inputs,
                              // The two thresholds are the stopping points the
                              // doc comment above names, and every caller below
                              // passes the energy one first.
                              //
