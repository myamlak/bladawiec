...
//
// THE QUESTION. Does DIIS/CDIIS behave on DFT's
...
// it reports are record, and the assertions are the invariant that
...
// reading (grid noise can only reach the stop through the density leg)
...
TEST(RksConvergenceProbe, ToleranceSweepOnOneSystem) {
    ...
    // this measurement, not a gate the probe assumes: the sweep's subject is
    // the (energy x density) tolerance
...
// 0.80 the registry then shipped; 0.08 since 4bddaceb folded it) with GGA
...
//            weights (0.08/0.72 since 4bddaceb), PW91 correlation instead of
...
// it is pyscf run with the weights the registry shipped BEFORE 4bddaceb,
