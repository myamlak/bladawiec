...
// (2.0034108576810308, tolerance 1e-6), the initial-guess tiers
// (GWH and SAD must reach the same minimum in FEWER iterations than the
// core P = 0 start), the MOM driver, and the rejection paths.
// The seeded run uses the unpolarized SAD start (see the recorded
// finding: the literal P=0 start cannot converge this system's two-solution
// landscape), and the guess tiers run the plain iterator against the
...
// DIIS on): the stage-plan baseline the guess tiers must beat.
...
// The O2/STO-3G SAD guess (the fragment runs + embedding; the core test
// seeds the UNPOLARIZED variant - the alpha/beta average - which
// reproduces the minao-like unpolarized start the pyscf reference run
// itself used; see the recorded finding for why the literal P=0 start
// cannot converge this system).
...
TEST(UhfTest, O2TripletConvergesToPinnedEnergy) {
    ...
    // The core run: all-default options except the start. The literal P=0
    ...
    // the recorded finding). The unpolarized SAD start (alpha = beta = the
    // SAD average) is what the pyscf reference run's default minao guess
    ...
    // chaotic-trajectory instance of the class the record documents for the
    // direct path).
    ...
    // PRODUCTION DEFAULTS (owner ruling 2026-09-15): the shared pin's band
    // is the cluster scale (5e-6 energy / 1e-6 <S^2>), which covers the
    // member the default gate stops on - measured PASS on this tree
    // 2026-09-15. The band is the 2026-09-06 pin-downgrade (a documented
    // re-pin): the certified-lane per-range band's within-budget
...
TEST(UhfTest, O2TripletCarriesTheConvergedState) {
    // The state handoff (UhfResult trailing fields): per-spin
...
// The P=0 saddle of the O2/STO-3G two-solution landscape (the recorded
...
// The P=0 baseline the guess tiers must beat: plain iteration under
...
// certified-lane kernels flip the second fact on windows-msvc (the
// 2026-09-06 pin-downgrade, a documented re-pin): the
...
// kO2P0SaddleTotalEnergy in 26 iterations - the pre-band kernel
// pinned the family 5/5 while the corrected kernels land
...
qcx::scf::UhfOptions MakeProductionDefaultsOptions() {
    // THE PRODUCTION DEFAULTS, by owner ruling (2026-09-15): "This is
    ...
    // site states the property the default cannot assert (2026-09-15:
    // three sites do).
    //
    ...
    // (-147.63394682034689). The two guess-tier rows that were red at HEAD on
...
TEST(UhfTest, GwhGuessReachesSameMinimumInFewerIterations) {
    ...
    const qcx::scf::UhfOptions baselineOptions = MakeProductionDefaultsOptions();
    ...
    // 1e-8/1e-6 defaults on 2026-09-15 (MakeProductionDefaultsOptions above), and
...
TEST(UhfTest, SadGuessReachesSameMinimumInFewerIterations) {
    ...
    // The POLARIZED SAD guess (the deliberate deviation from
    // spin-unpolarized fragments stays); as with GWH, the polarized start
    // runs plain against the P=0 baseline.
    const auto sad = BuildSadGuessForO2(*molecule, *basis);
    ASSERT_TRUE(sad.has_value()) << sad.error().message;
...
TEST(UhfTest, MomKeepsTheSadReferenceState) {
    ...
    // The maximum-overlap occupation rule keeps the SAD fragment reference's
    // occupation pattern: the fragment alpha/beta sets pair up, so the run
    // converges to the reference-character state with the ideal-triplet
    // <S^2> = 2.0 - NOT to the pinned state (<S^2> = 2.0034). Without MOM
    // the same start converges the pin, so the <S^2> assertion is the
...
TEST(UhfTest, NegativeLevelShiftIsRejected) {
    // Level-shift validation: a negative shift value is refused up
...
TEST(UhfTest, LevelShiftWithMomIsRejected) {
    // Level-shift validation: the shift's zero block is defined
...
TEST(UhfTest, JointSystemWithTwoPhaseDiisIsRejected) {
    // Joint-system validation: the two-phase cap is per-spin (each
    // channel's own phase handoff window); the joint form couples the
    // channels into one subspace with one window (the joint-system DIIS: the
    // two-phase cap does not combine with the joint form) - the combination
...
TEST(UhfTest, JointSystemWithoutUseDiisIsRejected) {
    // Joint-system validation: the joint extrapolator engages through
...
TEST(UhfTest, TwoPhaseDiisWithoutUseDiisIsRejected) {
    // Two-phase validation: the two-phase handoff logic lives on the DIIS
...
TEST(UhfTest, ZeroLevelShiftRunsAsANoOp) {
    // Level-shift validation: a zero shift is a legal no-op - the run converges
