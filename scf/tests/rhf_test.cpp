...
TEST(RhfTest, NearSingularOverlapIsRejected) {
    ...
    // overlap to either SCF loop. REVISED (2026-09-14)
    // (2026-09-14): the old response was to abort the run. A linearly
...
TEST(RhfTest, RemovedDirectionsRecoverTheFullSpaceAnswer) {
    // THE ACCEPTANCE COMPARISON for the linear-dependence removal (2026-09-14;
    // the owner's ruling: "a run that merely proceeds is not the deliverable;
    // a run that proceeds AND gets the same answer is").
    //
...
TEST(RhfTest, H2Sto3gCarriesTheConvergedState) {
    // The state handoff (HfResult trailing fields): the result
