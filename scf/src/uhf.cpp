...
// the <S^2> diagnostic, and the initial guess tiers (GWH, SAD).
...
// BLAS above): the density build is BLAS-shaped, following
// rhf.cpp's routing. Below the threshold the
...
// One spin channel's PROJECTOR residual: the Frobenius norm
...
// contraction rides the linalg seam (BLAS-shaped, following rhf.cpp's
// routing): Eigen below the threshold - bit-identical to the direct
// product - vendor BLAS above. The X^T (...) X pair stays Eigen: it runs
// in the serial section and is the blocked diagonalization's concern.
...
// The CDIIS window lengths and the two-phase handoff threshold. None of the
// three has a published source; each is a fixed engineering constant, and
// the criterion stated with it is what selects it.
//
// kDiisHistoryLimit - the full window. The DIIS system carries one
// coefficient per stored pair, and stored error vectors approach linear
// dependence as the SCF converges, so a pair appended past the point where
// the history still spans distinct directions adds a near-dependent row to
// the constraint matrix: it lowers the system's smallest singular value
// without adding a direction to the extrapolation. The window is therefore
// BOUNDED rather than extended, and the bound is on the history LENGTH: 8
// pairs (8 dense N x N matrices per spin, the memory side of the same
// bound) keeps the (m+1)-dimensional system far from the near-singular
// solve while still spanning enough iterations for the extrapolation to see
// the trajectory's direction.
constexpr std::size_t kDiisHistoryLimit = 8;

// The engagement window: the minimum history length
// that extrapolates at all. One pair fixes no direction (its DIIS system
// solves to the unit coefficient and the extrapolation returns that Fock
// unchanged), so two pairs are the first history a combination other than
// the identity exists for. The loop runs on exactly that minimum, because
// far from convergence the error vectors have no consistent direction and a
// wide fit over them extrapolates the noise. UHF runs no engagement
// threshold of its own: the extrapolation engages as soon as the history is
// ready, the floor guard excepted.
constexpr std::size_t kDiisPhaseHistoryLimit = 2;

// kDiisPhaseThreshold - the phase-1 to phase-2 handoff. The operand is the
// per-spin CDIIS error norm in the orthogonal basis, the Frobenius norm of
// X^T (F D S - S D F) X - the same quantity the floor guard measures - and
// the phase-1 window hands off to the full one when that norm drops below
// the threshold. The criterion is an order-of-magnitude bracket on the
// trajectory's decay, not a fitted value: the guess starts this norm orders
// of magnitude above 0.5 and the run's convergence thresholds sit at 1e-8,
// so 0.5 (in the Fock's own units) marks the first digit or so of decay -
// late enough that the wide window's coefficients are not fitted to a
// directionless trajectory, early enough that the wide window carries most
// of the convergence. The flip is one-way, with no hysteresis: once the
// full window is in force it stays.
constexpr double kDiisPhaseThreshold = 0.5;

// The two-phase DIIS wrapper of one spin channel: a phase-1 extrapolator
// with the minimum history (kDiisPhaseHistoryLimit) until the spin's error
// norm drops below kDiisPhaseThreshold, then a one-way handoff to the full
// kDiisHistoryLimit window. The handoff snapshots the phase-1 extrapolator
// and restores the state into a full-window one (the Snapshot/Restore
// round-trip is bit-identical, DiisTest.SnapshotRestoreRoundTrips), so no
// history entry is lost or duplicated. The phase logic lives entirely here;
// no public DiisExtrapolator surface carries it.
struct TwoPhaseDiis {
...
// solve can then pick the wild first Fock (a singular solve would have to
// drop one such vector to proceed; dropping the degenerate one up front,
// before it enters the history, is the equivalent). The
...
// Runs the engaged extrapolation path for one iteration: the
// joint system extrapolates BOTH spins from one coefficient vector over
// the combined error subspace (the joint form), where the per-spin
...
// the joint-system hypothesis. The two-phase handoff is not ported (the
...
// The maximum-overlap occupation mask, Besley, Gilbert & Gill, J. Chem.
// Phys. 130, 124308 (2009) [Besley2009]: with C0 the reference (guess)
// occupied block and C the current MOs, the projected overlap
// O = C0_occ^T S C measures how much of each current MO lies in the
// reference occupied space, and a current MO's share of that space is the
// squared norm of its column of O. The method occupies the numOccupied MOs
// with the largest shares: the state is held by the orbital space that most
// resembles the reference's, so the run converges to the state the
// reference describes rather than collapsing onto the ground state.
...
Eigen::VectorXd MomOccupationMask(const Eigen::MatrixXd& coefficients,
    ...
    const Eigen::MatrixXd projected = referenceOccupied.transpose() * overlap * coefficients;

    // (share, MO index) pairs, sorted by descending share; the sort is not
    // stable, so MOs whose shares tie are ordered by the sort's own
    // algorithm rather than by index.
    ...
    {
        projections.emplace_back(projected.col(j).squaredNorm(), j);
    }
...
qcx::Result<UhfLoopResult> RunUhfLoop(const qcx::molecule::Molecule& molecule,
    ...
    if (options.useLevelShift && options.useMom)
    {
        ...
        // scope (recorded in the D-record), so the combination is refused.
        return std::unexpected(qcx::Error{qcx::ErrorCode::kInvalidArgument,
    ...
    // truncation, so it is refused by name instead. The test is exact rather
    // than
    ...
    // The blocking guard's per-spin disclosure state, declared beside the
    ...
    // returned orbital spectra follow the returned densities (the owner's
    // ruling of 2026-09-13 - the CONTRACT change, rhf.cpp's twin block carries
    // the derivation). The converged-state contract closes the second half:
    ...
    // form the nonlinear Vxc leg requires; scoped out.
    //
    ...
    const auto reportReturnedDensityEnergy =
        ...
        // owner's ruling of 2026-09-13): the returned MO sets are the returned
        ...
        // in D - and bounded by it: T2's HF one-pass exactness).
    ...
    // Joint-system DIIS: when options.useJointDiis, ONE
    // extrapolator over the combined alpha+beta error vectors replaces the
    // two per-spin ones (the joint form). The restart state's two
    ...
    for (int iteration = 0; iteration < options.maxIterations; ++iteration)
    {
        ...
        // its own density goes into its own extrapolator) or the joint path
        // over the combined error subspace, dispatched in
        // MaybeExtrapolateFockPair.
        ...
        // the P = 0 record is structurally absent; shifting after the
        ...
        // coefficients and is skipped on iteration 0 (there is no previous
        // C to build the projector's occupied block from).
        ...
        if (converged)
        {
            ...
            // The reported energy is re-formed from the returned
    ...
    // The budget exit reports the returned density's energy too
...
// The aufbau (Madelung) filling order of a neutral atom and its angular
// degeneracies, GENERATED from the rule rather than transcribed - the rule
// is the provenance: the (n, l) shells sorted by increasing n + l, ties
// broken by increasing n, each carrying its angular degeneracy 2l + 1 (the
// shell's spin-orbital count; its electron capacity is twice that). The
// first kAufbauShellCount entries run 1s, 2s, 2p, 3s, 3p, 4s, 3d, ... and
// end at 7p, the last shell a neutral atom fills across the periodic table.
constexpr int kAufbauShellCount = 19;

constexpr std::array<int, kAufbauShellCount> BuildAufbauShellStates() {
    std::array<int, kAufbauShellCount> states{};
    int shell = 0;

    for (int sum = 1; shell < kAufbauShellCount; ++sum)
    {
        // l = sum - n must satisfy l <= n - 1, i.e. n >= (sum + 1) / 2; the
        // first such integer is (sum + 2) / 2 in integer arithmetic, and
        // ascending n from there is the rule's tie-break at fixed n + l.
        for (int n = (sum + 2) / 2; n <= sum && shell < kAufbauShellCount; ++n)
        {
            states[shell] = 2 * (sum - n) + 1;
            ++shell;
        }
    }
...
constexpr std::array<int, kAufbauShellCount> kAufbauShellStates = BuildAufbauShellStates();

// The highest partially filled shell of a neutral atom, with the average
// occupations the atomic fragment run uses on it. The orbitals below the
// shell are the closed core (two electrons each); the shell's own d orbitals
// carry one fractional occupancy per spin.
//
// The fractions are DERIVED, not taken from a published parametrization - no
// published source defines this averaging for an atomic fragment - from the
// invariance the open shell of a free atom must have: the atom may not single
// out an orientation within its own shell, so the shell block of each spin
// density must be unchanged by any rotation of the shell's orbitals. Those
// orbitals span an irreducible representation of the rotation group, so by
// Schur's lemma the only invariant shell block is a multiple of the identity
// there: all d orbitals carry the same occupancy n_sigma / d, fixed by the
// n_sigma electrons of that spin in the shell. Hund's rule fixes the spin
// split of the shell's n electrons (it fills singly to half), so
// n_alpha = min(n, d) and n_beta = max(n - d, 0), and the two fractions
// follow. The same invariance is what makes a real atom's open shell
// spherically symmetric rather than directional.
struct DegenerateShell {
    int noCoreStates; // Doubly occupied orbitals below the shell.
    int noDegStates; // Shell degeneracy d.
    int noDegElectrons; // Electrons n in the shell, at most its capacity 2d.
    double averageOccupancyAlpha; // Per-orbital alpha occupancy, min(n, d) / d.
    double averageOccupancyBeta; // Per-orbital beta occupancy, max(n - d, 0) / d.
};

// The shell holding the atom's last electrons: walk the aufbau order
// accumulating the shells the electrons complete (2d each, two per orbital -
// the Pauli capacity) and stop at the first shell they do not complete. That
// shell is the highest partially filled one; the electrons accumulated below
// it form the core, half as many orbitals as electrons; the electrons left
// over are the shell's own count.
qcx::Result<DegenerateShell> FindDegenerateShell(int electronCount) {
    int electronsBelow = 0;

    for (int shell = 0; shell < kAufbauShellCount; ++shell)
    {
        const int degeneracy = kAufbauShellStates[shell];
        const int capacity = 2 * degeneracy;

        if (electronCount - electronsBelow > capacity)
        {
            electronsBelow += capacity;
            continue;
        }

        const int inShell = electronCount - electronsBelow;
        const double alphaElectrons = static_cast<double>(std::min(inShell, degeneracy));
        const double betaElectrons = static_cast<double>(std::max(inShell - degeneracy, 0));

        return DegenerateShell{electronsBelow / 2,
                               degeneracy,
                               inShell,
                               alphaElectrons / degeneracy,
                               betaElectrons / degeneracy};
    }

    return std::unexpected(qcx::Error{qcx::ErrorCode::kUnimplemented,
                                      "atoms beyond the last aufbau shell (7p) are not supported"});
}
...
qcx::Result<AtomicFragmentResult> RunAtomicFragment(int atomicNumber,
    ...
    // The fragment multiplicity from the degenerate-shell occupancy, by
    // Hund's rule: the shell's n electrons occupy distinct orbitals of the d
    // degenerate shell singly until it is half full, so below the half fill
    // all n are unpaired and above it the unpaired count is the holes,
    // 2d - n. The multiplicity is the unpaired count + 1 (2S + 1 with
    // S = unpaired / 2) - the ground-state multiplicity of the neutral atom.
    const int unpaired =
        std::min(shell->noDegElectrons, 2 * shell->noDegStates - shell->noDegElectrons);
    const int multiplicity = unpaired + 1;
    ...
    // The fragment's occupation vectors: 1.0 on the doubly occupied core
    // orbitals below the shell, the shell's own average occupancy on its d
    // orbitals (n_sigma / d - the rotation-invariance derivation on
    // DegenerateShell).
...
qcx::Result<SadGuess> BuildSadGuess(const qcx::molecule::Molecule& molecule,
    ...
    // A deliberate deviation from the spin-unpolarized parent-atomic-density
    // form of the SAD guess (both spins given the same spatial density, the
    // atomic density split by the alpha/beta electron fractions): here the
    // atomic spin polarization is PRESERVED - each fragment's P_alpha/P_beta
    // is scaled by the molecular N_alpha/N_beta over the atomic totals - so
    // an open-shell atom seeds a polarized start carrying the fragment's own
    // spin structure into the molecular run.
