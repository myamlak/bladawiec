...
// AO-space symmetry decomposition and blocked diagonalization (the
// first pass, diagonalization-only scope): when the computational point
...
// How far one Fock matrix is from being symmetry-adapted - the
// precondition, measured rather than assumed.
//
// The equivalence rests on ("U^T (X^T F X) U is block-diagonal up to
...
/// Measures the precondition of one Fock (see SymmetryAdaptation).
///
...
// The relative infinity norm of the off-block part of
