...
// FockBuilder<Derived> CRTP can absorb these
...
// 1e-4, measured against the stress case at execution. The leg guards
// NON-STATIONARY premature declaration (the
// direct-path 2.2e-7-off case); it cannot discriminate
...
// at execution): the M2 records pin their 2-cycle energies to 2e-13
...
// "quiet" cycle alternating by < 1e-6 Eh is deliberately NOT rescued.
// Lower the floor, never the parity tolerance, if one appears.
...
// The monitor-only measures of one extrapolation's coefficients (the
// the ruled trace increment; the coefficient-trajectory analysis d4c7362
// verdict): negativeMass = (sum|c_i| - 1)/2 - the total magnitude of the
...
// reversal-invariant anyway). The d4c7362 verdict is explicit that these
...
// The per-iteration trace writer (memory-anomaly diagnostics): a
...
// run. The dense n^4 pair is explicitly temporary: RI-J replaces it
// in Phase 1.
...
// optimization (honoured, refused by name, or demoted with the
// disclosure).
//
...
// The virtual-space level shift [niedoida shift_fock_matrix.cpp, verbatim
// form; hoisted from uhf.cpp's local copy to the shared seam]:
// F += (S·C)·B·(S·C)^T with B = shift·I and B(i,i) = 0 for the
// numOccupied lowest orbitals - a positive shift raises the virtual
// energies above the occupied block, damping the density oscillation of
// the plain iterator on the stress case (the P=0 O2 record). A zero
// shift is a legal no-op. Consumers: the UHF per-spin path under
// UhfOptions::useLevelShift and the RHF cycle-escape trajectory of
// rhf.cpp. The numOccupied parameterization is the documented
// future-ROHF hook of the two-shift decomposition (niedoida
// rohf.cpp: shift_1 on the single Fock over the alpha occupied set, then
// the difference form shift_2 - shift_1 over the beta set) - not active
// behavior; the deferral is recorded.
inline void ApplyVirtualSpaceLevelShift(Eigen::MatrixXd& fock,
    ...
    // The shift's B block lives in the ORTHONORMAL dimension, which the
    // linear-dependence removal makes smaller than n: coefficients is
    // n x numKept, so B is numKept x numKept (the plain square case is
    // numKept == n and this is unchanged).
    const Eigen::Index numKept = coefficients.cols();
    const Eigen::MatrixXd a = overlap * coefficients;
    Eigen::MatrixXd b = shift * Eigen::MatrixXd::Identity(numKept, numKept);

    if (a.rows() != n || numOccupied > numKept)
    {
...
// The joint-system DIIS extrapolator [niedoida diis.cpp:111-204, read-only
// reference]: ONE DIIS subspace over the combined alpha+beta error vectors.
// The B-matrix entries are the symmetrized sum 0.5·(<eAlpha_i, eAlpha_j> +
// <eBeta_i, eBeta_j>), one Lagrange constraint, and a single coefficient
// vector c applied to both spins' Fock histories (new_F_sigma =
// sum_i c_i · F_sigma,i) - the channels are coupled through the shared
// coefficients, where the per-spin extrapolators minimize each spin's own
// error in its own subspace, blind to the other channel. The two-phase cap
// is NOT ported to the joint form (one kDiisHistoryLimit window); the COD
// solve and the floor guard substitute for niedoida's LU singular-pivot
// ejection (the recorded equivalence).
struct JointSystemDiis {
    // historyLimit: the full kDiisHistoryLimit window (the two-phase cap is
    // not ported to the joint form - the two-phase design).
    ...
    // Extrapolates BOTH spins from the shared coefficient vector: the
    // joint B-system [[0.5·(<eA_i,eA_j>+<eB_i,eB_j>)], -1; [-1^T, 0]] with
    // the unit-sum constraint, solved as a minimum-norm least-squares
    // problem (the COD - the same substitution the per-spin path uses),
    // and the one coefficient vector applied to each spin's own Fock
    // history.
    qcx::Result<std::pair<Eigen::MatrixXd, Eigen::MatrixXd>> Extrapolate() const {
        ...
        for (Eigen::Index i = 0; i < m; ++i)
        {
            ...
            {
                b(i, j) = 0.5 * ((_errorHistoryAlpha[static_cast<std::size_t>(i)].array() *
                                  _errorHistoryAlpha[static_cast<std::size_t>(j)].array())
                                     .sum() +
                                 (_errorHistoryBeta[static_cast<std::size_t>(i)].array() *
                                  _errorHistoryBeta[static_cast<std::size_t>(j)].array())
                                     .sum());
            }
        ...
        const Eigen::VectorXd coefficients = b.completeOrthogonalDecomposition().solve(rhs);
...
// Runs the joint-system DIIS extrapolation when engaged (Step 5): the
...
// The generalized period-k stationarity watchdog: the
...
// (detectedPeriod = 2), and the firing behavior is unchanged. When the
...
// strict > for the shorter-lag floor, matching the detector. The
...
// converged-state contract, and the instrument that made the O2 triplet
