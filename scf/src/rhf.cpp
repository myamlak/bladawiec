...
// The returned state's PROJECTOR residual (the converged-state contract): the Frobenius norm
...
// The escape rung's virtual level shift: 1.0 Ha - the ladder's
// recorded middle value (uhf.hpp kLevelShiftDefault), provisional-measured
// per the convention. No recorded case exercises the RHF rung yet
...
// engaged on exactly the near-degenerate systems that need it;
// pyscf's CDIIS extrapolates unconditionally from its second
...
qcx::Result<HfResult> RunScfLoop(const qcx::molecule::Molecule& molecule,
    ...
    // the finalizer must re-derive the labels AFTER it rebinds the returned
    ...
    // BuildGwhGuess, the Step-C tier) instead of the zero matrix. The
    ...
    // between -93.23 and -58.38 Eh against the true -76.041844; the same
    // lesson was recorded on O2), and it feeds the DIIS history an exactly-zero
    ...
    // previousDensity - the literal expression (see the seeding
    ...
    // fields are no-ops, so default-constructed options take the start.
    ...
    // is bit-identical to the behavior). applyLevelShift turns the
    ...
    // placement - the shifted matrix never enters the energy path, so the
    ...
    const auto runTrajectory = [&](bool applyLevelShift,
        ...
        // the finalizer re-derives it from the returned-density Fock after
        ...
        for (int iteration = 0; iteration < options.maxIterations; ++iteration)
        {
            ...
            // the placement (uhf.cpp's comment, shared seam). finalFock
            ...
            // The bookkeeping, AFTER the gate. The comparison above
    ...
    // the reported numbers change, the walk does not. The rungs are
    ...
    // energy leg.
    //
    // The orbital spectrum follows the energy. The returned MO coefficients and
    // orbital energies are the returned-density Fock's eigenpairs. The trajectory's
    ...
    //
    // The converged-state contract closes the other half of the same class: the returned DENSITY
    ...
    const auto reportReturnedDensityEnergy = [&](HfResult& result) -> qcx::Result<void> {
        ...
        // The orbital spectrum is RE-DERIVED rather than carried over: the returned MO
        // set is the density
        ...
        // in the residual and bounded by it - the contract's HF one-pass exactness).
    ...
    // moved into its result). Period 2 is the literal expression - the
    ...
    // on the four records); periods 3 and 4 generalize the average
