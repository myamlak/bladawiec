...
density-grid charges, ESP fits) live in `properties/`. It does **not** evaluate
integrals (that is `integrals/`) and does not run SCF.
...
- `grid/include/qcx/grid/internal/lebedev_tables.hpp` and
  ...
  (`--check` mode is part of the regeneration check) —
  never hand-edit the generated tables.
