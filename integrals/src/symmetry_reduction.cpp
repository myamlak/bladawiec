...
// is pinned by the hand-built pair list in the integrals test). The block
// value of a canonical quartet is invariant under the
...
// call site states the storage it needs: the allocation family a container is
...
// land in the class pair's orbits vector, so the class_table allocation family
...
// return, and the class_table allocation family charges the TABLE the trace reports
...
qcx::Result<ClassMaterializationCounts> CountClassMaterialization(
    ...
    // allocation family: the family measures the TABLE the trace reports, and these
...
void GenerateClassPairOrbits(PairClassTable& table,
    ...
    // The class_table allocation family: the on-demand orbit entries (and their
