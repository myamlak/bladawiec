...
struct DirectJkFockBuilder::State {
    ...
    // SparsityPattern representation is a follow-on wiring). Optional
    ...
    // The LightPath state:
    // the geometry-only light store (one MdPairData per pair, the
    ...
    // runtime loop never re-derives k from the live Remaining (the
    // cumulative high-water accounting is tight - the charge authorized
    // exactly this many slots' live mass).
    std::size_t _maxConcurrentSlots = 1;
    // Calibration term G: the per-canonical-pair
...
struct RunPassPartial {
    ...
    // Chunk-local per-element filter drop count (TRAP 4 - the
...
// starting at 0 within the batch - a batch-local layout, per batch).
...
// member quartet contracts in the precision its own certified gate routed it to
...
struct ClassRepTask {
    ...
    // The streaming-rung partial: the screened members' positions in
...
std::vector<ClassRepTask> BuildClassRepTasks(PairClassTable& classTable,
    ...
    // The streaming-rung partial: the screened-member position
...
// The class-task sort (the streaming-rung partial): sorts one
// precision half's class task list IN PLACE into the canonical rep
// emission order - the same five-key comparator SortScreenedTasks uses over
...
// The bounded-concurrency batch loop: the batch-order
...
struct FockContractor {
    ...
    // false reproduces exactly today's behavior). The vector's
    // lifetime is the BuildFock call that built it - never cached across
    // calls (TRAP 5: the incremental wrapper alternates full D and Delta-D).
    ...
    // re-derives k from the live Remaining (the cumulative high-water
    // accounting is tight: the charge authorized exactly this many slots'
    // live mass).
    std::size_t _maxConcurrentSlots = 1;
    ...
    // Assembles ONE class batch into batch-scoped buffers - the batch
    ...
    qcx::Result<AssembledHalf> AssembleBatch(internal::MdClassBatch batch,
        ...
        if (fp32)
        {
            ...
            // :1425 - are all fp64-branch-only, as a reader sweep
            // confirms). Skipping the resize drops the per-call 8*T
    ...
    // matrix - the per-batch analog of the earlier merged contract: the
    // batch's own index space chunked across threads
    ...
    // batch-major across the call, bit-identical to the earlier serial
        ...
        } else
        {
            ...
            RunPassPartial delta = internal::FixedOrderReduce<RunPassPartial>(
                ...
                [&](const RunPassPartial& partial, std::size_t chunkIndex) -> RunPassPartial {
                    ...
                    // A per-chunk FRESH contractor, bound to the
    ...
    // class is gone by construction). The earlier merged single
    // fork/join becomes per-half, per-batch parallel
    // regions; the serial fallback contracts in batch-major order, which
    // IS the earlier whole-call canonical order (each batch's tasks are
    ...
    qcx::Result<void> RunPass(const std::vector<internal::MdQuartetTask>& fp64Tasks,
        ...
        auto runHalf = [&](const std::vector<internal::MdQuartetTask>& tasks,
            ...
            {
                // The cache decorator, sliced over the sorted master
                // (the streaming-rung positional order): the decorator
                // materializes its request's
                ...
                // order - bit-identical to the earlier cached path. The
                // slice cap counts the lane's OWN request bytes (the
                // values-term increment): the fp64 values mass
            ...
            // incremental partitioner (the streaming-rung partial):
            // MdClassBatchCursor emits the
            ...
            // contracted immediately - the per-batch unit is unchanged.
            ...
            // The bounded-concurrency batch loop:
            ...
            // contract: at k = 1 the loop below is the shipped batch
            // sequence verbatim, no gate, no per-slot region-thread
            ...
            // under the backend's team-size policy.
            ...
            // it, discard it - the exact per-batch sequence of the
            // streaming unit. \p batchIndex is the batch's
            ...
            if (kEff <= 1)
            {
                // k = 1: the shipped per-batch sequence verbatim (no
    ...
    qcx::Result<void> RunClassPass(const std::vector<ClassRepTask>& classTasks64,
        ...
        auto runHalf = [&](const std::vector<ClassRepTask>& tasks, bool fp32) -> qcx::Result<void> {
            ...
            // SortClassRepTasks before the pass ran - the class-path
            // sort): position t of repTasks is the orbit rep of
            ...
            // k = 1 the loop below is the shipped per-batch sequence verbatim.
            internal::MdClassBatchCursor cursor(
            ...
            auto processBatch = [&](internal::MdClassBatch&& batch,
                ...
                // bound (weight(member) * bound(orbit rep), the certified
                // composition).
...
// so a shared binding would race (the per-chunk-contractor
...
qcx::Result<void> ContractOne(const FockContractor& contractor,
    ...
    if (fp32)
    {
        ...
        // error, consumable as certified. The weight is the
        // positional transport:
        // fp32Weights is parallel to the canonical-sorted fp32 master
...
// The LightPath light store: one geometry-only MdPairData per canonical
// pair - exactly the
...
// reused containers are not Reservations - the cumulative high-water
// accounting sees one peak,
...
qcx::Result<DirectJkFockBuilder> DirectJkFockBuilder::Create(
    ...
    // The adaptive-memory mode decision: the Create-time footprint
    // estimate against the budget's
    ...
    // The class-path admission gate (the pair-class root fix,
    ...
    // keeps the unconditional engagement, bounded only by the
    ...
    if (options.workspaceBudget != nullptr)
    {
        ...
        // budget-clamped. k == defaultTeamSize means the
        // fired k was the whole ceiling-clamped team, not a budget split.
        ...
        // fit (the refusal fires only when the
        // light rung ALSO cannot fit, below). The bound is the COUNTED
        // pattern - the Schwarz-exact survivor count under the
        // BuildNeighborList cutoff (the exclusion fix): the all-survive
        // form over-refused
        ...
        // RI-J's nested exchange half reaches its rung through here: with
        // the count skipped, the sweep below
        ...
        if (!lightPath)
        {
            ...
            // half and the QFMM near-field) carries no exchange mass, so
            // the term would be a phantom charge.
            // The bounded-concurrency batch loop: the
            // exchange term is also the slot-authorized term - k slots only
            // on the budgeted fast path with the exchange engaged and the
            // cache off (the cache-engaged, Coulomb-only and LightPath
            // rungs run k = 1). The slot lift extends
            // the authorization to a pre-reserved nested half
            ...
            // LightPath rungs) are unchanged - the k=1 pins stay
            // green by construction. k = min(max(1, floor(remaining /
            ...
            if (!fastFired)
            {
                ...
                // LightPath DECISION with the counted terms (the refusal
                // fires only when the
            ...
            } else
            {
                ...
                if (!budget.Reserve(info.predictedBytes))
                {
                    ...
                    // the decision and the reserve (the accounting is monotone -
        ...
        if (lightPath)
        {
            ...
            if (chunkPairs == 0)
            {
                ...
                // the light model - exchangeEngaged false is threaded
                // explicitly).
                ...
                // screened-quartet task machinery rides them too (the
                // whole-run charge): the sorted task
            ...
            // The fewer-bytes gate (the brief's nested contract): the light
    ...
    // slack to kNormal's cutoff (a pre-push self-review catch,
    // 2026-08-23: the kTight slack was void, 1e-12 instead of 1e-14).
    ...
    // The engine tier's engagement - the in-memory ERI-value cache
    // and the engine-decorator seam that can stand under
...
qcx::Result<qcx::memory::Tensor<double, 2, qcx::backend::CpuTag>> DirectJkFockBuilder::BuildFock(
    ...
    // never the sum; the cumulative high-water accounting sees one peak, not
    // a sum over chunks). The
    ...
    // The lane's device-governed default (the device probe's verdict,
    // a 2026-09-12 ruling) resolves at the ONE point
    ...
    // the density-weighted bounds below (the fix of 2026-08-21: the
    ...
    // max |D_block| per canonical pair (TRAP 5: rebuilt per call - the
    ...
    // The certified global budget enforcement (opt-in, FastPath only): one walk
    ...
    // The distinct-pair stamping (calibration term P): one stamp per canonical
    ...
    // passes POSITIONALLY (the streaming-rung partial):
    // the fp32 master is sorted ONCE per pass
    ...
    // The ladder pass: the B_screen co-term from the
    ...
    auto runAssembly = [&](const std::vector<internal::MdQuartetTask>& f64Tasks,
        ...
        {
            // The plain pass over both precisions (the merged pass,
            // restructured per-half, per-batch): each half
    ...
    if (lightMode)
    {
        ...
        for (std::size_t c = 0; c < numChunks; ++c)
        {
            ...
            // the sorted fp32 master (fp32DensityWeights - the
            ...
            // containers are not Reservations - the cumulative high-water
            // accounting sees one peak,
            // not a sum over chunks).
