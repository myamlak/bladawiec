// The host orchestration of the CUDA MD ERI lane: the device kernels and the
// class table live in eri_cuda.cu and the generated
// eri_cuda_dispatch_gen.hpp. Responsibilities:
//
...
//     helper dispatches the CPU kernels; a deliberate deviation from the
//     CPU assembly - the sort and the run/batch split are copied verbatim).
//
...
Result<void> RunBatchOnDevice(const EriCudaOptions& options,
                              ...
                              // the task/ranges uploads of this batch
    ...
    if (family == kEriCudaFamilyV2)
    {
        ...
        // no padding). The primCount key is part of the per-(rowAB, primB)
        // slot layout -
...
struct EriCudaEngine::Impl {
    ...
    /// The Create-time device footprint estimate, filled
    ...
    /// The Create-time mode decision: the FastPath/LightPath decision
    ...
    // The per-call transfer accounting: the Fock-path traffic sums of the
    // last BuildFock call. Reset at every
    ...
    // (the host/device-split instrument, benchmarks/gpu_eri_benchmark.cpp):
...
Result<EriCudaEngine> EriCudaEngine::Create(const qcx::molecule::Molecule& molecule,
    ...
    // bytes of the host tables just built, the fixed Boys copy, the
    // per-call working set and the structural allowance. Computed here -
    // before the first device allocation - so the device-budget refusal can
    // fire with the estimate in hand and no device memory committed.
...
struct GpuJkFockBuilder::State {
    ...
    // CSR the CPU builder constructs, so both builders screen the same
    // candidate lists (the CPU-vs-device parity gate).
...
qcx::Result<void> GpuJkFockBuilder::State::RunDevicePass(
    ...
    // host/device-split instrument of benchmarks/gpu_eri_benchmark.cpp):
...
qcx::Result<GpuJkFockBuilder> GpuJkFockBuilder::Create(
    ...
    // Forward the device workspace budget and the
    // forced-LightPath knob into the engine options - the Create-time mode
    ...
    // same candidate lists (the CPU-vs-device parity gate).
...
qcx::Result<qcx::memory::Tensor<double, 2, qcx::backend::CpuTag>> GpuJkFockBuilder::BuildFock(
    ...
    // The per-call device-timeline span:
    ...
    // The lane's device-governed default (the device probe's verdict of
    ...
    // The ladder pass: the B_screen co-term, the budget, and
    ...
    // The residency-contract debug assert: every Fock-path transfer site
    // is wired into the accounting, so the
