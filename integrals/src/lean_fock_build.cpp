...
//
// Point-group symmetry reaches this path in two depths (both off by
// default): the
...
// arithmetic, not in IEEE double - the 2026-09-12 amendment). The
...
// The generous margin of the Create-time last-resort memory check: the
// builder refuses only when the simple-actuals
...
// BOUND of that state by construction (a capability must never be gated on a
// never-under bound).
//
...
// memory audit) charges that ledger itself.
...
// The window oversubscription factor of the auto row-window count: with
// auto chunks the row space is cut into this many windows per thread, and
// the window loop is scheduled
...
// floor of the 8-16x band: enough windows for the dynamic
...
// fixture is UNCHANGED call wall - while
...
// The row-window plan of one build: how many windows the row walk is cut
// into and how many threads the window region
// runs with.
struct WindowPlan {
...
// Plans the row windows of one build.
//
...
// Narrows a window plan to a smaller window count, under the host plan's own
// thread rule: never more threads than windows and never more than the host
// plan runs, so the family is the host's rule with the window count moved
// down and nothing else. At or above the host window count the host plan
// comes back unchanged, so a search that never descends is the plan the
// uncapped path runs. The bottom of the family is the one-window plan - the
// same plan the k = 1 pin names, one window on the calling thread with no
// OpenMP involvement - which is the smallest decomposition the builder can
// execute: a window is the unit of the row split, so there is nothing below
// one window.
// \param hostPlan The plan the host's own rule gives (PlanWindows).
// \param windowCount The window count to narrow to.
// \returns The plan of that window count.
WindowPlan NarrowedPlan(const WindowPlan& hostPlan, std::size_t windowCount) noexcept {
    if (windowCount >= hostPlan.windowCount)
    {
        return hostPlan;
    }

    const std::size_t windows = std::max<std::size_t>(1, windowCount);
    const std::size_t hostThreads = static_cast<std::size_t>(std::max(hostPlan.threadCount, 1));

    return {windows, static_cast<int>(std::min(windows, hostThreads))};
}
...
// whose flat clear() keeps the bra transform's CAPACITY and which since the
// chunked-teardown change deliberately leaves the per-prim-pair containers -
// their nested braWeights / ketTransforms vectors and the three per-axis
// E tables each element carries - uncleared. The chunked sweep measured the
// c24h50/def2-SVP store at ~41 MB retained per store (13.5 MiB of it the
// E tables alone) against the 1.57 MB the flush cap below charges, so
...
// any retained allocation (measured: the retention added
// ~3.2 MB of live payload per window at C4H10/def2-SVP and moved the
...
// \param plan The row-window plan the call will run (PlanWindows, or the
// narrower plan a ceiling selects - ChooseCeilingPlan - which keeps the same
// shape: the window count, whose wave width PlanWaveWidth derives and the
// accumulator term multiplies, and the region's thread count, which is the
// concurrency bound of both the window-local scratch and the auto wave
// width).
...
PeakEstimate EstimatePeakBytes(const qcx::molecule::Molecule& molecule,
    ...
    // thread count of them exist at once - the plan's threadCount, which is
    // min(the window count, the team) for the auto rule and min(the narrowed
    // window count, that same thread count) for a ceiling-narrowed plan. The
    // legacy per-window terms below stay at the full window count
    // (never-under, and at one window per thread the two counts coincide).
    ...
    // (the estimate correction): the Fock accumulator
    ...
    // touches (charge what the algorithm realizes, and justify any
    // oversubscription per term). The chunk is the same 512 MiB
...
// driver's pre-ramp consult (EstimateLeanEnvelope) - call it, so the early
// decision and the builder's own can never drift apart. A ceiling of 0 is
// no ceiling: no check, no text.
//
// The text names TWO requirements: the one the plan the ceiling selected
// carries, and the one the HOST's own plan carries (the plan the same
// envelope is computed for when no ceiling binds). The pair is what makes a
// refusal readable after the fact - the first number is the requirement of
// the plan the run would have used, the second the requirement the machine's
// own decomposition had - and the two are equal whenever no ceiling narrowed
// the plan.
// \param peak The envelope of the selected plan (EstimatePeakBytes).
// \param hostPlanBytes The envelope of the host's own plan, in bytes.
...
std::optional<std::string> LeanCeilingRefusal(const PeakEstimate& peak,
                                              double hostPlanBytes,
                                              double memoryCapGiB) {
    if (memoryCapGiB <= 0.0)
    {
        return std::nullopt;
    ...
    return std::string("lean-direct last-resort memory refusal: required_peak_bytes ") +
           std::to_string(static_cast<long long>(peak.totalBytes)) + ", host plan " +
           std::to_string(static_cast<long long>(hostPlanBytes)) + " exceeds available_bytes " +
...
// The plan a call runs and its envelope: the host's own plan when no ceiling
// binds, and otherwise the WIDEST plan that still clears the ceiling -
// searched down from the host's plan to the smallest decomposition the
// builder can execute (one window, NarrowedPlan's floor) - so a build that
// cannot afford the machine's own decomposition runs a narrower one instead
// of refusing, and refuses only when no plan at all fits.
//
// The search spends the budget on the WINDOW COUNT because that is the
// plan's memory knob: both plan-derived terms of the estimate (the live
// wave's accumulators plus the live windows' scratch, and the live windows'
// retained store payload) are non-decreasing in the window count, since both
// the live-window count and the wave width are non-decreasing in it. The
// fitting window counts are therefore the range's prefix 1..k and the widest
// one is found by bisection over the SAME verdict the check applies
// (LeanCeilingRefusal) - and every plan the search returns has been through
// that verdict, so the search can never hand back a plan the check then
// refuses. EstimatePeakBytes stays the only judge of a plan's bytes: the
// search asks it, it never re-derives the arithmetic.
//
// The plan is a property of the BUDGET, not of the machine: two hosts with
// different core counts differ only in where the search starts, so a build
// that fits no plan at all is refused on every one of them (the smallest
// plan is the floor of every family), while a build that can afford a
// narrower decomposition runs it.
// \param molecule Molecule providing the atom coordinates (Bohr).
// \param basisSet Basis set (the primitive-count authority).
// \param pairList The canonical shell pairs.
// \param options The lean options (the ceiling and the pinned spellings).
// \returns The chosen plan, its envelope, and the host plan's envelope total
// (the second number the refusal text names).
struct CeilingPlan {
    WindowPlan plan; ///< The plan the call runs.
    PeakEstimate peak; ///< The envelope of that plan.
    double hostPlanBytes; ///< The host's own plan's envelope total, in bytes.
};

CeilingPlan ChooseCeilingPlan(const qcx::molecule::Molecule& molecule,
                              const qcx::basisset::BasisSet& basisSet,
                              const ShellPairList& pairList,
                              const LeanFockBuildOptions& options) {
    // The host's own plan first, always: it is the uncapped choice, the
    // starting point of a capped search and the number the refusal names.
    const WindowPlan hostPlan = PlanWindows(options, pairList.pairs.size());
    const PeakEstimate hostPeak =
        EstimatePeakBytes(molecule, basisSet, pairList, options, hostPlan);

    // No ceiling: the host's plan, byte for byte what the rule has always
    // given, and one estimate - no search is entered.
    if (options.memoryCapGiB <= 0.0)
    {
        return {hostPlan, hostPeak, hostPeak.totalBytes};
    }

    // The host plan clears its ceiling (the common case): nothing to buy.
    if (!LeanCeilingRefusal(hostPeak, hostPeak.totalBytes, options.memoryCapGiB).has_value())
    {
        return {hostPlan, hostPeak, hostPeak.totalBytes};
    }

    // The floor of the family: one window. When even this refuses, the plan
    // stays the smallest one and the verdict is the check's own - the same
    // refusal, on every host.
    WindowPlan best = NarrowedPlan(hostPlan, 1);
    PeakEstimate bestPeak = EstimatePeakBytes(molecule, basisSet, pairList, options, best);

    if (LeanCeilingRefusal(bestPeak, hostPeak.totalBytes, options.memoryCapGiB).has_value())
    {
        return {best, bestPeak, hostPeak.totalBytes};
    }

    // The bisection: [low, high] keeps a fitting count at low and a refused
    // one at high, so it converges on the widest fitting count.
    std::size_t low = 1;
    std::size_t high = hostPlan.windowCount;

    while (high - low > 1)
    {
        const std::size_t mid = low + (high - low) / 2;
        const WindowPlan plan = NarrowedPlan(hostPlan, mid);
        const PeakEstimate peak = EstimatePeakBytes(molecule, basisSet, pairList, options, plan);

        if (LeanCeilingRefusal(peak, hostPeak.totalBytes, options.memoryCapGiB).has_value())
        {
            high = mid;
            continue;
        }
...
qcx::Result<LeanEnvelope> EstimateLeanEnvelope(const qcx::molecule::Molecule& molecule,
    ...
    // The plan a call will run and its envelope (ChooseCeilingPlan - the rule
    // Create and BuildFock apply), so the per-window scratch term and the
    // retention term are the ones the run's own decomposition allocates, and
    // a ceiling that binds narrows the plan here exactly as it does in the
    // builder.
    const CeilingPlan choice = ChooseCeilingPlan(molecule, basisSet, *pairList, options);
    const PeakEstimate peak = choice.peak;

    LeanEnvelope envelope;
    envelope.totalBytes = peak.totalBytes;
    const std::optional<std::string> refusal =
        LeanCeilingRefusal(peak, choice.hostPlanBytes, options.memoryCapGiB);
...
struct LeanDirectFockBuilder::State {
    ...
    /// The row-window plan every BuildFock call runs (ChooseCeilingPlan): the
    /// host's own plan when no ceiling binds, otherwise the widest plan that
    /// clears the ceiling - the same plan peakBytes above was computed for,
    /// stored here so the build decomposes the row space the envelope
    /// charged and never re-plans per call.
    WindowPlan plan{1, 1};
...
qcx::Result<LeanDirectFockBuilder> LeanDirectFockBuilder::Create(
    ...
    // The orbit action (the petite-list mechanism): the tables the row walk's
    ...
    // The Create-time envelope, always computed (the driver reports it
    // through EstimatedPeakBytes); the check below
    // consumes the same number, so the record and the refusal can never
    // drift apart. The plan is the one a call will actually run
    // (ChooseCeilingPlan): the host's own plan when no ceiling binds, and
    // otherwise the widest plan that still clears the ceiling, so a build
    // that cannot afford the host's decomposition runs a narrower one
    // instead of refusing, and the per-window scratch term is the scratch
    // the call's own decomposition allocates while the retention term rides
    // the same windows' concurrency bound. The plan is stored in the state
    // below and is what BuildFock runs - it is never re-planned per call.
    const CeilingPlan choice = ChooseCeilingPlan(molecule, basisSet, *pairList, options);
    const PeakEstimate peak = choice.peak;
    ...
    // driver-side pre-ramp consult of the same envelope
...
qcx::Result<qcx::memory::Tensor<double, 2, qcx::backend::CpuTag>> LeanDirectFockBuilder::BuildFock(
    ...
    // The row windows: the plan chosen at Create (ChooseCeilingPlan -
    // PlanWindows when no ceiling binds, otherwise the widest plan that
    // clears the ceiling, down to the one-window plan; auto oversubscribes
    ...
    // variance across the team (round-robin over oversubscribed rows is good
    // enough once the schedule can rebalance). The plan is read from the
    // state rather than re-planned: it is the plan the Create-time envelope
    // above was computed for, so the run allocates what the admission
    // charged.
    const WindowPlan plan = state.plan;
    ...
    // The split probe: the two populated spans of the lean path. The ERI
    // span covers one flush's
    ...
    // The accumulation-side denominator (the mechanism-value audit,
    // 2026-09-13): one per ContractBlock call, i.e. per assembled class
    ...
    // The kernel-span probe's own partials (the kernel-span instrument): the three
    ...
    const auto runWindow = [&](std::size_t w, std::size_t accumulatorSlot) {
        ...
        const auto processFlush = [&]() -> qcx::Result<void> {
            ...
            // unbounded (the retention band). The band holds the
            ...
            // and the store element (fully rewritten, certified sums assigned),
        ...
        for (std::size_t row = rowBegin; row < rowEnd; ++row)
        {
            ...
            for (std::size_t q = 0; q <= row; ++q)
            {
                ...
                // orbit instead of once per cell (2,379,972 blocks over
                // 8,811,481 screened cells on c8h18/def2-SVP, 0.2701).
    ...
    // branch below); every plan the builder can hold hands back a one-window
    // plan only as a serial one - the k = 1 pin (an explicit
    // maxParallelChunks = 1), an auto call on a one-thread team, or a
    // ceiling-narrowed plan at the floor of its search (NarrowedPlan never
    // gives a window more threads than windows) - so the identity is pinned
    // here instead of assumed: a multi-window plan can never take a serial
    // path, and a one-window plan is never run on a team that could have run
    // windows in parallel.
