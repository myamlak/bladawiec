// The QFMM Coulomb-only Fock builder (qfmm_fock_build.hpp): per BuildFock the
// near-field direct build (the existing DirectJkFockBuilder in Coulomb-only
// mode, restricted to the octree near-field pair-pair set - the shared
// screening path decides what is actually contracted) plus the far-field
...
struct QfmmJBuilder::State {
    ...
    // The per-pair order bands: the adaptive per-far-pair truncation orders
    // (the M2L caps) and
    ...
    // The far-field chunk override: the QfmmOptions::maxParallelChunks value,
    // copied at Create for
    ...
    // What this build ran (the disclosed geometry-model default flip):
...
qcx::Result<QfmmJBuilder> QfmmJBuilder::Create(
    ...
    // The point-group reduction, refused by name (the disclosure contract named in
    // RiFullFockBuilder::Create: honoured, refused with its condition named,
    ...
    // builder enumerates at its own Create (the leaf-driven domain is that carrier).
    ...
    // geometric footprint to the
    // accuracy budget - a looser preset admits a larger missed-charge tail
    // at its own coarser budget and shrinks the boxes (more far-field
    // pairs), kTight keeps the skeleton's committed 1e-10 cutoff.
    // The geometry model: the recorded midpoint-padded bound by
    ...
    // An EXPLICIT theta keeps its recorded meaning, so no written accuracy
    // control is silently ignored once the surface test is the default
    // (QfmmOptions::theta == 0 is "not given" and leaves the model choice to
    // separationMode). theta > 0 selects the centre-to-width test at that
    // value - the historic contract, unchanged for every run that writes it;
    // theta < 0 is the degenerate gate (nothing well
    // separated, the far field empty) expressed as k < 0 so it works under the
    // surface test too, and the gate build stays the exact restricted
    // near-field direct build it always was.
    if (options.theta > 0.0)
    {
    ...
    // LMultForPreset(kTight) = 0) - the exact near-field-only rung. With an
    // absent theta and the surface test as the default, kTight would
    // otherwise silently acquire a live far field at L = 0, which is neither
    // that rung's contract nor an accuracy its budget was derived for. An
    // explicit theta still wins (theta > 0 selects the width test above).
    ...
    // The order selection: per far-field pair the minimal multipole order
    // whose a-priori
    ...
    // selection; every node stays -1 below.
    std::vector<int> farPairOrders;

    if (!farFieldPairs.empty())
    {
        const double epsInt =
            QfmmBudgetForPreset(options.accuracy) / static_cast<double>(farFieldPairs.size());

        farPairOrders =
            options.useAdaptiveOrder
                ? internal::SelectMultipoleOrders(*tree, farFieldPairs, epsInt, resolvedLMult)
    ...
    // The near-field domain: the leaf-driven carrier of the near-field
    // pair-pair set - for each
    ...
    // reference of the domain gate). Same candidate set under the same pair
    ...
    if (options.workspaceBudget != nullptr)
    {
        ...
        // systems - the chain's 24.44 GiB all-survive bound against a
        ...
        // (an asymmetry: the RI-J's nested exchange got that carrier, the
        // QFMM's did not).
        ...
        // exchangeEngaged flag zeroes it (the exchange fold-in).
    ...
    // The record states what RAN, not what was asked for: an explicit theta
    // resolves the width test, the negative theta and the k < 0 forms resolve
    // the gate, and the absent-theta default is the surface ball.
...
qcx::Result<qcx::memory::Tensor<double, 2, qcx::backend::CpuTag>> QfmmJBuilder::BuildFock(
    ...
    // passes, and the parallel list excludes it (the M2M is a small
