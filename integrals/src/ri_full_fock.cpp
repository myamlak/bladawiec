...
// rung in the ladder's ORDER (the owner's rule: direct screened first, then
...
// is free; the same two loops are the ones the composition's own test carries.
...
// The auxiliary-pair density screen's per-call structure: the
...
// the composition, and the identity anchored against the shipped eigen-path
...
// The screened half of the composition: one pass over the
...
struct RiFullFockBuilder::State {
    ...
    /// The auxiliary-pair density screen's Create-time stores:
    ...
    /// The batched contraction traversal's residency target
...
qcx::Result<RiFullFockBuilder> RiFullFockBuilder::Create(
    ...
    // be accepted and silently ignored. That is the silent substitution this
    // refusal forbids, and the honest surface is this refusal: a later change is
    ...
    // rather than composed, on the same rule as the two refusals above. The batched traversal walks
    // the whole auxiliary index (its partition is the aux FUNCTION index, not the surviving-cell
    // grid), so it cannot carry the screen's per-call surviving (pair, aux shell) structure: a
    // builder created with both would run the traversal and the screen's threshold would be
    // accepted and silently dropped. The refusal names both options and what each one costs to
    // keep, so a caller reads which knob to clear instead of guessing.
    ...
    // auxiliary shell range: it IS the contraction layout (rows u*n + v,
    ...
    // the orbital pair list, its Schwarz bounds, the aux shell bounds and the
    // aux shell FUNCTION ranges.
    ...
    // screen retains none of them (the path before the screen, byte for byte).
    ...
    // The screen's own class: the stores Create retains for it
    ...
    // The memory-ladder rung decision (the workspace-budget seam, decided
    // ONCE from the budget's remaining bytes -
...
qcx::Result<std::pair<Eigen::MatrixXd, Eigen::MatrixXd>> RiFullFockBuilder::BuildHalves(
    ...
    if (statsOut != nullptr)
    {
        ...
        // comment's rule): this builder calls no quartet kernel, so the
    ...
    // The contraction record of THIS call. Assembled from the
    ...
    {
        // The batched path: ONE traversal of the auxiliary
    ...
    } else
    {
        ...
        // the dense composition called them - the absence of the screen and of
