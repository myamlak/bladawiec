...
// the (n^2 x k_c) contraction-layout slice of (uv|P) for one contiguous range
// of aux shells, with the CHUNK-SCOPED screened task list (the full
// nPairs x nAuxShells list of the monolithic path is never materialized).
// The chunk runs through the EXISTING
...
qcx::Result<Eigen::MatrixXd> BuildRiTensorChunk(const qcx::molecule::Molecule& molecule,
    ...
    // a silent substitution, exactly as RiFullFockBuilder's own
    ...
    // The chunk-scoped screened task list: the Schwarz screen runs over the
    // chunk's aux shells only, with the chunk-scoped
    // exact reserve - the full nPairs x nAuxShells list never exists on
    // this path (the monolithic form carries the counted treatment too:
    // footprint.hpp taskListBytes charges the surviving-cell count, and
    // the chunked build must not inherit the monolithic reserve either way).
    ...
    // The counted exact reserve (the monolithic counted treatment mirrored on the
    ...
    // The chunk's (uv|P) values buffer (the 3c tensor term - the
