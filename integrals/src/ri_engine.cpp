...
// engine's first device integration made the same discovery: raw
...
// Create, and the dense (uv|P) tensor it materializes makes that budget a
// preset-level choice. With the kNormal budget (1e-10) no real basis
...
qcx::Result<std::vector<internal::RiTask>> BuildScreenedRiTaskList(
    ...
    // The counted treatment (the same counted pattern the exchange half's
    // pre-gate uses, applied to the task grid): the realized reserve is the
    // surviving-cell count, not the unconditional nOrbitalPairs x nAuxShells grid. The
    // footprint's taskListBytes charge (RiFootprint's screenedTaskCount) is
...
// the joint-grid expansion): the two-basis action plus the two Schwarz
...
// action on the CANONICAL SHELL PAIRS - and never `groupOrder > 1` (the
// mechanism's value, measured 2026-09-13). The two differ exactly where it
// costs: a non-C1 sign-only group (every atom on the symmetry element - a
// planar molecule's out-of-plane mirror, the measured hocl/Cs case) permutes
...
qcx::Result<void> ConsumeAssembledBatches(const internal::RiBatchAssembly& assembly,
                                          const Sink& sink) {
    // The light rung's per-iteration values buffer (the batch-capped per-call
    // buffer): the
...
// The METRIC occurrence's x/g3 accumulation (ri_engine.hpp
// RiTermCounters - the instrument's moments): the distinct phantom-pair
...
// The TENSOR-PASS occurrence's x/p3/g3 accumulation
// (ri_engine.hpp RiTermCounters - the instrument's moments; declared in
...
qcx::Result<qcx::memory::Tensor<double, 2, qcx::backend::CpuTag>> BuildAuxMetric(
    ...
    // The metric occurrence's term counts (the x/g3 terms of
...
qcx::Result<Eigen::MatrixXd> BuildBlockedAuxMetric(const qcx::molecule::Molecule& molecule,
    ...
    // The per-strip arena of the blocked-metric rung
    // (BlockedMetricStripBytes): the strip's values buffer is the term's
    // subject; the quartets and the assembly lists cross md_batch.cpp and stay plain. The scope
    // covers the whole strip loop (the buffer's capacity is retained at the largest strip).
    ...
    // The metric occurrence's term-counter stamp state (the x/g3 terms of
...
qcx::Result<qcx::memory::Tensor<double, 3, qcx::backend::CpuTag>> BuildRiTensor(
    ...
    // The joint-grid orbit expansion, when engaged: the screened grid is walked to
    ...
    if (RiOrbitExpansionEngaged(options))
    {
        ...
        // The note on the pathological all-screened list applies here
    ...
    // materialized tensor pass (the 3c tensor term - charged as
    ...
    // The tensor-pass occurrence's term counts (ri_engine.hpp
    ...
    //
    // The two indices read different lists - the orbital shell pairs and the
    // auxiliary shells - and the body names which is which at every use.
    //
    // NOLINTNEXTLINE(bugprone-easily-swappable-parameters)
    ...
    const auto supersededByLaterElement =
        [&orbitReduction](const internal::RiOrbitAction& action,
                          std::size_t repPair,
                          // repAux and g are the representative's
                          // auxiliary shell and the element index:
                          // the body pairs each with its own image
                          // array, so a swap is not spellable.
                          //
                          // NOLINTNEXTLINE(bugprone-easily-swappable-parameters)
                          std::size_t repAux,
                          std::size_t g,
...
struct RiJkFockBuilder::State {
    ...
    // (the unregularized solve was the 1e12
    // RiCoulombMatchesDirectCoulomb failure, 2026-08-21). The light rung keeps the
    ...
    // The light rung's per-pass term-counter partial (x/p3/g3): one count
...
qcx::Result<RiJkFockBuilder> RiJkFockBuilder::Create(
    ...
    // (the RI-J's own terms plus
    // the nested direct-exchange half's) against the budget's remaining
    ...
    if (options.workspaceBudget != nullptr || options.forceLightRung)
    {
        ...
        // The engaged orbit action's retained tables,
        // carried to the same two RiFootprint calls: the SAME formula
        ...
        if (budget != nullptr)
        {
            ...
            // BuildNeighborList cutoff: the all-survive bound
            // over-refused large-sparse systems (an 81,003-pair chain:
            // 24.44 GiB all-survive against a ~38 MB counted pattern), and
            ...
            // (decided 2026-09-17). The counted pattern is the
        ...
        // refused the stack (the same reading, one decision earlier). The light branch
        // prices with
        ...
        auto priceExchange = [&](std::size_t cap, std::size_t riBytes) -> ExchangeFootprint {
            ...
            // exchange term is not part of it - exchangeEngaged false),
            // the same term the light decision's own chunk sizing passes.
        ...
        auto attempt = [&](bool light, bool blockedMetricRung = false) -> Attempt {
            ...
            // live here, exchangeEngaged true.
            ...
            // proved decision-inert (a review pass found the
            // clamp's refusal region strictly contains the floor's, so
            // every floor refusal is clamp-shadowed) and was deleted with
            ...
            // post-reservation band has room for the k live per-slot bounds (the
            // authorization
            // is budget-conditional) and clamps ITS cap to its own
        ...
        if (budget != nullptr)
        {
            ...
            if (!modeInfo.has_value())
            {
                ...
                // rung takes over: the light rung with the metric built strip-wise
                // (BuildBlockedAuxMetric - no
                ...
                // solver's own working-matrix count is one on both), so its own estimate can
    ...
    // and a separate piece of work; engaging the flag on a rung that would
    ...
    // The run's Create-time term counts (ri_engine.hpp
    ...
    if (lightRung)
    {
        ...
        // The light rung's per-pass term-counter partial: one count over
    ...
    // systems - but that pre-gate is counted now, so it can no longer refuse a
    // stack the outer proved
    ...
    // The metric's floored eigen-inverse: M is SPD but extremely
...
qcx::Result<qcx::memory::Tensor<double, 2, qcx::backend::CpuTag>> RiJkFockBuilder::BuildFock(
    ...
    // so the gap is harmless in practice but documented: the certified-bound
    ...
    // The per-call term accumulations (ri_engine.hpp
...
qcx::Result<Eigen::VectorXd> RiJkFockBuilder::BuildRiCoulombVector(
    ...
    // per-iteration 3c recompute - a different contraction order,
    // not bit-identical to the materialized products, validated within the
    ...
    // identical to the path without it. The light rung never calls it.
    ...
    {
        // The light rung's per-iteration recompute slice:
        // the assembly's containers capture the
