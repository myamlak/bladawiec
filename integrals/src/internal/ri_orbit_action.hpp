...
/// \file
/// RI-J's JOINT TASK-GRID ORBIT ACTION. The 3c work RI-J does is a grid of
/// (braPair, auxShell) cells; the group acts on BOTH indices, so a cell's
/// orbit is joint and the walk can evaluate ONE representative per orbit and
/// expand the rest of the orbit from that representative's block.
///
/// THE PATTERN IS THE LANDED ONE, not a new one. The petite-list mechanism
/// landed exactly this shape for the 4-index walk (lean_orbit_action.hpp:
/// the per-index action tables, the walk-time
...
///
/// WHAT IT BUYS, AND WHAT IT DOES NOT. The ceiling measured 2026-09-13 on
/// the grid's own fixture (c8h18/def2-SVP + def2-universal-jfit, C2h,
/// order 4): the
...
/// difference the 4-index landing measured at ~1e-15/1e-16 per element. The
...
///
/// The two `LeanOrbitAction`s are the same object the 4-index landing built,
/// over the two bases' own pair lists. The orbital instance's `pairImage` is the
...
inline void RiExpandOrbitMemberBlock(const RiOrbitAction& action,
    ...
    // instead of once per block element. Measured on c8h18: the per-element
    // form made the expansion cost more than the kernel evaluation it
    // replaces.
    //
