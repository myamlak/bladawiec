    ...
    for (const qcx::integrals::ShellInfo& info : pairList.shells)
    {
        ...
        for (const std::vector<double>& row : shell.coefficients)
        {
            ...
            for (std::size_t primitive = 0; primitive < shell.exponents.size(); ++primitive)
            {
                ...
                // (2a/pi)^(3/4) (the standard radial convention) times the solid-harmonic
                // unit normalization of md_defs.hpp (the standard QC
                // normalization contract the basis-set coefficients assume).
                // Cartesian shells keep that factor only for now.
                const double solid =
...
// The per-task batch metrics shared by the eager AssembleClassBatches and the
...
qcx::Result<std::vector<MdClassBatch>> AssembleClassBatches(
    ...
    while (runStart < items.size())
    {
        ...
        for (std::size_t t = runStart; t < runEnd; ++t)
        {
            ...
            // allocations ride the cap too (the whole-run charge; TaskItemBytes
...
// The canonical-sorted masters and the incremental batch partition.
...
qcx::Result<void> RunBatches(const std::vector<MdClassBatch>& batches, std::size_t regionThreads) {
    ...
    // assignment. The batch vector is
