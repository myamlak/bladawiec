...
/// The canonical role form of one raw screened task: the canonicalization
/// of CanonicalizeQuartetOrder applied to
...
/// sequence (the batch-partition contract, 2026-09-04): the split
...
/// in the scaffold - the flag-gated executor (and
...
/// certified sums of the listed pairs so the next chunk's build starts
/// clean. The geometry fields, the prim pairs (their nested weights/transforms and the
/// per-axis E tables each carries) and every vector capacity survive - the
/// chunk arena is capacity-preserved by contract (the reused containers are
/// not Reservations, so per-chunk alloc/free cannot stack
/// under CWA retention), and the pair builder re-fits every slot of the
...
/// list: emits the class batches ONE at a time with AssembleClassBatches'
/// exact
...
public:
    ...
    /// numBatches, the batch-boundary metadata of the streaming-rung charge).
    /// Appended to per emission in
    /// both modes.
...
/// per-class chunk assignment). Every batch writes only its own disjoint
/// output block and its own global certified-bound slots, so the run is
/// bit-identical across team sizes by
...
/// the fix-A bounded-concurrency seam - each concurrent slot's assemble
/// region forks its slot's share of the team (q = max(1, team/k)), never the
/// full team, so the k-slot thread product stays at or under the team
