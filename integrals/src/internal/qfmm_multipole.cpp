...
// kQfmmMaxLMult, the
// density-weighted leaf aggregation, the M2M/M2L/L2L passes over the
// VERIFIED emitted translation tables (qfmm_tables_gen.hpp - generated and
// verified, never hand-derived), and the far-field J accumulation in the
// direct builder's 2J convention. The monopole
// gate is the L_mult = 0 degenerate case of the same seams: the (0, 0)
// moment block IS the overlap block through the trusted overlap engine,
// and the (0, 0) rows of the emitted tables are exactly {1.0, 0, ...}
// with kQfmmM2LDenomPower[0][0] = 1, so the M2M reduces to the exact 1.0
// coefficient, the M2L to Coulomb's law at the node centers, and the L2L
// to the identity - every pass degrades to the monopole code path
// bit-identically (the tests pin this). The moments themselves are the
// "add one angular momentum" trick: the raised (la + lMult, lb)
...
/// the binomial shift: the a, b, c degrees run to the multipole order).
...
/// the base of the Step-4 reduction (pinned by the tests).
...
/// growing the shared bound (the shared bound is load-bearing for the
/// other builders). Only the t = 0 slot is consumed downstream.
...
/// raised (la + lMult, lb) per-axis E tables (the shift trick), the
...
qcx::Result<QfmmMomentTable> BuildMomentTable(const ShellPairList& pairList,
    ...
    for (std::size_t p = 0; p < pairStore.size(); ++p)
    {
        ...
        if (pairOrders[p] == 0)
        {
            // The Step-4 path, unchanged: the (0, 0) moment block of an
...
std::vector<int> SelectMultipoleOrders(
    ...
    // The cube's circumscribed-sphere radius factor sqrt(3): the pair's
    // charge distribution lies inside its box's circumsphere, so r = sqrt(3)
    // * halfWidth is the valid source radius for the a-priori multipole
    // bound. The larger of the two radii bounds both M2L directions of the
    // unordered pair.
    for (const auto& [a, b] : farFieldPairs)
    {
        ...
        const double radius = std::sqrt(3.0) * std::max(nodeA.halfWidth, nodeB.halfWidth);
        const double ratio = radius / distance;

        // The minimal L with ratio^(L+1) <= epsInt, by repeated
        // multiplication (the distance is never zero - a well-separated far
        // pair satisfies distance >= (wA + wB) / theta > 0 - and the ratio
        // is below 1 at the preset thetas, so the product shrinks; at
        // ratio >= 1 no L qualifies and the cap wins, which is the safe
        // reading either way).
        double bound = ratio;
        int order = orderCap;

        for (int L = 0; L < orderCap; ++L)
        {
            if (bound <= epsInt)
            {
                order = L;
...
std::vector<double> AggregateLeafMoments(const ShellPairList& pairList,
    ...
    // The chunked pass: chunk over the NODE ranges - every leaf's moments are
    // accumulated by
    // exactly one chunk (single-writer per element), so the chunked
    // evaluation is bit-identical to the serial in any schedule.
...
/// ...}), the Step-4 symmetric Coulomb term. The distance is never zero: a
            ...
            for (int mt = -lt; mt <= lt; ++mt)
            {
                ...
                for (int ls = 0; ls <= order; ++ls)
                {
                    ...
                    for (int ms = -ls; ms <= ls; ++ms)
                    {
                        ...
                        // distance at power 1 - the Step-4 denominator).
...
/// The production M2L entry: the fold, no instrument (the 7-parameter form
/// the pass and the instruments call).
...
/// The chunked L2L: the root step first (its children's blocks must be final before any subtree
...
void AccumulateFarFieldJ(Eigen::MatrixXd& fock,
                         ...
                         const std::vector<int>& nodeOrders,
                         std::size_t chunkCount) {
    // The chunked pass: chunk over the NODE ranges. Every canonical pair
    // lives in exactly one
    // leaf and the distinct pairs' function-block index sets (both
    // orientations) are disjoint, so the fock writes are single-writer per
    // element - the chunked evaluation is bit-identical to the serial in
    // any schedule.
    auto accumulateRange = [&](std::size_t begin, std::size_t end) {
        for (std::size_t nodeIndex = begin; nodeIndex < end; ++nodeIndex)
        {
            ...
            for (const std::size_t pairIndex : node.pairIndices)
            {
                ...
                // so the Step-4 path is unchanged bit-for-bit. The caps
