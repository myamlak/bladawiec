...
/// exact byte formulas for the MD pair store (E tables, transforms, weights),
/// the neighbor pattern, the per-thread batch scratch and the structural
/// terms, evaluated from the shell data available at Create(). The formulas
/// mirror the engine's
...
/// a device-budget feature decision of the GPU path (the scaling-
/// measurement fix; the GPU family's admission accounting stays the driver
...
/// n^2 working-set term, reconciled separately). The term
...
/// (the pre-chunking
/// sweep built the WHOLE pair store, 9.95 GiB at C42H86/def2-QZVP -
...
/// (the exclusion fix): the mode selection's exclusion (i) PRE-GATE -
...
/// The pair-class table's Create-time structural bytes (the pair-class
/// root fix, 2026-08-31): the class path materializes the FULL canonical
...
/// ceiling of the actual allocation, and the sure-fit admission band is
/// unchanged.
...
/// cumulative high-water accounting sums sequential commits): the batch shrinks by
...
/// coefficient when the caller knows it - the concurrent batch loop's
/// exchange-engaged fast path passes threadCount + slotCount x (2|3), so
/// the shrink removes the
...
/// exchange term's whole-call envelope. ExchangePerCallBytes no longer
/// charges it - the per-batch restructure
/// turned the exchange into per-batch assemble-and-contract for
...
/// charged by ExchangePerCallBytes (per-batch) - retained as
/// the GPU lane's per-call quartet floor (device_footprint.hpp) and as
/// the whole-run ScreenedQuartetFraction law's anchor at n = 586.
...
/// whole-run screened-quartet charge): the screened-quartet count
/// grows like a fixed fraction of n^4 at small n and ~ n^2 at the
/// 5000-scale (the measured 10-30M-quartet plateau)
/// - the piecewise log-log law through the measured anchors:
...
/// measured ~n^2 survival anchor. Flat 2.5e-3 below 298 (the screening
...
/// streaming rung's honest charge, replacing the earlier whole-call form):
/// the fp64 task master's MdQuartetTask entry. The earlier whole-call
/// rows of the old 224 B/q control mass (the per-half
...
/// input must be resident - the impossibility argument). The
...
/// The whole-run screened-quartet task-machinery bytes (the whole-run
/// charge, at the streaming rung's survivor): ScreenedQuartetFraction(n) x n^4 x
...
/// The exchange per-slot per-call live-mass bound (the
/// values-term redesign): the structural per-call peak terms replacing the
...
/// floor and the whole-run anchor). The per-batch restructure
/// turned the exchange
...
/// fp64 - F64Bound only, kTight's boundary), kLoose/kNormal route
...
/// cumulative high-water accounting sums them). Never-under by construction:
/// every assembled batch or
...
/// lone over-cap quartet batch (itemBytes > maxBatchBytes, the
/// review amendment's exception) stays inside the terms' slack at every
/// basis an admission can reach. The term is a Reservation charged once
/// at Create through Total() (the per-call buffers are
/// fresh allocations, the high-water accounting note, not per-thread scratch), and scales
...
/// the cumulative high-water accounting covers k slots x one live batch each, never 1 x k batches -
...
/// batchStarts tables of the concurrent batch loop (fock_build.cpp
...
struct DirectFootprintTerms {
    ...
    std::size_t exchangePerCallBytes = 0; ///< The nested exchange's per-call live-mass bound
                                          ...
                                          ///< the values-term redesign, k-folded by the slot
                                          ///< count), plus the
                                          ///< k > 1 rung's batch-boundary metadata at slotCount
                                          ///< > 1 (BatchStartsTablesBytes - the per-half
                                          ///< batchStarts tables of the concurrent dry walk, shared
...
/// term would be phantom and must not be charged.
...
/// \param slotCount The authorized concurrent batch slots: the
...
/// free-standing runs and, since the slot lift, a
...
/// the scaling fix). The metric's filled pairs are the same store
...
/// The RI-J builder's FIRST Fock iteration's tensor-class terms (the
/// first-iteration finding): the per-call allocations the Create-time
...
struct RiFootprintTerms {
    ...
    std::size_t metricBytes = 0; ///< The metric store (the shared-phantom pair store) and the
                                 ...
                                 ///< State eigenvector copy); the blocked-metric rung's metric
                                 ///< build carries two (its input dies before the State copy)
...
/// store and the aux ket store, the screened task-list charge (the
/// counted treatment - screenedTaskCount x 16 B, the surviving
...
/// measured reconciliation of those internals, 2026-09-03), the 3c batch arena, and
...
/// retainedListBytes - a reconciliation item now closed).
...
/// otherwise.
/// \param screenedTaskCount The screened RI task-grid count: the
/// surviving (orbital pair, aux shell) cells of the
...
inline RiFootprintTerms RiFootprint(const qcx::molecule::Molecule& molecule,
    ...
    // on both rungs (the 2026-09-03 reconciliation): the
    ...
    // DirectFootprint exchangePerCallBytes term's subject - the earlier
    // premise that the driver base's n^2 term covered them was refuted at
    // C24H50, measured ~12.6 GiB/call before the per-batch restructure;
    // the term now charges the per-batch bound, F64Bound +
    // F32Live of the values-term redesign; the driver base's n^2 term stays
    // the driver-side working set, reconciled separately.
    ...
    // rungs unconditionally is a defensible worst case under the cumulative
    // high-water accounting's heap-retention note.
