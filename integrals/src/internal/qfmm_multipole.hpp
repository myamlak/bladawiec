...
// The multipole machinery of the QFMM skeleton: the moment table, the
// per-iteration
// density-weighted leaf aggregation, the M2M/M2L/L2L passes at general
// L_mult (0..kQfmmMaxLMult), and the far-field J accumulation. The monopole
// gate is the L_mult = 0 degenerate case of the same seams: the
...
// and the L2L to the identity), and every pass degrades to the Step-4 code
// path bit-identically. The acceptance properties: with theta -> 0
...
/// The adaptive per-interaction multipole order: for each far-field pair
/// (A, B) the minimal order
/// L whose a-priori geometric bound (r / d)^(L+1) <= epsInt, with r = the
/// larger source-box radius sqrt(3) * max(wA, wB) (the max bounds BOTH M2L
/// directions of the unordered pair - the r_A reading is the safe
/// one-sided special case), d = the center distance, and epsInt the
/// per-interaction error budget (the preset's QfmmBudgetForPreset divided
/// by the far-pair count). L is capped at orderCap - the resolved
/// multipole order of the build (min(kQfmmMaxLMult, LMultForPreset) on the
/// default path) - so the preset ladder stays the accuracy contract and
/// the selector only LOWERS an interaction's order where the bound proves
/// it safe; interactions whose bound needs more than the cap keep the full
/// order. Geometry-only and deterministic (same tree and pair list, same
/// selection), so the result joins the far-field pair list as the per-pair
/// order vector.
/// \param tree The octree result (the node half-widths and centers).
...
/// The per-node needed order: node n's needed order is the maximum over the
/// far pairs touching n
...
/// monomials exact 0), so a leaf's moment 0 is the Step-4 charge sum in
...
/// Step-4 child-charge sum in the old expression/order.
...
/// QfmmM2LCoefficientStats). The pair's truncation is ITS order (per-pair orders - the
/// contract): both the source moments and the target writes stay within
...
/// applies (the identity at order 0 - the Step-4 single 2 · q · pot term,
