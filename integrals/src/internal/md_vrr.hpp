...
void RunVrrQuadruple(const MdPrimPair& braPrim,
    ...
    // and bounds-checked in MSVC Debug builds (2026-08-21 - the
...
qcx::Result<void> ComputeEriClassImpl(const MdClassBatch& batch) {
    ...
    // call). Batching across tasks is a Step-2 tuning item.
    //
...
/// The certified fp32 class kernel (the certified dispatch-table entry).
