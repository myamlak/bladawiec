...
/// \file
/// Create-time device-footprint estimation for the GPU device workspace
/// budget: the byte terms EriCudaEngine::Create fills the public
...
/// re-pinned n^-1 law floored at footprint.hpp's fitted fractions) and the
...
/// zero until the RI path's device residency lands (design section 3: the
/// RI tensors become device-resident; their bytes charge this slot).
...
/// size (measured on the T1000 via the 64 KiB probe). Calibrated on the
/// Quadro T1000 (2026-08-30): every measured
...
/// (the measured create can under-run the estimate - errs safe). The gate
/// evidence is in the D-record.
...
/// step-1 gate (2026-08-30): the flat fractions (0.0088 / 1.42e-4)
/// under-charge the C12H26/STO-3G measurement 7.7x (the measured per-call
/// growth was 30.0 MiB vs the flat-fit 4.2 MB - the same C12 refutation the
/// CPU seam records). The C12 anchor is
...
/// the flat fractions, so the estimate never charges less than the old
...
/// n^-1 law of kGpuSurvivalAmp / kGpuQuartetAmp (the step-1 gate
/// re-pin; see the constants above), floored at the flat fractions of
/// footprint.hpp.
...
/// Fills the engine's Create-time device footprint record (design section
/// 5): the caller passes the exact uploaded-table bytes (the hostTables
