...
// The GEMM dispatch of the MD transforms: the
// internal register-tiled micro kernel for tiny shapes, the linalg batched
// seam for the fp64 lane, Eigen products for the certified fp32 lane
// (the seam's float path was added for that lane). The ket transform batches
...
//
// NOTE (an open optimization): the transform GEMM dimensions here are the
