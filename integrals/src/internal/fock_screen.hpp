...
// evaluate EXACTLY the same screening decisions on the same candidate lists
// (the GPU-vs-CPU parity gate demands bit-identical lists). Header-only:
// every function here is inline, and both consumers are MSVC C++23 host TUs
// (this header is never included from a .cu TU - the device code gets its
// own CUDA-safe copy of the decisions it needs, with the same constants).
...
// (ScreenOne), and the per-element thresholds divide by the raw value - the
// niedoida current_threshold shape (standard_k_matrix_generator.cpp).
...
// less, keep at equality - the legacy gate's boundary convention
...
// The cached neighbor-list cutoff slack: the list is
// built with threshold x this factor, i.e. more permissive than the live
// check was - see the construction comment in BuildNeighborList for the
// rationale.
...
// the team (DefaultOmpTeamSize - the team policy; the thread_cap ceiling
// clamps it, so a cap of 1 yields numChunks 1 - the serial path; the auto
// split is team-sized,
// not hardware_concurrency-sized, so the fp64-half/fp32-full split is
// clamped before it happens); FockBuildOptions::maxParallelChunks (0 =
// auto) lets the schedule-independence test force the serial fallback (1)
// or a fixed split (>= 2) on any machine.
...
// The ParallelReduce element of the screening pass: the
...
struct ScreeningContext {
    ...
    // |D_block| per canonical pair, BuildShellPairMaxDensity), or nullptr
    // when the per-element screening flag is off. nullptr/empty selects the
    // legacy six-block-max gate in ScreenOne and disables the per-element
    // re-filter - exactly today's behavior (the flag contract).
    // The vector's lifetime is the BuildFock call that built it.
    const std::vector<double>* pairMaxDensity = nullptr;
};

// The screening decision for one neighbor-list candidate (bra,
// ket): the density weight (max |D| over the six blocks the J/K
// contractions touch - shared by the screening gate and the routing
// gate), the density-screening gate, and the certified-mixed-precision
// routing. Pushes into the caller's lists - every chunk passes its OWN
// local lists, never shared ones (the shared-vector race the parallel
// dispatch exists to avoid). The routing gate sits OUTSIDE the screening
// block - the certified lane must route (and its bound must be delivered)
// whether or not density screening is enabled (2026-08-22: it used to be
// nested, so useDensityScreening=false silently disabled the lane).
inline void ScreenOne(std::size_t bra,
                      ...
                      // before the routing gate consumes the mixed bound.
    ...
    // The QFMM near-field restriction: an engaged restrictToPairPairs limits
    // the build to the caller's pair-pair subset - the octree near-field list - before any
    // screening work (the unrestricted path costs one Empty() test). The
    // caller sets both orientations of every near-field leaf pair, so the
    // canonical (bra, ket) iteration of the Schwarz neighbor list matches
    // either way.
    ...
    // lookups - the entire screening-pass cost win) or, on the legacy path
    // (pairMaxDensity null), from the six block scans exactly as before. The
    // two paths deliver identical values: dMax is the max of the same six
    // blocks either way (the routing gate below must keep receiving the
    // six-block max). The cross-pair lookups are canonicalized before
    // PairIndexOf.
...
// BuildNeighborList collected), chunked across threads: a static bra-row
// range per chunk - the preferred natural
// boundary. Row lengths vary (compact molecules: s-shell pairs hold most
// kets, high-l pairs few), but on the benchmark molecules the variance is
// moderate and the per-chunk row range still spreads the work evenly; a
// flattened-position split buys nothing here (it is reserved for wildly
// uneven rows). Each chunk builds its OWN local task lists via
// ScreenOne - the shared vectors would be exactly the shared-_fock race -
...
// of the raw CSR vectors. The pattern is host-canonical (the
...
// The Schwarz-exact neighbor-pattern count (the counted exclusion): the
...
// The budget decision's exclusion (i) tests the EXACT pattern bytes
// (8 x this count) against the remaining bytes instead of the all-survive
// upper bound 8 x nPairs(nPairs + 1) / 2: the all-survive form over-refused
// large-sparse systems at ~nPairs^2/2 above their real pattern (the
// C80H162/STO-3G misfire: the 26,246,268,048 B bound vs a ~38 MB counted
...
// take when no sweep has run, in exactly the shape the counted exclusion
// gave the pattern term: the all-survive form survives only as the cheap
...
// The RI-J task-grid survivor count: the number of
...
// kNeighborListSlack: the 3c screen is preset-level, where the
// neighbor-list cutoff carries the slack for the per-iteration density
// screen). The footprint's taskListBytes charge (RiFootprint's
...
// by equality, the strongest never-under form (the unconditional
// dense grid was the pre-screening reservation, an over-charge at any
// sparse scale). Never under-bounds: on an all-survive grid the count
...
// The cached Schwarz neighbor list construction: the ket
// candidates of every bra pair under the pair-cutoff test, precomputed
// once per builder Create(). The live per-quartet Schwarz check this
// replaces is static across SCF iterations (Q_ab never changes), so this
// is an exact precomputation - the same decision, evaluated once per
// Create() instead of once per BuildFock call. The construction cutoff
// carries a multiplicative slack (x0.01, niedoida's
// standard_j_matrix_generator.cpp pattern): the CACHED list is slightly
// more permissive than the live check was, so a borderline pair near the
// cutoff can never be permanently excluded by list construction even if a
// later iteration's density weight would have made it relevant. Costs a
// few extra entries (constant factor, not asymptotic); the per-iteration
// density screen still runs inside the loop and decides what is actually
// contracted.
...
// The leaf-driven form of the cached neighbor-list construction: the same
// candidate rows as
...
// The count-only form of the leaf-driven construction above (the counted
// exclusion): the exact number of (bra, ket) rows the leaf-driven
...
// The windowed form of the neighbor-list construction (the LightPath chunk
// pass): builds the ket candidates
// of the bra rows [rowStart, rowEnd) over the FULL ket range (ket <= bra,
...
// the "peak = max chunk, never Σ chunks" rule, with the buffers never
