...
/// \file
/// The LightPath rung's Create-time footprint arithmetic, shared by the
/// builders that
...
// MdPairData the light store already carries) - the "mean per-class pair
// cost" taken as the max, so the chunk sizing is conservative
...
// The LightPath chunk auto-sizing (a closed form): the largest
...
// marker pass is O(pattern) once with a stamp vector - the "peak = max
// chunk, never the sum" rule: the tail chunks' kets are the tail rows'
...
struct LightFootprintTerms {
    ...
    std::size_t screenedQuartetBytes = 0; ///< The whole-run screened-quartet task machinery
                                          ...
                                          ///< whole-call survivor, kScreenedTaskBytesPerQuartet
...
inline LightFootprintTerms LightFootprint(const qcx::molecule::Molecule& molecule,
    ...
    // model and is not consulted - exchangeEngaged false (the fold-in's
    // explicit threading).
    ...
    // law's input is the function count, available a priori; the consult's
    // verification ask): the light rung runs its chunks off the sorted
