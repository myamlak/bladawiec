...
/// by their extents - the padded-box computation applied at EVERY
/// level, so each node's box contains the padded extent of every pair in
/// it and the θ test's box geometry alone is honest (the premise the
/// separation test rests on). A raw octant would violate it: a pair's cloud straddles its
...
/// The centre-to-centre distance of a node pair: what both separation forms
/// measure and what the error arm's bound divides by. One copy, because the
/// two forms and the arm are branches of one predicate.
inline double CenterDistance(const QfmmTreeNode& a, const QfmmTreeNode& b) noexcept {
...
// The distance and the order are the pair a caller could transpose: the one
// call site passes both from locals named for their role, in this order, and
// the body reads each in a way the other cannot take - the distance is the
// guard and the ratio's denominator, the order is the loop bound.
//
// NOLINTNEXTLINE(bugprone-easily-swappable-parameters)
...
// The distance and the budget are the pair a caller could transpose: both call
// sites pass them from locals named for their role, in this order, and the body
// reads each in a way the other cannot take - the distance divides the radius,
// the budget is the threshold every rung is tested against.
//
// NOLINTNEXTLINE(bugprone-easily-swappable-parameters)
int MinimalMultipoleOrder(double radius, double distance, double epsInt, int orderCap) noexcept {
    ...
    // Repeated multiplication rather than pow: the bound is a product of the
    // same ratio, and the loop is exactly the order ladder the caller reads.
    const double ratio = (distance > 0.0) ? (radius / distance) : 1.0;
    double bound = ratio;

    for (int degree = 0; degree < orderCap; ++degree)
    {
        if (bound <= epsInt)
        {
            return degree;
    ...
    // The loop carries degree's own bound out of the last iteration, so the
    // cap is tested like every rung below it rather than assumed.
    return (bound <= epsInt) ? orderCap : -1;
}
...
bool IsWellSeparated(const QfmmTreeNode& a,
    ...
    {
        if (!IsWellSeparated(a, b, separation.theta))
        {
            return false;
        }
    } else
    {
        // The surface-to-surface form: the distance must clear both charge
        // radii plus a buffer proportional to the larger one (the multipole
        // error bound's demand). k < 0 is the degenerate gate - nothing is
        // well separated, the theta -> 0 analogue.
        if (separation.k < 0.0)
        {
            return false;
        }

        if (CenterDistance(a, b) <=
            a.radius + b.radius + separation.k * std::max(a.radius, b.radius))
        {
            return false;
        }
    }

    // The error arm: the geometry says the pair is apart, the bound says
    // whether the apart-ness is enough. Both radii are charge radii of the
    // distributions the expansion is OF, so the larger one is the source radius
    // of the unordered pair, the same quantity the order selector reads. A pair
    // no allowed degree can represent within the budget is not well separated -
    // the traversal descends it into the near field, where it is exact, rather
    // than letting the far field carry an interaction its budget does not
    // cover. The arm sits AFT of whichever geometric form classified the pair,
    // so it means the same thing under both: the form says apart, the budget
    // says representable.
    if (separation.orderCap < 0)
    {
        return true;
    }

    return MinimalMultipoleOrder(std::max(a.radius, b.radius),
                                 CenterDistance(a, b),
                                 separation.epsInt,
                                 separation.orderCap) >= 0;
}
