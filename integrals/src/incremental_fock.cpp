...
qcx::Result<qcx::memory::Tensor<double, 2, qcx::backend::CpuTag>> IncrementalFockBuilder::BuildFock(
    ...
    // linearity identity - BuildFock of a summed density, less its one H,
    // equals the sum of the two individual BuildFocks each less its own H -
    // requires H to appear in the accumulator exactly once, so the extra H
    // of every incremental contract must be stripped.
    if (mustRebuild)
    {
