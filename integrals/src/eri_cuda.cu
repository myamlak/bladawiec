...
/// per that layout - the bra contraction weights live in the full
        ...
        for (int chunkStart = 0; chunkStart < totalSlots; chunkStart += args.chunkRows)
        {
            ...
            if (tid < slotsInChunk)
            {
                ...
                for (int g = 0; g < ket.primCount; ++g)
                {
                    ...
                    // overwrite dropped 8 of 9 STO-3G pairs from the a-priori
                    // bound - fixed on the CPU 2026-08-22, mirrored
...
/// Launches the V2 VRR kernel for one (run, g). Defense-in-depth (int-5):
...
int EriCudaLaunchBoundClass(EriCudaClassArgs& args) {
    ...
    // The same int-5 defense as the VRR launcher: an empty run must not
