...
/// \file
/// The lean direct Fock builder (the 2026-09-08 lean-direct reframe):
/// a Schwarz-only recompute loop over ONE immutable 24-byte
...
struct LeanFockBuildOptions {
    ...
    ///
    /// The ceiling also CHOOSES the row-window plan, because the window
    /// count is the plan's memory knob: Create searches the window count
    /// down from the plan the options name - the host's automatic plan
    /// (maxParallelChunks above) unless that is pinned - and
    /// runs the WIDEST plan whose estimate clears the ceiling, so a build
    /// that cannot afford the machine's own decomposition narrows its
    /// decomposition instead of refusing, and refuses only when even the
    /// one-window plan does not clear (lean_fock_build.cpp
    /// ChooseCeilingPlan). The plan is thereby a property of the budget
    /// rather than of the machine's core count: two hosts differ in where
    /// the search starts, not in whether a fit exists, so a build no plan
    /// fits is refused on both.
    ...
    /// A positive memoryCapGiB searches DOWN from this plan, so the
    /// override is where the ceiling's search starts rather than a floor:
    /// the budget still decides the plan a call runs.
...
struct LeanEnvelope {
    ...
    /// reports, computed for the row-window plan the call will actually
    /// run - the plan the ceiling selects when one binds, so this is the
    /// requirement of that plan. Always computed.
    ...
    /// the builder's Create-time check can never drift apart. It names two
    /// requirements: the selected plan's (required_peak_bytes) and the
    /// host's own plan's (host plan), equal whenever no ceiling narrowed
    /// the plan.
    std::string refusal;
};
...
/// read one formula, and the plan the ceiling selects is part of that one
/// formula - so a consult made before the ramp admits or refuses exactly
/// what the builder's own Create would.
...
public:
    ...
    /// which reports required_peak_bytes, the host plan's own requirement,
    /// available_bytes and the largest contributor).
    ...
    /// required_peak_bytes reports, and the same plan's: when a ceiling
    /// selected a narrower row-window plan than the host's automatic one,
    /// this is that plan's envelope, which is the plan BuildFock runs.
    /// Exposed for the driver's memory audit,
