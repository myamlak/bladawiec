...
/// builder's screened set recurs ~98% across SCF iterations, and the cache
/// turns that recomputation into memcpy traffic.
///
...
/// so both lanes are cacheable across iterations;
...
/// Cumulative counters and live footprint of one EriBatchCache: hit/miss
/// per lane over the cache's whole lifetime, the live payload bytes of the
/// two arenas, and the payload budget at Create.
...
/// never names it.
///
/// This is the shape the module boundary requires rather than a preference: the
/// disk tier is storage's CachedEriBatchEngine and storage is downstream of
/// integrals in the DAG (core -> ... -> integrals -> storage), so the builder
