...
/// (ri_full_fock.hpp RiFullFockBuilder) runs the first three as its
/// per-iteration chain; the batched build (BuildBatchedRiContractions, the
/// batch option's entry point) is the cache-tuned re-expression of that
/// chain, and the builder's DENSE per-call path runs it in place of the chain
...
/// The composition both halves are built from is
...
///
/// Two exact algebraic identities are what make this transform the object the
/// occ-RI-K path rests on. Both carry zero approximation error, so
/// between them they separate a transform defect from the auxiliary fit error
/// (the discrimination a missed accuracy target needs):
...
///   round-off (the shipped path's floored eigen-inverse, ri_engine.cpp);
...
/// not measured here: measuring the aux K-fit error per fixture it touches is
...
///   residency target - the cache-tuned part of the build;
...
/// traversal of the auxiliary index (the batched transform+K
/// build).
/// \ingroup qcx-integrals
...
/// transform+K chain, and the entry point that adds it.
///
...
///
/// **MEASURED 2026-09-16, single-run and contended (eight lanes building, CPU
...
/// speed question the acceptance asks is still OPEN here: it is a
...
///
/// The term counters are untouched: this entry point evaluates no
...
/// index is transformed - the exchange contraction needs no more, and that is
/// what halves the per-iteration flops of the
/// AO-basis form.
///
...
/// screening and no batching, both of which the composed builder's other
/// paths own. The contraction is one n x nOcc by nOcc x n update per auxiliary
/// function.
