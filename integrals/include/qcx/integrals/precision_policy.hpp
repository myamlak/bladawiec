...
/// method.precision cap. Pure policy - no device code, no Eigen: the Fock
/// builders feed it the a-priori bounds of their screened batches and
/// band-tag them, so the hot kernels stay branch-free (precision is a
/// dispatch dimension BETWEEN class runs, never a branch inside a kernel).
///
...
/// cheap on the half-ratio datacenter class) → the fp64 reference lane.
...
/// The certified lane's epsilon (the kCertifiedEpsilon of md_vrr.hpp,
/// restated for the public policy): the fp32 band's a-priori bound is
/// eps * cClass * (1 + rounding) * Q_bra * Q_ket * |D|.
inline constexpr double kCertifiedBandEpsilon = 1e-7;

/// The fp16 band's certified epsilon (the ladder's own extension beyond
/// the literature): the fp16 lane stores the Boys values at half-ULP of
...
/// lane's device default flips (the device probe's verdict, owner's ruling
/// 2026-09-12): at or below it fp32's throughput premium does not cover
...
/// (owner's ruling, 2026-09-12: fp32 "may be far more useful on GPUs and
/// AVX machines", so the lane's default is HARDWARE-AWARE rather than a
...
/// The per-build budget arithmetic, the formula verbatim: all terms in Eh,
/// B(preset) the preset's J/K budget,
...
/// Prices the band boundaries from the measured device ratio: the mixed
/// band's domain scales with how cheap fp64 is - on the half-ratio
/// datacenter class the certified comfort zone shrinks toward the mixed
/// band's ceiling, on a 1/32 consumer device the certified lane keeps the
/// wide comfort zone.
/// \param ratio The measured fp32/fp64 throughput ratio (1.0 = unknown:
/// no pricing).
