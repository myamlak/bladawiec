...
/// machinery ([McMurchie1978]) at runtime
/// angular momentum (no per-class instantiations): overlap and kinetic as
...
/// rejection of non-s shells is replaced by the l > 6 parser-cap rejection
/// - p/d/... shells are evaluated positively.
...
/// from; defaults to the coordinate origin. Origin convention (matching
/// niedoida's multipole moments): all moments are relative to the input
/// coordinate origin AS GIVEN - the caller is responsible for recentering
/// the molecule first if a center-of-mass or center-of-charge dipole is
/// wanted.
/// \param molecule Molecule providing the atom coordinates (Bohr).
/// \param basisSet Basis set; every shell must satisfy l <= 6.
...
/// and lk integrals coincide, so only the upper triangle is built.
/// \param origin The reference point R (Bohr) the operator is measured
