...
/// molecule (petite list): the data the class-aware Fock
...
struct PairClass {
    ...
    /// class_table allocation family's tagged allocator (the tag is the scope
    /// active when the entry is built - BuildPairClasses' kClassTable scope).
...
/// of the rep block (sign-mixed shells make the member block a
...
struct ClassOrbitMember {
    ...
    /// identical expanded block (the consistency verdict), so any
    /// valid element works; the representative's own member carries the
    /// identity element 0.
...
struct ClassOrbit {
    ...
    /// are closed under the group action. The class_table allocation family's
...
struct ClassPair {
    ...
    ///
    /// The orbit expansions are generated ON DEMAND (the 2026-08-31
    /// restructuring): BuildPairClasses stores the pair classes and the
    ...
    /// (the c60 0xC0000409 death class). The class_table allocation family's
...
struct PairClassTable {
    // The class_table allocation family (the DirectFootprint classTableBytes
...
/// pair and one orbit entry per canonical member quartet - and the
/// 2026-08-31 restructuring made it a gross over-charge: BuildPairClasses
