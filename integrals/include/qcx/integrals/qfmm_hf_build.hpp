...
/// to QFMM - the Coulomb half through the QfmmJBuilder (near field + multipole
/// far field, H + 2J) and the exchange half through a DirectJkFockBuilder in
/// its exchange-only mode (H - K), composed as H + 2J_QFMM(rho) - K(rho) with
...
/// store, and this exchange store) are the triple-store footprint the
/// adaptive design already counts. With a
...
/// budgets {1e-5, 1e-7, 1e-8} Eh, with the kTight rung re-derived
/// from 1e-9 by the 2026-09-13 ruling) and the split-pass summation
