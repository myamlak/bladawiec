...
/// Everything here is measured against the accuracy targets: absolute error
/// 5e-14 in double precision and 1e-7 in single
/// precision for the Boys kernel, which is the accuracy bottleneck of all
/// subsequent integral recursions.

// boys.hpp forwarding shim:
// the Boys-function kernel is built from the boys submodule's
// sources (external/boys, pinned gitlink - v1.0.0). This header forwards
// the submodule's public API into
// namespace qcx::integrals so every include site compiles
