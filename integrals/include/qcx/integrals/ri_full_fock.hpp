...
/// \file
/// The composed full-RI Fock builder: it evaluates BOTH halves of the
/// Kohn-Sham matrix through the auxiliary basis,
...
/// 3-center tensor at Create rather than joining it - and no second tensor
...
///   sum_P B^P rho B^P exactly for any C_occ (an identity verified at
...
///   `ri_jk` request whose auxiliary basis in effect is not a JK-optimized fit
///   is refused, and the predicate is fixed
///   over the RESOLVED AUX NAME (`aux_basis.hpp` IsJkOptimizedAux). A BasisSet carries
...
///
/// Scope ("runnable, dense, unscreened" was the landing; the auxiliary-pair
/// density screen - RiAuxScreenOptions - was added later and is off at its
...
/// and for its own reason, which is an UNMET acceptance: the target was
...
/// it (the silent-substitution class). The MEMORY LADDER is engaged: the
/// 3-center tensor is always materialized at
...
/// contraction ("the density factors (max_d over the block) are
/// per-iteration quantities, vacuous here: BuildRiTensor is basis-only,
/// called once at Create", ri_engine.cpp's screening note). This screen is the
...
/// The default is 0 because the VALUE is a required bound the screen must
/// respect - the screening-induced converged-energy move must stay at or below
/// a tenth of the measured auxiliary-fit error on the same fixture - and no
/// number is settled; the preset's own `DensityThreshold` is the
...
/// on both batched-traversal fixtures), and a narrower width regroups the
/// Coulomb
...
/// reason is the acceptance for this treatment, which is UNMET and unclaimable.
/// The target was >= 20% faster than exact screened exchange at 600 BF, on a
...
/// contraction, which the batching does not touch - the target is not
...
/// as the evidence that the wiring is reached (the rule: a run record
...
public:
    ...
    /// working set. That is the composition's own arithmetic - B replaces the
    ...
    /// why the reduction is refused here rather than accepted: a later change
    /// is aimed by these numbers, not by an estimate.
    ///
