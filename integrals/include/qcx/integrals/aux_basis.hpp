...
/// one from the orbital basis name instead of requiring
/// the user to name an aux set explicitly.
///
/// The rule is QUALITY-TIERED, not provenance-based (ruled 2026-09-13). See
/// SelectAuxBasis for the tiers and AuxSelectionNotice for the weak regions
/// that warn but still run.
...
/// Which Fock builder the aux basis will feed: the aux
/// family choice depends on whether exchange also comes from RI. The
...
/// worse error class, which is why kRiJk resolves to a JK fit
/// and never to a J fit - see IsJkOptimizedAux, the predicate that refuses
/// the pairing if it ever arrives by another road.
...
/// standard ends_with because the style guard carries starts_with in
/// its library-token allowlist and not ends_with, so the direct call is a
/// pre-commit failure (the same reason IsJkOptimizedAux spells its own test
/// this way rather than working around it in the guard, whose file this
/// module does not own).
...
///
/// **The rule is a quality tier, not a provenance test** (ruled
/// 2026-09-13). The previous rule refused every orbital basis outside the
/// `def2-*` and `cc-*` families, on the stated ground that pairing, say,
/// STO-3G with a def2 fit "would run but produce a numerically nonsensical RI
...
/// default (the owner's standing ruling: "if for some calculations we have no
/// default aux basis, set one as default").
///
/// **CORRECTION, 2026-09-13 (measured).** The refuted claim is corrected here rather
/// than deleted, because the paragraph was wrong in its conclusion and partly
/// right in its observation. Measured, H2O at one geometry, `ri_j_link`
/// against the exact `direct` path, `def2-universal-jfit` held fixed:
...
///   and the ruling names RI-K/JK and diffuse bases as the regions most
///   likely to REVERSE this conclusion. The rule below is therefore a
///   default with warnings, explicitly not a certification.
///
...
/// the NON-diffuse universal J-fit - the second half of the ruling's own
/// named hazard ("the real risk is ... a non-diffuse aux for a diffuse orbital
/// basis"). It is a \c -rifit, so it is J-only by construction and can never
/// reach a JK request.
/// \ingroup qcx-integrals
...
/// SMALLER than the orbital basis is the one direction the ruling's
...
inline std::optional<std::string_view> DunningJkfitRung(std::string_view orbitalBasisName) {
    ...
    // an underscore and so trips the snake_case style guard's library-token
    // scan (the same constraint AuxNameEndsWith documents above).
...
///     the ruling names as the real risk. AuxSelectionNotice reports the
///     fall-through.
...
///     universal J-fit - the ruling's own named hazard, and the reason
...
/// meet), which is where the ruling reserves the hard refusal.
/// \ingroup qcx-integrals
inline qcx::Result<std::string> SelectAuxBasis(std::string_view orbitalBasisName,
    ...
    // both kinds was the old rule, and it is the one pairing the ruling
    // calls the real risk.
...
///
/// **Warn, do not refuse** (the ruling): every region below RUNS, and the
/// boundary is the error class rather than the provenance of a name. The hard
/// refusal is reserved for a singular fit matrix or a tolerance even the
/// largest vendored aux cannot meet - neither is decidable from a name, so
/// neither lives here.
///
/// Stated where it bites: a notice the run record cannot show
/// is not a warning, it is the silence this contract exists to end. This
/// function RETURNS the text; the resolution point that owns the record is
/// responsible for writing it there (NOT stderr-only). The record's
...
/// one of the two texts (the member's own note, `result_json.hpp`, the block
/// described as a sibling of `riTensorMode`/`riChunkBytes`; read 2026-09-13,
/// when that file was in flight, so the member name is the citation and the
/// line number is not). The repo's own answer to
/// a second writer, taken twice already (`riTensorMode`, `riChunkBytes`), is a
/// SIBLING block on `resources_resolved` - and that block is what this notice
/// is shaped to feed.
///
/// **CORRECTION, 2026-09-13.** The diffuse clause used to read "no
/// diffuse-capable aux is vendored (tier 3)" for EVERY diffuse
...
/// is not a certification that a diffuse-capable fit makes it safe (the ruling
/// names diffuse bases as one of the two regions most likely to reverse its
/// RI-J finding).
///
...
inline std::optional<std::string> AuxSelectionNotice(std::string_view orbitalBasisName,
    ...
    // a DEFAULT, not a match, and the ruling names this pairing - a
    // J-fit silently reused for K - as the risk the whole rule exists to
    // prevent. It runs, and it says so. "Matched" is decided by the same
    ...
    return "orbital basis \"" + std::string{orbitalBasisName} +
           ...
           *selected + "\" for it: " + regions +
           " the run proceeds, and the RI error class is reported rather than assumed (warn, "
           "still run, and reserve refusal for a singular fit matrix or an unmeetable tolerance)";
}

/// Whether a RESOLVED auxiliary basis name is a JK-optimized fit - the
/// predicate an `ri_jk` request must pass (the refusal, resolved by `ri_jk`'s
/// approvals of 2026-09-12: "refuse when the aux in effect for the JK kind is
/// not a JK-optimized fit").
///
...
///   doors.
///
/// **This predicate is the guard the ruling's named hazard turns on**
...
/// and what it returns now.** Before the ruling, SelectAuxBasis mapped
...
/// effect is not JK-optimized. Kept beside the predicate so the
/// wording and the test cannot drift apart, and so the message can name its
/// own remedy rather than only the fault - the ruling's own requirement, and
/// the standing rule that a refusal names the condition that fired.
