...
// change that adds an AccuracyPreset enumerator cannot leave it unplaced; the
...
/// Q_ij * Q_kl below it are dropped. The mapping was validated against the
/// committed pins by the preset sweep (benchmarks/preset_sweep.cpp,
/// 2026-08-23).
...
/// The density-weighted screening threshold of a preset: density products
/// |D_element| * Q_ij * Q_kl below it are dropped by the direct Fock
/// builders. The values mirror SchwarzThreshold's J/K energy budgets
/// (kLoose 1e-6, kNormal 1e-10, kTight 1e-12 Eh - the PRESET targets, not
/// the delivered errors: the sweep measured 6.6e-9 / 6.3e-12 / ~1e-14
/// energy error on the fixtures, each more than an order of magnitude
/// inside its budget). Validated against the committed pins by the preset
/// sweep (benchmarks/preset_sweep.cpp, 2026-08-23).
/// \param preset The accuracy preset.
...
/// The certified-mixed-precision gate of a preset: a shell quartet may use
/// the certified fp32 lane when its a-priori density-weighted bound
...
/// The kLoose/kNormal gates were tightened by the preset sweep
/// (benchmarks/preset_sweep.cpp): at the original 1e-6/1e-8 the lane
...
/// The QFMM well-separatedness parameter of a preset: smaller theta means a
/// stricter near/far classification - more pairs computed exactly, higher
/// accuracy, more cost; theta -> 0 degenerates to "everything is near
/// field" - the near-field-only path, bit-exact when the near path's
/// screening thresholds match the comparison's (the (kNormal, C12)
/// vacuous-tooth 0.0 pins that case). The recalibrated ladder
/// {0.45, 0.3, 0.0} for {kLoose, kNormal, kTight} was measured at the
/// per-preset extents {1e-6, 1e-8, 1e-10} on the C12/C24 alkane chains -
/// the earlier {1.05, 0.85, 0.7} held only at the all-tau = 1e-10 extent,
/// and the extent reclassifies near/far pairs, so those earlier values are
/// not transferable (the 567-cell committed grid is
/// benchmarks/data/qfmm_ladder_sweep_full.csv). kLoose (0.45, 5) is the
/// cheapest common in-budget cell (reduced slack on C12 - the record states
/// 6.2x, the budget is not widened); kNormal (0.3, 5) is pinned by C24 (the
/// C12 tooth is vacuous there - all-near, bit-exact; the far-alive pin uses
/// the next-larger fixture); kTight 0.0 is the degenerate gate - the
/// near-field-only path, the designed converge-to-direct fallback. At the
/// ORIGINAL kTight budget (1e-9) no (theta, L) reached it at tau 1e-10 in
/// the committed kNormal-screened-direct comparison, and that budget has
/// since been re-derived to 1e-8 precisely because that comparison's ground
/// truth screens coarser (1e-10) than the kTight near path (1e-12) - the
...
/// ground - zero multipole benefit at kTight - NOT on unreachability. At
/// 1e-8 the tight end now has LIVE in-budget cells on the committed sweep
/// grid (C12 (0.7, 7) 3.19e-9 with 282 far pairs; C24 (0.3, 4) 6.13e-9 with
/// 107), so a "no vacuous ladders" acceptance clause is satisfiable at
/// kTight only by moving the rung - a rung change, not part of the budget
/// re-derivation.
/// \param preset The accuracy preset.
...
/// The QFMM multipole expansion order of a preset: higher L_mult means a
/// more accurate far field at more cost per interaction. {5, 5, 0} for
/// {kLoose, kNormal, kTight} - the sweep winners: kLoose and kNormal need
/// L = 5 (kLoose C24 at L = 4 overflows the 1e-5 budget, 4.54e-5);
/// kTight's 0 rides the degenerate theta-0 rung, where the multipole
...
/// ComputeExtent's Gaussian falloff radius sqrt(-ln(tau) / p). This is the
/// GEOMETRIC "how big is this pair's footprint" cutoff of the octree boxes
/// - deliberately distinct from the preset's Schwarz/density screening
/// thresholds, which answer a different question ("is this quartet worth
/// computing"); conflating the two is a trap this ladder avoids by keeping
/// them separate by mechanism.
...
/// original kQfmmExtentThreshold (the committed
...
/// {1e-5, 1e-7, 1e-8} of the ladder sweep,
/// benchmarks/data/qfmm_ladder_sweep_full.csv, the kTight rung re-derived
/// 1e-9 -> 1e-8 on the 2026-09-13 measurement recorded below): the
...
/// of the AUXILIARY FIT: tightening the preset does not improve that fit, so
/// the fit's measured per-atom error is the number to read against these
/// bars and NOT against a preset-invariant absolute. A mapper that read as a
/// whole-path accuracy budget would promise exactly the improvement that
/// distinction rules out.
///
...
/// auxiliary-pair density screen is `DensityThreshold`. This file
/// deliberately carries no second table for them: the preset sweep's pins
/// and the gate-tolerance check both compare against the mapped values, and
/// a copy is the drift the one-home rule prevents.
///
...
/// accuracy cell (`integrals/tests/ri_jk_validation_test.cpp`; the two-cell
/// table and the ratios are in the run-schema document, not restated here).
///
...
/// that printed it (schema 31, `RunExchangeError`).
///
...
/// bars are per atom and the ratios in the table are per atom, so a record
...
/// an approximated-exchange run discloses beside its preset's bar (schema 31).
///
...
/// only, so no preset can be served by a coarser fit. Marking it per preset
/// rather than leaving it implied is what stops the two knobs being
/// conflated - a `kTight` request is not a request for a better fit, and a
/// `kLoose` request does not license a J-only one (which `aux_basis.hpp`
/// IsJkOptimizedAux refuses outright).
/// \param preset The accuracy preset.
