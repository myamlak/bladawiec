...
/// \file
/// The 3-center RI-J engine: the
/// (uv|P) integrals over an auxiliary basis through the matrix-form MD
...
struct RiEngineOptions {
    ...
    /// behavior is bit-identical to that constant. Zero disables
    ...
    /// path within the per-preset budget. With a budget set, the light
    ...
    /// (in place since 2026-09-13): the screened
    ...
    /// samples on a machine busy with concurrent builds and tests): three
...
/// omission, never null.
///
/// The count moments are defined by this instrument, by occurrence class:
...
/// processed in
/// byte-capped strips over the metric rows (each strip closed before the
...
/// The cross-chunk dedup state of a chunked 3-center build's
...
///
/// The chunking contract: the screened task list is
...
/// do not depend on batch grouping - assembly only partitions work; the
/// bit-identity pin records this). The Create-time screen applies
