...
struct EriCudaOptions {
    ...
    /// Package C step 2: the device workspace budget mirror the Create-time
    /// mode decision runs against - the AM-B FastPath/LightPath surface
    ...
    /// Package C step 2: the forced-LightPath knob - true FORCES the
...
/// The Create-time mode record of the GPU engine (Package C step 2): the
/// AM-B decision generalized to the device context, plus the byte terms
...
/// The per-call device-host transfer accounting (Package C step 4, design
/// section 3 - the residency contract: per-iteration bus traffic = density
/// up, Fock up/down, screened lists - and nothing else, at any system
...
/// The engine's Create-time device footprint estimate: the byte
/// terms the device workspace budget is charged against - the exact
/// uploaded table sizes, the fixed Boys copy, the per-call density/Fock
/// matrices, the kV2 per-batch global scratch, the per-call output mass and
/// the structural allowance. Computed once at Create from the pair data,
...
public:
    ...
    /// The Create-time device footprint estimate (design section 5): the
    /// byte terms the device workspace budget is charged against. Computed
    ...
    /// The Create-time mode (Package C step 2): the AM-B FastPath/LightPath
    ...
    /// The Create-time mode record (Package C step 2): the decision plus
    ...
    /// The per-call transfer accounting (Package C step 4, design section
    /// 3): the device-host bus traffic of the last BuildFock call, by
