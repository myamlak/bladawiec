...
/// \file
/// The GPU J/K Fock builder:
/// the DirectJkFockBuilder contract with the J/K contraction moved onto the
/// CUDA device. The per-iteration screening is the SHARED path of
...
/// elementwise (the device-parity gate).
...
public:
    ...
    /// quartet the routing gate sent through the fp32 lane (0.0 when the
    /// lane is disabled): each quartet's kernel bound times its max-|D|
    /// block weight - the same weight the routing gate uses, the same
    ...
    /// replaces the routing and the per-band passes dispatch between
    /// class runs.
    ...
    /// The engine's Create-time device footprint estimate (design section
    /// 5): the byte terms the device workspace budget is charged against.
