...
/// \file
/// The direct J/K Fock builder:
/// batched ERIs contracted with the density on the fly - no dense n^4
/// tensor. Schwarz screening, density-weighted screening, and the
/// certified mixed-precision routing per iteration. The engine performs no
...
/// Per-call diagnostics of one direct Fock build: filled by BuildFock only
/// when a statsOut pointer is passed;
...
/// sub-span, the prep's own three-part split and the kernel-span probe's
/// three sub-spans and four counters), the pair-build count,
...
///
/// THE k > 1 REGIME (the bounded-concurrency batch loop;
...
struct FockBuildStats {
    ...
    /// The ket-transform sub-span of the kernel time (the kernel-span probe, the
    /// lean path only): the group's one KetTransformBatched call, from the
    ...
    /// The bra-transform sub-span of the kernel time (the kernel-span probe, the
    /// lean path only): the whole pass-2 task loop - the per-task bra
    ...
    /// The pass-1 group census (the kernel-span probe, the lean path only): how
    ...
    /// The (group, ket primitive pair) iteration count (the kernel-span probe,
    /// the lean path only) - exactly the KetTransformBatched call count,
    ...
    /// The merge chain's busy time: time in the
    ...
    /// Optional per-window wall times (the window probe, the lean path
    ...
    /// of the builder's Create). Cleared and refilled when non-null; order is
...
/// contracts quartets whose pair-pair (bra, ket) is in the restriction.
/// Stored as a symmetric bitset over leaf-node pairs - nLeaves^2 bits, bit
...
/// ~8.5 GB for c60_sto3g, the 2026-08-29 incident). The bitset is
...
struct FockBuildOptions {
    ...
    /// The two-level density screen - the quartet-level product gate AND the
    /// per-element re-filter (the flag contract). On (the default): the
    ...
    /// per-target threshold shape; the J sections halve the threshold
    ...
    /// builds without the re-filter bit-for-bit.
    ...
    /// does not and where no probe result is available (the owner's
    /// conservative interim, 2026-09-12). An explicit request still wins
    ...
    /// The certified bound's global budget ENFORCEMENT (opt-in, off by default): when
    ...
    bool buildCoulombOnly = false; ///< Skip K (the UHF J-only mode, the companion
                                   ///< to buildExchangeOnly). At most one of the two should
    ...
    /// (the pair-class root fix, 2026-08-31): the table's Create-time
    ...
    PairPairRestriction restrictToPairPairs; ///< QFMM near-field restriction: when
                                             ...
                                             ///< bit-identical to the unrestricted one (the QFMM
                                             ///< acceptance gate).
    /// The QFMM leaf-driven near-field domain: when engaged,
    ...
    /// The adaptive-memory workspace budget: when set, Create() decides the
    /// build mode from the Create-time footprint estimate against the
    /// budget's remaining bytes (FastPath
    ...
    /// no reservation - save the class-path host-RAM guard (2026-09-01):
    /// with a symmetryReduction, Create() refuses (kOutOfMemory) when the
    ...
    /// The enclosing builder's counted surviving-pair sweep (the QFMM outer): when non-zero,
    ...
    /// default) auto-sizes the chunk from the budget's remaining bytes at
    /// Create (the chunk formula's closed form - the loose bounds'
    /// conservative floor).
    ...
    /// decision runs against - the adaptive-memory FastPath/LightPath surface
...
/// against the budget's remaining bytes at Create (the cumulative monotone
...
struct FockModeInfo {
    ...
    /// everywhere else. The field records the actual k a fired run carried.
    std::size_t concurrentSlots = 1;
    ...
    std::size_t diskBytes = 0; ///< The disk rung's modeled on-disk (uv|P) payload, the
                               ...
                               ///< in the model but NOT reserved from RAM (the
                               ///< RAM reservation list is that same composition:
    ...
    bool classPathDisengaged = false; ///< The class-path admission gate fired (the pair-class
                                      ///< root fix, 2026-08-31): the table's never-under
...
/// The global budget enforcement's per-call outcome
...
public:
    ...
    /// the routing gate admitted through the fp32 lane (0.0 when the lane is
    ...
    /// delivered Fock error, consumable as certified); the committed sweep
    /// checks the per-quartet bounds against the preset J/K budgets.
    ...
    /// fired estimate terms and the reservation outcome; nullopt when no
