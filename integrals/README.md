...
the class-aware (petite-list) symmetry reduction, and the in-memory ERI cache
tier. It does **not** run SCF (that is `scf/`, which drives these builders
through the `FockBuilderFn` seam) and does not persist integrals to disk (that
is `storage/`).
...
- `ComputeEriBatch` / `ComputeEriBatchCertified` — the batched ERI API and its
  certified fp32 lane.
- `DirectJkFockBuilder` — the density-screened direct J/K builder.
...
- The Boys kernel is an exception: it is owned by the boys submodule.
  ...
  with explicit paths. This repository never regenerates or edits submodule
  files; fix upstream.
