// Create-time footprint formula tests (footprint.hpp): the exact per-pair
// byte formulas against hand
...
// the 70-water STO-3G counted-admission band (the counted-admission fix:
// the exclusion tests the exact Schwarz-screened pattern bytes, so
...
// counted-task tests pin CountSchwarzSurvivingRiTasks against the RI-J
...
// magnitudes the screening and certified-lane gates expect (the fock_build_test
// helper's shape, deterministic).
...
TEST(Footprint, ClampAcrossTwoArenas) {
    ...
    // band one arena wide (a review fix [1]).
...
TEST(Footprint, ModeSelectionHugeBudgetFastPathReservesExactly) {
    // The direct builder on a generous budget (fix A: the exchange term is
...
// 2026-08-31): 208 B per canonical member quartet (the member, a
...
TEST(Footprint, ClassTableBytesArithmetic) {
    ...
    // class-table containers carry the tagged class_table allocator, and
...
// The class-table admission gate (the pair-class root fix, 2026-08-31):
...
// The class path's materialization charge at the 24-water cluster, measured
// 2026-09-13: CountClassMaterialization on the fixture's
// reduction and the production Schwarz bounds, through
...
// The class-table admission gate (the pair-class root fix, 2026-08-31;
// the materialization accounting, 2026-09-13): the class path's
...
// The null-budget escape-hatch host-RAM guard (2026-09-01): the legacy no-budget path
// (workspaceBudget null - memory_cap_gib = 0, the documented opt-out) engages the class path
// unconditionally - the budget-path admission gate never runs, so a c60-shaped request would still
// die as a raw machine-OOM allocation growing the canonical member-quartet space. The guard charges
// the same never-under estimate (ClassTableBytes) against the node's TOTAL physical RAM and refuses
// CLEANLY (kOutOfMemory) when the table cannot fit. The 60-water cluster IS the c60 STO-3G shell
// shape (300 shells, 45,150 shell pairs, 420 functions): the never-under charge ~228 GiB exceeds
// the total physical RAM of any machine this suite runs on (up to ~128 GiB CI runners), so the
// guard fires deterministically. The refusal is Create-time only - BuildFock is never reached, no
// Fock is built.
...
TEST(Footprint, RiModeSelectionReservesTheFullStack) {
    ...
    // After the nested-exchange lift that nested decision fires k = team into its
...
TEST(Footprint, RiAuxStoreModelsThePerShellKets) {
    // The aux term is the PER-SHELL ket
    ...
    // stores - the far larger sum the metric used to charge before the fix.
...
TEST(Footprint, RiFirstIterationBytesArithmetic) {
    // The first-iteration estimate terms: the first
    ...
    // treatment, the list the per-iteration batch loops iterate
    ...
    // The 104 B/task envelope vs the allocation-site struct sizes:
    // Item (RiTask + 2 ints + 4 size_ts, 56 B),
    ...
    // (the counted slice fix): the light rung's slice charges 104 B per
    ...
    // no-count dense form above - the delta the razor budgets in the
...
TEST(Footprint, RiFirstIterationFiresTheLightRung) {
    ...
    // drafted for; the check itself proved decision-inert (the review found
    // the clamp's refusal region strictly contains the floor's) and
    ...
    // today-clamp zone [oldTotal - 2T(batch-1), oldTotal]. The plan's
    ...
    // sliceBytes), a documented deviation from the literal family the
    // band was drafted on.
    ...
    // computes it (the counted-task fix): the footprint's taskListBytes
    ...
    // assertion at its earlier margin). The screened-quartet charge
    ...
    // sits ABOVE the razor: the survivor charge no longer lifts the charged
    ...
    // The charged-total geometry (measured 2026-09-03): the fast rung's
    // ineligible mass (506,797,184 B) sits above the 458,116,763 B razor
    // - its refusal is by mass now (the band role the screened charge had
    ...
    // included). After the nested-exchange lift the nested exchange's k = team
...
TEST(Footprint, RiFirstIterationFiresTheBlockedMetricRung) {
    ...
    // arena; the blocked metric's own counts) sits below the light rung's by the
    ...
    // After the nested-exchange lift the nested exchange fires k = team into its
...
TEST(Footprint, ExchangePerCallBytesArithmetic) {
    ...
    // the earlier fit, 2026-09-03): ExchangePerCallBytes now charges the
    ...
    // post-fix C24H50 fp64 bound ~1 GiB at the 512 MiB default cap) and
    ...
    // F64Bound only, the pure-fp64 boundary), kLoose/kNormal route
...
TEST(Footprint, ScreenedQuartetFractionLawArithmetic) {
    // The fraction-law arithmetic pins:
    // the piecewise log-log law through the measured anchors - flat
    ...
    // 4e-8 at and above n = 5000 (the audit's ~n^2 survival anchor).
    ...
    // 2026-09-03 for the survivor charge (the same quartet counts x
    ...
    // the never-under end of the split, the ExchangePerCallBytes lane
    // convention).
    EXPECT_EQ(
...
TEST(Footprint, CoulombOnlyStackCarriesNoExchangeCharge) {
    // The Coulomb-only pin: the exchange per-call term
    ...
    // = 1,610,612,736 B - the preset flag gate keeps the per-batch
...
TEST(Footprint, ExchangePerCallTermRidesTheBatchClamp) {
    // The values-term band pin (the earlier-fit flip test, reframed 2026-09-03): the
    ...
    // the very budget the earlier fit refused (the fix's observable:
    // the model no longer refuses admissions the chunked code fits). The
    // razor is the margin-based form (the user's 2026-09-07 cross-platform
    // band ruling): one full batch cap of the two-arena clamp under the
    ...
    // blocked metric gap (1,262,080 B after the blocked-metric counts)
    ...
    // F64Bound + F32Live, kNormal) and Total() folds it. The
    ...
    // charge, the fraction-law anchor).
    ...
    const std::size_t lightClampIneligible =
        lightTerms.Total() - lightTerms.scratchBytes - lightTerms.valuesBufferBytes;
    // The razor budget (the user's 2026-09-07 cross-platform band
    // ruling): the margin-based form, derived from the platform-computed
    ...
    // the pre-term total (the earlier razor's character), at team size 1 the
    ...
    EXPECT_LT(lightClampIneligible, budgetBytes);
    // THE ABSORPTION (the earlier flip's inverse, at the clamp level): the
    ...
    // capacity and refused this budget; the fix restores the
    ...
    // earlier refusal at this budget is gone) - the light rung fires with
    ...
    EXPECT_LE(info->predictedBytes, budgetBytes);
    // After the nested-exchange lift: the nested exchange's k = team charge
...
TEST(Footprint, CacheBytesModelsTheEntryMapAllowance) {
    // The EriBatchCache entry map is
...
TEST(Footprint, QfmmModeSelectionNestedCarriesTheSweepCount) {
    // The spurious-nested-refusal band for the QFMM family (scaling-audit
    // F8, the fix-[2] asymmetry - the RI-J's nested exchange got its carrier
    // at 0db4972, the QFMM's nested near-field did not): the budget fits the
...
TEST(Footprint, ModeSelectionValidatedSweepSkipsExclusionOne) {
    // Review fix [2]: a nested builder carrying the enclosing RI-J's
...
TEST(Footprint, RiModeSelectionNestedAllSurviveBandConstructs) {
    // The spurious-nested-refusal band (review fix [2], end-to-end):
    ...
    // bound - the razor re-derives on the charged ineligible mass (the
    // same pattern as
    // RiFirstIterationFiresTheLightRung).
    //
    // The values term (the structural F64Bound + F32Live
    // per-batch terms replace the whole-call fit): the fixture's batch is
    ...
    // batch).
    //
    // The honest screened-quartet charge (the 224 B/q
    // control mass replaced by the ~24-32 B/q survivor): the exchange
    ...
    // the earlier razor's mass is gone and the razor re-derives on the
    ...
    // The band: the budget rides the all-survive bound (the charged
    ...
    qcx::integrals::internal::BuildNeighborList(
        *pairList, *schwarz, options.accuracy, offsets, indices);
    // The screened task-grid count, exactly as the engine's decision block
    // computes it (the counted-task fix): the mirror recompute below
    ...
    EXPECT_LE(info->predictedBytes, budgetBytes);
    EXPECT_EQ(info->reservedBytes, info->predictedBytes - info->exchangeBytes);
    // After the nested-exchange lift: the nested exchange fires k = team at the
...
TEST(Footprint, LightPathKnobForcesTheModeAndReserves) {
    ...
    // pair set) and the retained flattened shells (5 shells) - the chunk
...
TEST(Footprint, LightPathKnobBitIdentityPin) {
    ...
    // the named pure-fp64 pin preset (the certified lane off).
    auto molecule = qcx::testing::MakeH2oSto3g();
    auto basis = qcx::testing::MakeH2oSto3gBasis();
    ASSERT_TRUE(molecule.has_value());
    ASSERT_TRUE(basis.has_value());
...
TEST(Footprint, LightPathKnobChunkSizesAgreeWithinReductionOrder) {
    ...
    // bits; the bit pin (chunkPairs = nPairs) is the separate single-chunk test.
    auto molecule = qcx::testing::MakeH2oSto3g();
    auto basis = qcx::testing::MakeH2oSto3gBasis();
    ASSERT_TRUE(molecule.has_value());
    ASSERT_TRUE(basis.has_value());
...
TEST(Footprint, LightPathCountedAdmissionAtScaleSelectsTheFastRung) {
    // The counted-admission fix at the 70-water scale (the direct-builder
    ...
    // LightPath (the misfire shape: a 24.44 GiB all-survive
    // bound against a ~38 MB counted pattern). The all-survive bound is
    // the premise (the pre-gate runs); the admission is the fix.
    // Create-only (no BuildFock - the fixture is the largest scale here); the
...
TEST(Footprint, LightPathExclusionBoundsTheChunkByTheCountedPeakRow) {
    ...
    // canonical pairs (the 586-function point the RI-J print below uses). The
...
TEST(Footprint, RiJkNestedExchangeReachesItsOwnLightRungOnTheCountedPreGate) {
    ...
    // (the engine-exclusion-i record, 2026-09-17: the counted pattern's 183.74 GiB of
    ...
    //
    // The band is FORCED by the budget, not by the system size (the owner's rule: the
    // property is size-independent), and both rungs are recorded: the OUTER's tensor term
    ...
    //
    // The fixture is this file's own counted-admission fixture (the 586-function anchor,
    ...
    // small s-only fixtures cannot host it (at 24 waters the survivor charge is 2.1 MB against
...
TEST(Footprint, CountSchwarzSurvivingPairsDenseAndDegenerate) {
    ...
    // pole of the counted-admission fix: the exact count must never admit a pattern the
...
TEST(Footprint, CountSchwarzSurvivingPairsMisfireShapeAdmitsTheBenchBand) {
...
TEST(Footprint, CountSchwarzSurvivingRiTasksMatchesTheSweep) {
    // The counted-task count's bit-identity on a real RI-J fixture (the
...
TEST(Footprint, CountSchwarzSurvivingRiTasksDenseAndDegenerate) {
    ...
    // pole of the counted-task fix: an all-survive grid must count every
...
TEST(Footprint, CountSchwarzSurvivingRiTasksSparseGridShrinksTheDenseReserve) {
    // The 5000-shape story (the counted-task target): the pre-fix
...
// The free 586-point structural print (the evidence ladder's first step,
// run unconditionally by the 2026-09-13 ruling): the engine's
...
TEST(RiFootprint, PrintsTheEngineCreateTimeFootprintAtC24H50Def2Svp) {
    ...
    std::printf("\n  the anchor print - the engine's own Create-time footprint, C24H50/def2-SVP\n");
...
// ---------------------------------------------------------------------------
// The 5000-basis scale print (the admission audit's model half, 2026-09-13)
//
// The audit names five paths at the 5000-basis target - direct
// screened, QFMM, RI-J fast, RI-J light, and the streaming rung - and
// asks each path's peak under the 16 GiB cap regime. This test answers it from
...
// the 586-function def2-SVP anchor of the ladder is printed first so
...
std::size_t PrintScaleLadderPoint(const ScaleLadderPoint& point) {
    ...
    // terms: the streaming rung's whole-call task-master survivor (the survivor
    // charge), the class table and the all-survive pattern bound.
    ...
    // and for the RI-J nested exchange: the chunked LightPath. Its own footprint function
    ...
    // is walked and every outcome is printed: the answer the scale question
...
TEST(RiFootprint, PrintsTheScaleLadderAtThe5000BasisFixture) {
    ...
    const std::array<ScaleLadderPoint, 4> ladder{{
        ...
        {"the anchor: C24H50 / def2-SVP (n = 586)", 24, "def2-svp", "def2-universal-jfit"},
    }};
    ...
    // The two counts the fixture owes: the 586-function anchor's 586 (so the printed
    // ratios are read against the already-measured point) and the 4,974-function
    // target's
    // 4,974 (the manifest file's own count - a print that lands elsewhere is
    // not the fixture the audit names).
