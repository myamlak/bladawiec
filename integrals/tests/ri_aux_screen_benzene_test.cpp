...
// with the screen's 0.01 kcal/mol = 1.59e-5 Eh bar stated beside the number.
//
// The unscreened side is this repo's own dense occ-RI-K over the SAME
// metric-transformed tensor (the contraction entry points - the code the
// composed builder ran before the screen landed), never a second builder's:
// one tensor, one density, one occupied block, and one threshold, so the two
// sides differ only by the screen.
//
...
using qcx::testing::riAuxScreen::kScreenTolerance;
...
TEST(RiAuxScreenTest, BenzeneDef2TzvpScreenedEnergyStaysWithinOneHundredthKcalPerMol) {
    if (IsFastOnlyMode())
    {
    ...
    // The preset is not named for this row; kNormal is the operating
    ...
    RecordMeasurement("benzene_def2tzvp.screen_tolerance", kScreenTolerance);
    RecordMeasurement("benzene_def2tzvp.screen_tolerance_kcal",
                      kScreenTolerance * kCaloriesPerHartree);
    RecordMeasurement("benzene_def2tzvp.screen_tolerance_ratio", outcome->move / kScreenTolerance);
    ...
    std::cout << "[ri_jk screen] benzene/def2-TZVP acceptance (aux " << kAuxName
              ...
              << " kcal/mol against the acceptance bar " << kScreenTolerance
              << " Eh = 0.01 kcal/mol, ratio " << (outcome->move / kScreenTolerance) << "\n";

    EXPECT_LE(outcome->move, kScreenTolerance)
        << "the screen's acceptance bar: the screened converged energy must stay within "
    ...
    // evidence the threshold ruling needs, and the gate's
