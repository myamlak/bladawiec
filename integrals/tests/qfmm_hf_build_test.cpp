...
//      FarFieldPairCount liveness flag asserted per rung (kLoose C12 is
//      far-alive - 14 far pairs at (0.45, 1e-6); kNormal
//      C12 is the VACUOUS tooth - all-near at (0.3, 1e-8), so its budget
//      row measures the split-pass fp pairing, and the kNormal rung's real
//      pin lives on the C24 chain at the driver level),
//   4. keep the C24 far-alive rungs alive at Create time (counts 1997 /
//      499 far pairs at the kLoose (0.45, 1e-6) and kNormal (0.3, 1e-8)
//      winners - the C24 BUDGET rows run at the driver level, threaded,
//      where the suite's serial-C12 / threaded-C24 division of the
//      evidence lives).
...
// equal-preset comparison protocol, like BuildFusedDirectFock).
...
// budget metric (Eh) of the composed approximation.
...
// and deliberately do NOT follow the kTight re-derivation
...
TEST(QfmmHfBuildTest, CreateWiresTheWorkspaceBudgetAcrossBothHalves) {
    ...
    // budget/mode records gap fix): a workspaceBudget is
    ...
    // two halves at arena-dominated caps is engine-model work outside
    // this suite; the pin runs in the natural-cap regime the
...
TEST(QfmmHfBuildTest, PresetLadderStaysInsideTheCommittedBudgetsAtEqualPreset) {
    // The committed QFMM ladder on the C12 chain, each rung at its recorded
    // winner, each measured against a fused direct reference at the SAME
    // preset. The kLoose rung is far-ALIVE (14 far pairs at the (0.45,
    ...
    // rung is the VACUOUS C12 tooth (all-near at (0.3, tau 1e-8) - zero far
    // pairs, count asserted below; the row measures the split-pass fp
    // pairing, and the kNormal rung's real pin lives on C24 at the driver
    // level). The kTight rung is the theta -> 0 degenerate gate (near-field-
    // only by design).
    if (IsFastOnlyMode())
    {
    ...
    for (const AccuracyPreset preset : presets)
    {
        qcx::integrals::QfmmOptions options = MeasurementOptions();
        options.accuracy = preset;
        // The preset ladder below is a record of the RECORDED model (midpoint
        // extents + the width test); pin it so the record keeps measuring what
        // it recorded. The DEFAULT is now the corrected
    ...
    // field would hide the multipole truncation - the fixture gap), the
    // kNormal rung is far-ALIVE too since the default moved to the corrected
    // pair (the recorded vacuous-by-design count was 0 at kNormal on this
    // fixture; the corrected default measures 2864 far pairs), and the kTight
    // rung stays degenerate BY DESIGN: its own rung is the theta -> 0 gate, so
    // a preset whose rung IS the gate stays exact rather than being handed a
...
TEST(QfmmHfBuildTest, C24KeepsItsFarAliveRungsAliveAtCreateTime) {
    // The C24 chain carries the non-vacuous kNormal pin (the C12 tooth is
    // all-near at (0.3, 1e-8)) and the strongest kLoose
    // exercise. At CREATE time the liveness flag must report the recorded
    // far-alive winners (1997 far pairs at kLoose (0.45, 1e-6), 499 at
    // kNormal (0.3, 1e-8)) and the degenerate kTight rung must be empty -
    ...
    for (const AccuracyPreset preset : presets)
    {
        ...
        // kLoose and kNormal are the recorded far-ALIVE winners (1997 / 499
        // far pairs); kTight is the degenerate theta -> 0 gate (empty).
