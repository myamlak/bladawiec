...
// never split a shell),
// and the chunk-range validation refuses the degenerate ranges. The
// chunk-scoped task-list contract (the full nPairs x nAuxShells list never
// materialized) is structural in the implementation; the values below
// verify the partition-equivalence the chunked build rests on, and the
...
TEST(RiEngineChunkTest, ChunkedEqualsMonolithicOnTheSameTasks) {
    ...
    // Byte-for-byte on the same tasks (gate 6.7.1).
    EXPECT_EQ(
