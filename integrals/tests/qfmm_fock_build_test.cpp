// QfmmJBuilder acceptance tests: the composed builder (near field = the direct Coulomb
...
//      L_mult and in L_mult at every fixed theta (carried through the
//      builder, on the C12 alkane fixture),
//   4. keep the accuracy-preset ladder strictly decreasing at the top
//      rungs (kLoose > kNormal in both the relative Frobenius and the
...
//      re-derived from 1e-9). On this fixture
...
//      fixture, the tight-end floor, re-measured 3.33386e-9 on 2026-09-13). That
//      floor is exactly why the kTight budget was
//      exactly why the kTight budget was
...
// measurement configuration of the Step-4/5 gates (qfmm_fixture.hpp's
...
qcx::Result<Eigen::MatrixXd> BuildQfmmFockGated(
    ...
    // The model the PRODUCTION side runs: the equivalence claim below
    ...
    // product ball with the surface test; the legacy pair stays the default
    // here so the older legs keep comparing the geometry they were pinned on.
    ...
    // all-lMult vectors reproduce the plain runs exactly.
...
// J_direct))| - the budget metric (Eh). The core Hamiltonian cancels.
...
TEST(QfmmFockBuildTest, PresetConstantsMatchTheCalibratedLadder) {
    // The committed preset constants: the theta ladder {0.45, 0.3, 0.0} for
    // {kLoose, kNormal, kTight} was measured at the per-preset extents
    // {1e-6, 1e-8, 1e-10} on the C12/C24 alkane chains - the earlier
    // {1.05, 0.85, 0.7} held only at the all-tau = 1e-10 extent. kLoose
    // (0.45, 5) is the cheapest common in-budget cell (reduced
    ...
    // extent ladder {1e-6, 1e-8, 1e-10} is unchanged (theta alone is the
    // ladder knob, not tau).
    ...
    // sentinels (0 / -1), and the default maxLeafSize.
...
TEST(QfmmFockBuildTest, ThetaZeroGateRecoversTheDirectCoulombMatrix) {
    // THE Step-6 wiring gate: at QfmmOptions::theta < 0 (the degenerate
    ...
    // order - the Step-4 gate re-verified through the composed class).
    auto molecule = MakeH2oSto3g();
    ASSERT_TRUE(molecule.has_value()) << molecule.error().message;
    auto basis = MakeH2oSto3gBasis();
    ASSERT_TRUE(basis.has_value()) << basis.error().message;
...
TEST(QfmmFockBuildTest, DefaultsResolveToTheNormalPreset) {
    ...
    // ThetaForPreset(kNormal) = 0.3 / LMultForPreset(kNormal) = 5, so the
    // default-options build must be
    ...
    // An ABSENT theta no longer resolves to the recorded (theta, L) pair: the
    // separation model is
    // separationMode's (the surface ball, k = 1.0) and the geometry is the
    // product ball. The resolution path is still what this test exercises, so
    // the explicit spelling of the NEW default must reproduce the default
    // build bit for bit (LMultForPreset still resolves lMult = -1).
    ...
    explicitOptions.extentModel = qcx::integrals::QfmmExtentMode::kProductBall;
    explicitOptions.separationMode = qcx::integrals::QfmmSeparationMode::kSurfaceBall;
    explicitOptions.separationK = 1.0;
    ...
    // The retired pair stays selectable and is a DIFFERENT build (the flip
    // moved the default, not the availability): naming the recorded model must
    // not silently equal the default.
    qcx::integrals::QfmmOptions retired = defaults;
    retired.theta = qcx::integrals::ThetaForPreset(qcx::integrals::AccuracyPreset::kNormal);
    retired.extentModel = qcx::integrals::QfmmExtentMode::kMidpointBound;
    retired.separationMode = qcx::integrals::QfmmSeparationMode::kWidthTheta;
    retired.separationK = 0.0;
    const auto retiredBuild = BuildQfmmFock(*molecule, *basis, *core, density, retired);
    ASSERT_TRUE(retiredBuild.has_value()) << retiredBuild.error().message;
    EXPECT_GT(MaxAbsoluteDifference(*retiredBuild, *defaultBuild), 0.0);
}

TEST(QfmmFockBuildTest, CrossoverHeuristicRespectsThePlaceholderAndOverride) {
    // The crossover-heuristic seam: the default (-1) resolves
...
TEST(QfmmFockBuildTest, FarFieldErrorDecreasesMonotonicallyInThetaAndLMult) {
    // The monotonicity properties through the BUILDER, on the C12 alkane
    // fixture (38 atoms, 86 functions, the acceptance fixture these gates
    // were first written for): at every fixed L_mult the relative Frobenius
    ...
    // The machinery guard (the sanity bound carried forward): the
...
TEST(QfmmFockBuildTest, PresetLadderMatchesTheRecordedP3Rungs) {
    // The preset validation: at each accuracy preset the combination
    // (ThetaForPreset, LMultForPreset) must stay inside its committed budget
    // - the QFMM budgets {1e-5, 1e-7, 1e-8} Eh (the kTight rung re-derived
    // from 1e-9; the ladder is QfmmBudgetForPreset, accuracy.hpp)
    // (benchmarks/data/qfmm_ladder_sweep_full.csv - the
    // QFMM-vs-direct budget split: the direct screening budgets keep applying to
    ...
    // deterministic, the tight-end floor (re-measured 3.33386e-9 on 2026-09-13), asserted
    // as the (1e-9, 1e-8) band below - a strict monotonicity assertion
    // across the degenerate boundary does not survive the measurement).
    if (IsFastOnlyMode())
    {
        GTEST_SKIP() << "fast mode";
    }
                                                        ...
                                                        qcx::integrals::AccuracyPreset::kTight})
    {
        qcx::integrals::QfmmOptions options = MeasurementOptions();
        options.accuracy = preset;
        // The committed ladder is a record of the RETIRED model - its pins (the
        ...
        // floor band in the budget block below.
        ...
        // The committed budgets: the QFMM production budgets {1e-5, 1e-7,
        // 1e-8} for {kLoose, kNormal, kTight}, measured on the C12/C24
        // chains at the per-preset extents {1e-6, 1e-8, 1e-10}. The earlier
        // 0.71 / 0.38 / 1.5e-3 were calibrated at the all-tau = 1e-10 extent
        // and are not transferable. On this fixture: kLoose (0.45, 5)
        // measures 1.605e-6 - 6.2x slack, REDUCED from the pick rule's
        // >= 10x (S5e, recorded honestly - the budget is not widened);
        ...
        // (the budget-vs-floor case was resolved in favour of the wider
        // budget).
    ...
    // not a QFMM error). The C24 pin fixture's strictness is carried by the
    // sweep evidence.
...
TEST(QfmmFockBuildTest, FarFieldPairCountGrowsAsTheExtentLoosens) {
    ...
    // loosens (kLoose tau 1e-6 shrinks the boxes - smaller boxes pass the
    // well-separatedness test more often). The measurement wins over the
    // intuition that a tighter tau means more far pairs, because the
    // mechanism (smaller boxes separate more often) runs the other way.
    // Both rungs must stay far-ALIVE (count > 0) - a vacuous far field
    // would hide the multipole truncation.
    if (IsFastOnlyMode())
    {
        GTEST_SKIP() << "fast mode";
    }
    ...
    // The fixed theta isolates the extent effect from the theta ladder.
    // The committed presets are unusable here: ThetaForPreset(kNormal) = 0.3
    // is far-dead on C12 at every tau (the vacuous tooth) and the kLoose
    // winner 0.45 is far-alive only at tau 1e-6. The explicit 0.7 keeps all
    // three per-preset extents far-alive on C12 with the monotone measured
    // counts 829 > 490 > 282 (kLoose/kNormal/kTight, sweep evidence).
    options.theta = 0.7;

    // The extent ladder below belongs to the retired model. Pin it explicitly:
    // since the 2026-09-15 flip the default geometry is the
...
TEST(QfmmFockBuildTest, ChunkedFarFieldBuildMatchesTheSerialPinWithinBudget) {
    // The chunked-far-field gate: the chunked parallel far field
    // (maxParallelChunks = 2 - the M2L chunked
    ...
    // The far-alive legs of the sweep: C12 at the explicit theta = 0.7 and
    // theta = 0.45 configs (L = 5), C24 at the same two. The theta is passed
    // EXPLICITLY here; the preset argument names the budget band only, and
    // kNormal's own theta is 0.3, so these are not "the kNormal config".
    {
        auto molecule = MakeAlkaneSto3g(12);
        ASSERT_TRUE(molecule.has_value()) << molecule.error().message;
        auto basis = MakeAlkaneSto3gBasis();
...
TEST(QfmmFockBuildTest, DefaultOptionsBuildMatchesDirectWithinTolerance) {
    ...
    // screening, the certified fp32 lane, auto-parallel) must run and match
...
TEST(QfmmFockBuildTest, LeafDrivenNearFieldIsBitIdenticalToTheGatedSweep) {
    // The near-field enumeration gate: the
    ...
    auto runLeg = [](const char* name,
        ...
        // Both sides are pinned to the same geometry for this gate: the
        ...
        // (the two paths walk the same tree), so a shared pinned geometry still
        // tests it; the corrected default's own near-field behaviour is carried
        // by TheQ073DefaultIsTheCorrectedPathAndTheGateStaysEmpty, and the
        ...
        std::cout << "leaf-driven " << name << ": theta " << theta << " L " << lMult << " preset "
    ...
    // theta 0.7 and kLoose theta 0.45 at the per-preset extents), and the
    // degenerate theta -> 0 gate at kTight.
    {
        auto molecule = MakeAlkaneSto3g(12);
        ASSERT_TRUE(molecule.has_value()) << molecule.error().message;
        auto basis = MakeAlkaneSto3gBasis();
...
TEST(QfmmFockBuildTest, TheQ073DefaultIsTheCorrectedPathAndTheGateStaysEmpty) {
    // The corrected path is on by default, stated as a test. Two obligations
    // ride the flip:
    // the build must RUN the corrected model (product-distribution extents,
    // surface-ball separation) and SAY so (ModelRecord), and the degenerate
    // gate must still produce the same numbers it produced before the flip -
    // the far field empty, the build the exact restricted near-field direct
    // build, bit for bit. The second is what makes the new default safe to
    // trust: where the far field IS live the values move, and where it is
    // empty they must not.
    qcx::integrals::QfmmOptions defaults = MeasurementOptions();

    EXPECT_EQ(defaults.extentModel, qcx::integrals::QfmmExtentMode::kProductBall);
    EXPECT_EQ(defaults.separationMode, qcx::integrals::QfmmSeparationMode::kSurfaceBall);
    EXPECT_GT(defaults.separationK, 0.0);
    ...
    // The record says what ran: the default names the corrected model.
    ...
    EXPECT_EQ(record.separationMode, qcx::integrals::QfmmSeparationMode::kSurfaceBall);
    EXPECT_EQ(record.separationK, -1.0); // theta < 0 is the gate under either model.

    // The chain fixture where the corrected default's far field IS live: the
    // default and the gate differ (the far field carries real work - the
    // retired default carried none on this fixture), and the difference stays
    // inside the kNormal budget's order (measured 1.1e-8 Ha).
...
TEST(QfmmFockBuildTest, TheCorrectedDefaultStaysInsideThePresetBudgetsOnC12) {
    // The flip's accuracy contract, asserted as a PROPERTY rather than a
    // re-banded pin: with the corrected default - an
    // ABSENT theta and no model keyword, so product-ball extents and the
    // surface ball at k = 1.0 - each preset's composed J must stay inside its
    // committed budget (QfmmBudgetForPreset). The retired ladder pins are
    // NOT re-banded here; the measured corrected-default numbers are printed
    // for the record (C12, kNormal: 1.4e-9 relative Frobenius, 2.1e-8 Eh
    // J-energy against the recorded vacuous 0.0 of the retired model).
    if (IsFastOnlyMode())
    {
        GTEST_SKIP() << "fast mode";
    }
