// The ri_jk validation cells: validation and disclosure of the RI-K exchange.
// The contraction was measured separately (ri_occ_k); this file measures the
// builder THAT ACTUALLY RUNS - the composed builder (RiFullFockBuilder) - so
// the number a reader of a `ri_jk` energy record is entitled to is taken on the
// object the run path constructs:
//   - AccuracyAgainstOurOwnExactScreenedExchange: K from
//     the composed builder against the exact screened exchange, elementwise
...
//     target is already recorded as missed on this fixture's STO-3G shape.
//   - TheConvergedDensityMovesBetweenExactAndRiJkExchange:
//     the non-variationality of robust RI fitting. Robust RI fitting makes the
//     approximated exchange >= exact, so the composed SCF converges to a
...
// measurement (the -74.96292827 anchor and the recorded iteration counts), and
// extracting it into a shared header would move the instrument of measurements
// already taken. The three copies are held together by re-anchoring the same
// in-tree pin, which is the drift detector.
...
// recorded beside every number. It is the J+K fit the module requires, and it
// is NOT auto-selected here: a cell whose aux came from a default would be
// measuring a different approximation than the one it names (the module's
// construction rule, applied to the module-level cell as well).
constexpr std::string_view kAuxName = "def2-universal-jkfit";

// The target for the exchange error against exact K: 0.1 kcal/mol
// = 1.59e-4 Eh. A TARGET, recorded as a comparison - never the pass line.
constexpr double kPlanExchangeTarget = 1.59e-4;

// kcal/mol per Hartree (CODATA-consistent).
...
// 2.6e-4 Eh respectively (the recorded values), so the K fit, not the contraction, is
// the error source these ceilings leave room for.
...
// is recorded at 2.6e-4 Eh on the 7x7 STO-3G block (with
// ri_engine_test.cpp:414-418), and the K/Coulomb elementwise ratio is measured
// at 1.18x (STO-3G) and 29.8x (def2-SVP), so the expected elementwise K error
// sits at the 1e-4-1e-3 class. This bound is 5e-3: it refuses a sign, a factor
...
// The three presets the acceptance is measured at, in preset order (loose,
// normal, tight). The acceptance's bar for each is read from
...
// counts below are comparable with the ones it recorded.
struct RhfOutcome {
...
// nested call carried (ri_engine.cpp's former composition).
...
struct AccuracyFixture {
    ...
    /// The preset BOTH sides run at (our own exact screened exchange, at the
    /// same preset). Carried per fixture so the per-preset acceptance below is
    /// a measurement and not a re-derivation.
...
void MeasureAccuracyCell(const AccuracyFixture& fixture) {
    ...
    // which returns F = H - K with the same density (the same reference,
    // and the nested call the composed path removed).
    ...
    // The acceptance: the PER-PRESET, PER-ATOM bar from the
    // preset mapping's own table (integrals accuracy.hpp
    ...
    // comparison, never as the pass line (the error class, not the preset
    // budget), and the same convention this file already uses for the
    ...
    std::cout << "[ri_jk] " << label << " K accuracy vs our own exact screened exchange (aux "
              ...
              << " kcal/mol = " << perAtomKcal << " kcal/mol/atom; target "
...
TEST(RiJkValidationTest, AccuracyAgainstOurOwnExactScreenedExchange) {
    ...
    // The small fixture, where the module's own RI cells already run
    // (H2O/STO-3G + the J+K fit).
    ...
    // The second shape: the fixture whose recorded aux K-fit ratio is the
    // worst one (5.82x the RI-J class).
...
TEST(RiJkValidationTest, ThePresetMappingBarsTheScreenOnlyAndKeepsTheJkFit) {
    ...
    // every rung (the mapper's predicate refuses a J-only fit outright). If a
    // consumer ever needs this to vary by preset, that is a change to the
    // path's approximation contract, not its screening.
    for (const qcx::integrals::AccuracyPreset preset : kPresets)
    {
    ...
    // The per-atom budget is the fixed table carried in Eh: 0.5 / 0.02 /
    ...
    // The Eh literals the mapper carries are those kcal/mol values to
    ...
    // mapper carries those three bars and not three other numbers.
