// The bundled aux-basis selection table, re-based on the tiered aux-selection
// rule (2026-09-13) and exercised exhaustively against the real
// data/basis directory names (the table below mirrors the 2026-09-13 listing;
// a renamed or removed aux set breaks the existence assertions instead of
// silently decaying).
//
// TWO TESTS IN THIS FILE WERE CHANGED BY THE TIERED RULING, not deleted:
...
// is exactly the "silently using a J-fit for K" substitution the tiered rule
// names as the real risk. Its replacement pins the opposite, and
// `NoSelectionEverHandsAJOnlyFitToAJkRequest` sweeps every bundled name to
// hold that line.
//
// A SECOND contract changed on 2026-09-13, when the
// corpus gained the sets the tiered rule had no member for. Two expectations in
// this file moved with it, and both were RE-POINTED rather than dropped:
...
// Every bundled orbital basis directory, as of the 2026-09-13 listing. The
...
TEST(AuxBasisSelectionTest, CcFamilyAppendsRifitOnlyForAJPureRequest) {
    for (const auto& [orbital, expected] : kCcOrbitalBases)
    {
        ...
        // because the fixed string is a corpus fact: it changed on 2026-09-13
        // when the cc family gained a JK fit, and the property did not.
...
// The 2026-09-13 corpus addition gave the cc family its JK fit; before it,
...
TEST(AuxBasisSelectionTest, CcFamilyJkRequestResolvesToTheVendoredDunningRung) {
    ...
    // the tiered rule's "cardinal matching is preferred, never required" does
    // not cover.
...
// The diffuse arm, both halves of it (2026-09-13). Before this, every diffuse
// def2 basis took the NON-diffuse def2-universal-jfit - the second half of the
// tiered rule's own named hazard, "a non-diffuse aux for a diffuse orbital
...
// THE HAZARD PIN (the rule's own named risk, first half). Sweeping
...
// The owner's standing ruling: "if for some calculations we have no default
...
TEST(AuxSelectionNoticeTest, NoticeNamesTheWeakRegionForEachListedFamily) {
    ...
    //
    // RE-POINTED 2026-09-13, not dropped. The case used here was "cc-pvdz",
