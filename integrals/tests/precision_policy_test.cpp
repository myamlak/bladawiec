// The precision-ladder policy tests: the
// budget arithmetic with hand-computed budgets on a fixed fixture, the
// band classification and its profile pricing, the
...
TEST(PrecisionPolicyTest, PresetEnergyBudgetsMatchTheLadder) {
...
TEST(PrecisionPolicyTest, EriBudgetHandComputed) {
    // The budget arithmetic: hand-computed budgets on a fixed fixture.
