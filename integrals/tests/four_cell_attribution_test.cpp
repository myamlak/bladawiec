...
// density screening, separated (2026-09-11).
//
...
// either mechanism from that pair alone. The two-toggle matrix below is the
// form that separates them. Both toggles are FockBuildOptions fields with
...
//
// THE SCREENING TOGGLE CARRIES THE WHOLE SCREENING CHAIN. The flag's own contract
// pairs the quartet-level product gate with the kernel per-element
// re-filter ("there is no product-gate-with-filter-disabled
// mode"), so the screening-off cells clear usePerElementScreening too -
...
// pair was retired under the owner's 2026-09-18 ruling (no convergence gate
// at or below 1e-10). They are constants of THIS FILE's own loop: no
...
// -5.10e-9 to -6.06e-9 with the same tightening, so on that path the
// differential is stopping-sensitive at the
...
/// One cell of the two-toggle matrix: the two FockBuildOptions fields the
/// matrix toggles, every other field at its default (accuracy
...
/// the kernel per-element re-filter still on - the split of the screening chain
/// the four-cell matrix deliberately keeps together.
...
Eigen::VectorXd DiisCoefficients(const std::vector<Eigen::VectorXd>& errors) {
    ...
    Eigen::VectorXd coefficients = a.completeOrthogonalDecomposition().solve(rhs);
...
qcx::Result<WalkOutcome> WalkRhf(const qcx::integrals::DirectJkFockBuilder& builder,
                                 ...
                                 // The two one-electron matrices the fixture
                                 // holds, read by name at every use below.
                                 //
                                 // NOLINTNEXTLINE(bugprone-easily-swappable-parameters)
                                 const Eigen::MatrixXd& coreH,
                                 const Eigen::MatrixXd& overlap,
                                 // The occupied count and the two thresholds are
                                 // documented above and named by every caller;
                                 // the count is never read as a tolerance.
                                 //
                                 // NOLINTNEXTLINE(bugprone-easily-swappable-parameters)
                                 std::size_t occupiedCount,
    ...
    for (std::size_t iteration = 0; iteration < kMaxWalkIterations; ++iteration)
    {
        ...
        // against itself until a614d44a (2026-09-12) and so stopped nothing.
...
TEST(FourCellAttributionTest, ConvergedEnergiesAttributeTheDivergence) {
    ...
    // OPERATING thresholds as the control the 1-vs-4 attribution
    // rests on.
    auto fixtureResult = MakeFixture();
    ...
    // difference. That is the pre-a614d44a driver exactly - its inert 1e-8
    ...
    // from the fixed point" - is REFUTED: the walk contracts
    // geometrically at rho ~ 0.2 per iteration with no stall, right through
