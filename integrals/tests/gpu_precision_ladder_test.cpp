// The GPU-gated precision-ladder tests (Package B step-6 gate and the
...
TEST(GpuPrecisionLadderTest, Fp64CapMatchesPlainFp64Build) {
    // The step-6 gate (GPU half): the fp64 cap routes every batch to the
...
TEST(GpuPrecisionLadderTest, LadderRunsOnDeviceWithTensorCores) {
    ...
    // device probe is public, Package B step 1), and a ladder build with
