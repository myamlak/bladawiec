// The GPU mode-selection gate: the per-device mode selection -
// the FastPath/LightPath decision generalized to the device context.
...
// the state-free slice of the device-footprint gate (peak minus create), so the
// suite's prior pool state does not affect the assertions. (5) The refusal
// before OOM: an artificially tiny device budget refuses at Create
...
/// quantum before any measured sequence (the device-budget gate's settle
/// cycle: the
...
/// The light rung's per-call estimate: the per-call
/// working set - the matrices, the batch scratch and the per-call output.
...
TEST(GpuModeSelectionTest, TinyBudgetRefusesWithKInvalidArgument) {
    // The refusal gate: an artificially tiny device
    // budget - below the light rung's per-call working set - refuses at
    // Create with kInvalidArgument and the device context named, before the
    // first device allocation of the Create (the statics upload and
    // cublasCreate): no CUDA allocation can fail for a budget the caller
    // declared.
    auto molecule = MakeH2oSto3g();
    ASSERT_TRUE(molecule.has_value());
    auto basisSet = MakeH2oSto3gBasis();
...
TEST(GpuModeSelectionTest, LightPathCommitsAndReleasesTheStaticsPerCall) {
    ...
    // the device-budget gate: the suite's prior pool state does not affect it.
    ...
    // device-budget gate's per-call law, re-asserted here).
    ...
    // (the state-free distinguishing slice of the mode-selection gate).
...
TEST(GpuModeSelectionTest, PerCallTrafficIsDensityFockAndScreenedListsOnly) {
    // The residency gate (the residency contract):
    ...
    // 1e-10, the certified gate): the far-apart terminal pairs of the C12 chain
