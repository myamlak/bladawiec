...
// magnitudes the screening and certified-lane gates expect.
...
TEST(FockBuildTest, DensityScreeningDropsNegligibleQuartets) {
    ...
    // 1e-10, and the per-target product gate's values are smaller still (the
    // J-mode product m^2 * Q * Q ~ 1e-22 * Q^2, the K-mode cross products
    // likewise; the product gate is provably tighter at m < 1, the
...
TEST(FockBuildTest, DensityScreeningKeepsThePhysicalPairs) {
    ...
    // the re-derivation above holds both gates to the same keep partition on
    // this fixture.
    if (IsFastOnlyMode())
    {
        GTEST_SKIP() << "fast mode";
...
TEST(FockBuildTest, CertifiedFp32LaneBoundDominates) {
    ...
    // density-weighted quartet bounds exactly (the certified-bound sweep proves the
...
TEST(DirectJkFockBuilderTest, BothSplitFlagsReturnBareH) {
    ...
    // the full stack on (kLoose: the density screen AND the certified fp32 lane
    // both active) so the contract holds at its strongest - the fp32 lane
...
TEST(DirectJkFockBuilderTest, SplitModeAtLoosePresetStaysInsideCertifiedBound) {
    // The fp32-lane interplay the accuracy-preset sweep records but never asserts: the
    // J-only split mode at kLoose (the density screen AND the certified fp32 lane both
    // active) must stay inside its certified bound sum - nothing approximate
    // unless the bound covers it - plus a physicality budget.
    if (IsFastOnlyMode())
    {
        GTEST_SKIP() << "fast mode";
    }
    ...
    // lane's default is the device's now (the device probe's verdict), and a
    // test that pins what the lane delivers must not ride a default the
    // host decides.
    ...
    EXPECT_LE(maxAbsDJ, 1e-5)
        << "physicality pin (the kLoose record on this fixture family is 1e-6-scale)";
}
...
TEST(DirectJkFockBuilderTest, ParallelChunkScheduleIsOrderIndependent) {
    ...
    // Both builds use the DEFAULT preset (kNormal - screening and the certified fp32
    // lane on): the serial-vs-parallel agreement must hold on the full
    // delivery path, not on a lane-off special case. maxParallelChunks
...
TEST(DirectJkFockBuilderTest, WaterC2vClassPathMixedPrecisionMatchesPlain) {
    ...
    // The lane is requested explicitly: its default is the device's now
    // (the device probe's verdict), and this pin's subject is the lane's
    // arithmetic on both the plain and the class path.
    ...
    // bound sum (the composition of the per-quartet certified bounds), so the
...
TEST(DirectJkFockBuilderTest, TrivialReductionKeepsPlainPath) {
    ...
    // to be unspecified (observed ~1e-14 element-wise on
...
// sit unnoticed on any machine whose CPU takes the AVX2 path).
...
// last ulp elementwise (the gcc Release leg:
// H2o's memcmp returned 1, SplitModes' exchange-only leg 2). The WSL gcc
// 15.2 reproduce at OMP_NUM_THREADS=1 measured the H2o max element
// divergence at 1.11e-16 (one ulp at 1.0); the CI byte evidence bounds the
// divergence at a few ulps of the largest element (~20 a.u. -> ~9e-15), and
// the band is that bound times ~3.4 (that geometry; the local actuals
// are recorded with the gcc Release leg). The comparison stays serial-only
...
TEST(DirectJkFockBuilderTest, PerElementFilterKeepsCertifiedBoundDominant) {
    ...
    // builds are bit-identical - the filter composes with the certified-lane routing
    // at every preset.
...
TEST(DirectJkFockBuilderTest, SplitModeStaysInsideCertifiedBoundWithFilterOn) {
    ...
    // Explicit, as in the sibling pin above: the lane's default is the
    // device's and this pin is about what the lane delivers.
    options.useCertifiedMixedPrecision = true;
    auto builder = qcx::integrals::DirectJkFockBuilder::Create(*molecule, *basis, *core, options);
    ASSERT_TRUE(builder.has_value()) << builder.error().message;
    double certifiedBoundSum = 0.0;
...
// but CI runners expose ONE physical core per machine (ubuntu-24.04 GitHub
// runners; empirically recorded on the clang Debug leg), and a one-core host
// caches team = 1 on the
// first team-size read. Team 1 collapses the k-factor formula
// k = min(max(1, floor(remaining / cap)), team) to k = 1 always, which
// neither exercises the k > 1 merge path nor reproduces the authored
// geometry - the gcc Release leg failed exactly
// there. Seeding OMP_NUM_THREADS to four before the first team-size read
...
// Fix A (the bounded-concurrency batch loop): the k = 1 vs
...
TEST(DirectJkFockBuilderTest, BatchLoopBitIdenticalAcrossConcurrentSlots) {
    ...
    // size deterministic on any host: a one-physical-core CI runner caches
...
// The nested-exchange lift's pin helpers: the process-wide team
...
// deterministic on any host: a one-physical-core CI runner caches team = 1
...
// The nested-exchange lift: the k-selection pin on the nested
...
// 1024-B cap unclamped, so the fired slot count is 4 - the earlier nested
// run forced k = 1 even with this slack. The
// bitwise pins repeat that argument at the nested seam: same cap, same
...
TEST(DirectJkFockBuilderTest, NestedExchangeHalfFiresBudgetConditionalSlots) {
    ...
    // The capture carries the observed-concurrency marker too: the stats out is
    // observational, the Fock and the
...
// The nested-exchange lift's k = 1 floor at the nested seam. A one-thread team cannot
...
// The nested-exchange lift's self-gating when the saturated k = team charge
...
// starts-tables rows (8 B per start row per half, batch-count
...
TEST(DirectJkFockBuilderTest, NestedExchangeHalfClampsItsOwnCapWithinItsBand) {
    ...
    // cap by ceil(8192 / 16) = 512. The starts-tables rows (that
...
// ---------------------------------------------------------------------------
// The class-table gate lane (2026-09-13): the LIVE class-table measurement.
// ---------------------------------------------------------------------------
//
...
// class_table allocation family's live request bytes, which every container of the
...
// shells, 45,150 canonical pairs, 420 functions), the scale the class-table record
