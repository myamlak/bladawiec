...
// The shared harness of the QFMM acceptance tests: the one-electron core
// Hamiltonian, the physical density, the QFMM build (octree near field -
// the direct builder
...
// measurements isolate the multipole truncation from the certified lane's
// error.
inline qcx::Result<Eigen::MatrixXd> BuildQfmmFock(const qcx::molecule::Molecule& molecule,
    ...
    // extent ladder (QfmmExtentForPreset = the skeleton's committed
    // kQfmmExtentThreshold): the fixture gates are the tight-rung
    // measurement config (the harness adopts the same call as the builder so
    // gates stay comparable; the builder itself drives the extent from the
    // preset).
    ...
    // node at \p lMult - the fixed-order order-vector form:
