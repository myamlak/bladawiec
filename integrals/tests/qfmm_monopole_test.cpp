// The L_mult = 0 QFMM J correctness gate: the J assembled from the octree
...
// (M2M/M2L/L2L at L_mult = 0 - the degenerate case of the general-L_mult
// machinery), must
//   1. recover the plain direct-sum Coulomb matrix BIT-IDENTICALLY at
//      theta -> 0 (everything near field) - the single most important
//      test here; a failure here means a bug in the interaction-
...
//      (kernel-level thetas 1.05 -> 0.85 -> 0.7 on the C12 alkane,
//      the acceptance fixture; the extent-padded boxes keep the far
//      field alive only at these theta values on a 20-40 atom chain;
//      these are raw kernel inputs, not the production preset ladder,
//      which was recalibrated to {0.45, 0.3, 0.0} x {5, 5, 0}),
//   3. stay within a loose sanity bound - monopole order is genuinely
//      coarse, and the point here is the MACHINERY, not accuracy.
...
TEST(QfmmMonopoleTest, MonopoleTableMatchesTheOverlapMatrix) {
    // The free correctness check: the monopole moment of a function
...
TEST(QfmmMonopoleTest, ThetaZeroRecoversTheDirectCoulombMatrix) {
    // THE gate here: at theta -> 0 everything is
...
TEST(QfmmMonopoleTest, FarFieldErrorDecreasesMonotonicallyWithTighterTheta) {
    // The acceptance property: on a spatially-extended molecule the QFMM
    // error (relative Frobenius norm vs the direct ground truth) must
    // decrease monotonically as theta decreases. The alkane chain C12H26
    // (38 atoms, 86 functions, 62 shells, 1953 shell pairs) is the
    // acceptance fixture. The near-field parts of the QFMM and the direct
    // build make the SAME screening decisions (same options, same density),
    // so the difference is purely the far-field multipole truncation -
    // exactly what the monotonicity measures.
    //
    // The ladder: with the extent-padded boxes (the box must contain the
    // clouds for the multipole test to be honest), every leaf holds a carbon
    // pair or a C-H pair, so the well-separated threshold is (wA + wB)/theta
    // with w ~ 7 Bohr, and theta <= 0.5 is far-dead on any chain within the
    // 20-40 atom fixture range (the illustrative 0.7 -> 0.5 -> 0.3 values
    // were written for raw octant boxes). The sweep thetas {1.05, 0.85, 0.7}
    // keep the far field alive and strictly shrinking at every rung -
    // kernel-level inputs, NOT the production preset ladder (the production
    // rungs were recalibrated to {0.45, 0.3, 0.0} x {5, 5, 0} at the
    // per-preset taus); 0.7 was the tightest rung of the earlier preset
    // calibration.
    if (IsFastOnlyMode())
    {
    ...
    // monopole order is genuinely coarse and the error must NOT be
    ...
    // (a sign or double-count bug). The real accuracy gate is the general
