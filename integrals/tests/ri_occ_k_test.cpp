// The occ-RI-K contraction core tests (ri_occ_k.hpp): the metric-transformed
// tensor, the occupied-orbital exchange transform and the batched
// auxiliary-index build.
//
...
//   - CoulombFromTransformedTensorMatchesShippedEigenPath: the acceptance cell
//     - J through the metric-transformed tensor against the
...
//     error class on the same fixture and aux, and the two legs the RI-K
//     acceptance is judged on (the error class, and the SCF iteration count).
//     The target of 0.1 kcal/mol (1.59e-4 Eh) is recorded; the pass rule is
//     that the RI-K exchange error is not worse than the RI-J error class.
...
//     1.5e-5 Eh) - so the error-class leg does NOT pass, the iteration-count
//     leg does pass, the target is MET on the acceptance fixture and MISSED on
//     H2O/STO-3G, and the stop is the outcome. The cell carries the measured
//     RATIO (a ceiling that a worse ratio fails and a better one passes); the
//     stop itself is recorded, not asserted.
...
//     they are single-run, contended, and are this measurement's input to
//     whether the batching is worth its wiring; the >= 20% paired-interleaved
//     600-BF claim is not made here.
//
...
// shows it. No assertion encodes an unruled threshold: the ones
// that exist are the exact identities' round-off class, the two acceptance
...
// The exchange-error target: 0.1 kcal/mol against exact K, in Eh
// (1 kcal/mol = 1.59360e-3 Eh). A TARGET, not the pass line - the pass rule is
// that the RI-K error is not worse than the RI-J error class.
constexpr double kPlanExchangeTarget = 1.59e-4;

// The round-off class for the J-from-B identity, read as RELATIVE to the
// compared magnitude - the reading ri_engine_test.cpp already uses for the
// same figure, where its comment says "(1e-12 absolute on elements of
// magnitude ~10 - 1e-12 relative - leaves a ~100x margin over the ~1e-14
// rounding spread)". The
...
// The SCF fixture's convergence threshold: 1e-8 on the energy, never tighter -
// both sides of the iteration-count comparison run the same threshold.
...
// ri_occ_k.hpp, and the Coulomb contraction shape the ri_j_link path composes
// with.
...
// MEASUREMENT INSTRUMENT for the acceptance's second leg ("the SCF iteration
// count is essentially unchanged") and for the converged density and orbital
// block the accuracy cells are taken at. It is not a production SCF - the scf
// module owns that and the module DAG puts it out of this test's reach - and it
...
// The acceptance identity: J through the metric-transformed tensor
...
// One fixture of the acceptance measurement: both legs measured on the same
...
struct ExchangeCellFixture {
    ...
    /// The characterization ceilings: the measured RI-K/RI-J error ratio
...
void MeasureExchangeCell(const ExchangeCellFixture& fixture) {
    ...
    // measured error is the FLOOR-SENSITIVITY datum (it informs the
    // metric-floor threshold work; no assertion rides on it).
    auto looseFloor =
    ...
    // The exact-K side: the direct builder at the same preset (the reference
    // the acceptance measurement is taken against), one fused build per SCF
    // iteration.
    ...
    // ri_j_link composes, and the identity test above pins J_B to the
    // shipped J).
    const auto riFock =
    ...
    // The shipped RI-J path on the same fixture and aux (the class the RI-K
    // error is compared against), and the exact J from the dense kernel.
    auto shippedFock = shipped->BuildFock(*densityTensor);
    ASSERT_TRUE(shippedFock.has_value()) << shippedFock.error().message;
    ...
    // at ri_engine_test.cpp) and the energy class (the same figure in Eh).
    ...
    // The aux K-fit error itself, in the exchange energy.
    ...
    // The density cell (energy agreement alone is not accepted as evidence -
    // recorded here).
    ...
    // The error-ratio CHARACTERIZATION PIN, never the verdict. The pass rule:
    // "the RI-K exchange error is not worse than the RI-J error class measured
    // on the same fixture". Measured on this fixture
    ...
    // H2O/STO-3G - so the error-class leg does NOT pass and the measured stop
    // is the outcome. What the suite carries is the measured
    ...
    // 1.0 means the stop no longer holds and the verdict
    ...
    // The iteration-count leg: the SCF iteration count is essentially
    // unchanged. One iteration of slack is the reading of "essentially" and
    // it is stated, not assumed; both counts are recorded above. This leg
    // PASSES on both fixtures.
    ...
    EXPECT_LE(iterationMove, 1) << label << ": the SCF iteration count moved by "
                                << iterationMove;
...
TEST(RiOccKTest, DirectScfMatchesTheInTreePins) {
    ...
    // (pyscf's gto.M defaults to Angstrom, so unit='Bohr' is required),
    // conv_tol 1e-10, measured 2026-09-12:
...
TEST(RiOccKTest, OccTransformMatchesTheAoBasisExchangeContraction) {
    ...
    // as well as from RiMatrix() - the two routes callers feed
    // them from.
...
// The DECISION measurement: the chain against the batched
...
// and contended - the machine runs other work too - and are
// recorded as this measurement's input to whether the batching is worth wiring,
// not as a claim: the >= 20% paired-interleaved 600-BF
// comparison against the ri_j_link row is neither run nor stood in for here.
// NO ASSERTION here is about a time.
