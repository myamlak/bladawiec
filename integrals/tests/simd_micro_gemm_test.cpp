...
// CPUID is x86-64 only: <cpuid.h> is a compile error outside x86 and <intrin.h>
// carries the intrinsic family that header belongs to. The two tests below that
// read it are excluded by the same architecture test the SIMD tier itself uses,
// so the exclusion moves with the tier rather than with whichever compiler
// happens to build this file.
#if QcxArchX86_64
#ifdef _MSC_VER
#include <intrin.h>
#else
...
// scalar copy to differ from (MicroGemmAddSimd/MicroGemmSimd are the portable
// kernels themselves), so this test's assertion that the difference is real
// would fail by construction on the one path it is meant to measure.
#if QcxArchX86_64
...
// tools/bench/probes/zeroprobe.cpp).
