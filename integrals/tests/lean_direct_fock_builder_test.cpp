// LeanDirectFockBuilder tests (lean_fock_build.hpp - the 2026-09-08
// lean-direct reframe):
...
//     -74.96292827 (the direct-family anchor, +-1e-5), byte-identical across two
//     independent walks,
...
//   - the prep probes' new fields: the prep split (pair data / assembly /
//     tile setup as disjoint spans inside eriPrepWallTime), the pair-build
//     count, and the per-window record (one entry per window, each inside
//     the call's span).
//   - the kernel-span probe (md_kernel_span.hpp): the three sub-spans
//     of the kernel time (VRR / ket transform / bra transform) and their
...
//     printing the plan and the envelope for an external peak-working-set
//     sampler.
//
...
TEST(LeanDirectFockBuilderTest, ReusedPairArenaPinsItsOwnBytesAboveTheFlushCap) {
    ...
    // The retention pin the earlier slice named as its follow-up. Every other
    ...
    // (The in-place re-fit is no longer exercised HERE for the
    ...
    // (the pair-rebuild instrument): pairBuildCount is a BUILD count, so
...
TEST(LeanDirectFockBuilderTest, StatsOutFillsTheSurvivorCountAndTheWallTime) {
    ...
    // The lean-backed phase accounting: the call
...
TEST(LeanDirectFockBuilderTest, ValidationAndLastResortCeilingRefusal) {
    // The Create-time validation and the last-resort refusal shape (Ruling
    // A): refuse only when the simple-actuals estimate exceeds the ceiling
...
TEST(LeanDirectFockBuilderTest, PreRampEnvelopeMatchesTheBuildersOwnNumber) {
    ...
    // EstimatedPeakBytes for the same options (the plan included), and its
    ...
    // plan: the plan the envelope rides must be the plan a call runs.
    ...
    for (const qcx::integrals::LeanFockBuildOptions& options : plans)
    {
        ...
        EXPECT_DOUBLE_EQ(refusedEnvelope->totalBytes, builder->EstimatedPeakBytes());
