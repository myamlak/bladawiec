// The GPU Fock builder tests - the GpuJkFockBuilder (gpu_fock_build.hpp)
// against the DirectJkFockBuilder parity gate - the same screened list, the
// same contraction formula, the same certified-bound bookkeeping - and the
// RHF pins wired through RunRhfScf with the GPU builder as the Fock source.
...
void H2ParityAgainstCpuWith(qcx::integrals::FockBuildOptions options,
    ...
    // The GPU parity pins run the per-element screening flag OFF (the flag
    // contract): the device kernel predates the per-element re-filter,
...
TEST(GpuFockBuildTest, H2ParityAgainstCpu) {
    ...
    // kNormal keeps the H2 quartets above the certified-bound gate: both
    // lanes are all-fp64 here, both certified sums are zero (the H2 pin
    // comment).
...
// Tensor factory - never through qcx::testing::ToTensor (that helper's
// tensor builder is square-only).
...
// re-filter), so the parity pins run the flag off on BOTH sides; this test
// instead runs the GPU on the legacy screen and the CPU on the full two-level
// screen (product gate + per-element filter) and bounds the deviation: each
// element the CPU drops carries at most 2*tau to a touched Fock element (the
// recorded bound), and each quartet the CPU's product gate drops that the GPU
// contracted has its elements bounded by the six-block keep condition times
// the block-max ratios of the physical density (~2-3 on this fixture - no
// near-zero blocks) - so tau * (total canonical quartet elements) is a
// conservative envelope for the whole deviation at kLoose. The certified lane
// is off (fp64-only both sides) so the residual is exactly the screen
// difference, not the lane's delivered error. The GPU side runs 3 times (the
...
TEST(GpuFockBuildTest, GpuNoFilterVsCpuFilterBounded) {
    ...
    for (int run = 0; run < 3; ++run)
    {
        ...
        EXPECT_LE((gpuMatrix - cpuMatrix).cwiseAbs().maxCoeff(), budget)
            << "GPU (legacy screen) deviates from the CPU (two-level screen) beyond tau*"
               "totalElements (tau "
...
// The device-path sweep: the certified bound walk of certified_bound_sweep_test.cpp
// over the device pipeline - the same canonical quartet list through
// EriCudaEngine::ComputeBatch (fp64) and ComputeBatchCertified (fp32),
// asserting the a-priori bound dominates the measured deviation, and the
// GPU-fp32-vs-CPU-fp64 cross-check (catches a device-specific systematic
// bias the GPU-vs-CPU fp64 parity alone would not surface). The CPU lane
// here is the host reference for the fp32 lane.
