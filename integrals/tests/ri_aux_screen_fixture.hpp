...
// recorded iteration counts), and extracting it into a shared header would move the
// instrument of measurements already taken. The copies are held together by
...
/// The screen's acceptance bar: 0.01 kcal/mol = 1.59e-5 Eh,
/// screened against unscreened RI-K.
inline constexpr double kPlanScreenTolerance = 1.59e-5;
...
/// (8.515e-5 Eh on water/def2-SVP and 3.524e-4 Eh on H2O/STO-3G - the recorded
/// accuracy cells, aux def2-universal-jkfit). A screen whose own truncation
/// reached that class would no longer be sub-dominant to the approximation it
/// screens, which is the reason for the bound (a non-variational truncation does not
/// shrink with the RI error). A CLASS bound with its provenance, not a pin.
...
/// tensor through the dense entry points (ri_occ_k.hpp) - the shape the composed
/// builder ran before the screen. It is the control every screened number
/// is taken against: same B, same density, same occupied block, one build
/// without the screen.
...
/// through the dense entry points, one build with no screen.
...
struct BudgetOutcome {
    ...
    /// for "does the screen pay", never the screen's acceptance).
...
inline qcx::Result<BudgetOutcome> MeasureBudgetCell(const qcx::molecule::Molecule& molecule,
    ...
    // The two SCF runs are timed, and the timing is a DECISION INPUT, not the
    // screen's acceptance: it is a single run on a machine with other work and
    // compilers live, so it is labelled as such wherever it is quoted, and only
    // a paired-interleaved protocol may turn it into a claim. The two sides take
    // the same inputs from the same builder - one tensor, one density, one
    // instrument, the same SCF threshold - so a difference between them is the
    // screen's, not a workload difference.
