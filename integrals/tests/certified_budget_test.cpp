// The certified fp32 lane's global budget enforcement (user ruling
// 2026-09-11): the lane's accumulated bound is COMPARED
// against a preset-derived budget and the comparison DECIDES the routing.
//
// The finding was that the production path computed a bound and
// discarded it - "a bound-routed heuristic with an unused diagnostic". The
...
TEST(CertifiedBudgetTest, TheFallBackIsTheLaneDisabledBuild) {
    ...
    EXPECT_DOUBLE_EQ(floor, 0.0) << "the direct build's reduction order is not deterministic again";
