...
TEST(EriBatchTest, ShellsetMatchesMpmath) {
    ...
    for (const qcx::integrals::ShellQuartet& quartet : batch->computed)
    {
        ...
        for (const GridRow& row : grid)
        {
            ...
            // canonical KET pair count (an earlier version used
...
TEST(EriBatchTest, IChainClass66ProducesFiniteValues) {
    ...
    // the class-(6,6) benchmark crash exposed. No reference
    ...
    const auto runGrid = [](const qcx::molecule::Molecule& molecule,
        ...
        for (const qcx::integrals::ShellQuartet& quartet : batch->computed)
        {
            ...
            for (const GridRow& row : grid)
            {
                ...
                // ket-pair count) - an earlier version used
...
TEST(EriBatchTest, BatchRunIsBitIdenticalAcrossTeamSizes) {
    if (qcx::testing::IsFastOnlyMode())
    {
        GTEST_SKIP() << "parallel-schedule pin - skipped in the fast smoke subset";
    }
    ...
    // The full canonical quartet set of the fixture (the sweep's
    ...
    // and the per-quartet certified bounds must all be bit-identical across
    // the two team sizes.
