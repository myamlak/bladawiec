// Shell-pair geometry of the QFMM skeleton: hand-computed checks of the
...
// primitive-center offset, maxed over primitive pairs). Five-line arithmetic
// checks - the gate before anything built on top of ComputeExtent is trusted.
...
TEST(QfmmGeometryTest, ExtentThresholdConstantIsPinned) {
    ...
    // per-preset extent ladder (QfmmExtentForPreset, accuracy.hpp): the
    // skeleton's committed value survives as the tight preset's footprint bound.
...
TEST(QfmmGeometryTest, ExtentLadderIsMonotoneInThePreset) {
    // The per-preset extent ladder {1e-6, 1e-8, 1e-10} for {kLoose, kNormal,
    // kTight} - the tight preset keeps the loosest (largest) footprint bound,
    // so the extent is monotone non-decreasing kLoose -> kTight on any real
    // pair (the tail-charge coupling: a looser preset admits a larger
    // missed-charge tail at its own coarser budget and shrinks the boxes).
