...
// rejection this fixture is meant to reach - the 2026-08-22 Lmax=2 fix).
...
// The mode-selection band orbital basis (the footprint_test fixture, inlined here):
...
// (2 x tensor + riMatrix - slice), which a literal s-only
...
// The mode-selection firing-rule band aux (the footprint_test fixture,
...
// The metric-floor sweep fixture: a near-degenerate (P|Q) with
...
// sweep points (a 1e-7 floor point measures 2.16e-16 - the delta^2
...
// inter-monomer overlap at that separation) - the mode-selection band fixture (the
...
TEST(RiEngineTest, CudaGemmMatchesCpuGemm) {
    ...
    // absolute on elements of magnitude ~10 - the 1e-12 relative -
...
TEST(RiEngineTest, NearDegenerateMetricEpsilonSweep) {
    ...
    for (const double epsilon : {0.0, kEpsilonLow, 1e-10, 1e-8, kEpsilonHigh})
    {
        ...
        if (epsilon == 0.0)
        {
            // The pinned epsilon == 0 contract (2026-08-27): the
...
TEST(RiEngineTest, LightRungMatchesMaterializedWithinThePresetBudget) {
    ...
    // not bit-identical by design, validated within the accuracy preset's
    // budget.
...
TEST(RiEngineTest, FirstIterationSelectsTheLightRung) {
    ...
    // (the clamp-IRREDUCIBLE floor check drafted for this zone proved
    ...
    // computes it (the task-count charge fix): the footprint's taskListBytes
    ...
    // The footprint folds the exchange per-call term into Total() too -
    // subtract it with the first-iteration terms so the "old" total stays
    // today's true pre-fix estimate (the budget's razor's edge keeps its
    // exact pre-change margins). The n34b screened-quartet charge does NOT
    ...
    //
    // The honest screened-quartet charge (the footprint_test
    // mirror's own amendment): the whole-call charge drops by
    ...
    // tensor and reserved the folded total. Since the nested slot
    // authorization landed, the nested exchange fires k = team into its
    // post-reservation band - the
...
TEST(RiEngineTest, BlockedMetricRungMatchesTheLightRung) {
    ...
    // screening and the certified fp32 lane are preset-consistent on both
    // sides):
    ...
    // fp32 lane off); 1e-11 leaves ~1e3 headroom over the measured last-ulp
    // noise
