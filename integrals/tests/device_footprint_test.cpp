// Device-footprint formula tests (device_footprint.hpp - the GPU device-
// footprint seam): the per-call output envelope against hand arithmetic (the
// gate re-pinned n^-1 screened-exchange law of device_footprint.hpp, floored
// at the scaling-audit fractions of footprint.hpp), and the
// GpuDeviceFootprint record -
// the pass-through of the caller's exact table bytes, the per-call
// matrices, the kV2-only batch arena and cuBLAS workspace, and the
// driver-pool allowance that is always charged (the per-Create module loads
// are fixture-independent). The VRAM-vs-estimate gate itself is the CUDA
// test (gpu_device_budget_test.cpp in qcx-cuda-md-tests); this suite pins
