// The certified fp32 lane's DEVICE-GOVERNED DEFAULT: fp32 "may be far more
// useful on GPUs and AVX machines", so the lane's default is HARDWARE-AWARE,
// never a blanket switch. The probe's verdict is precision_policy.hpp
...
// gate still carries them, not only a failing one - including the real
// device probe's own measurement, which
...
// travels with the assertion: the local Quadro T1000's own measurement
// (2416 / 77.5 GFLOP/s), the design's datacenter reference class, and the
...
// an invented number. Two measured anchors bracket it -
...
// the lane. This is the "no probe applies" case.
...
// The OFF half, end to end: a kLoose build whose unset request
...
// The ON half, end to end: the same kLoose build whose unset
...
// applies -> off" is itself a case the rule names.
