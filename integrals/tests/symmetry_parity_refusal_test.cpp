// The symmetry-parity refusal and the RI-K counter sink. Two governed paths
// had no point-group reduction: ri_k (whose builder RECEIVED RiEngineOptions
// and silently ignored the reduction fields) and qfmm (whose option set could
// not carry one). The RI-K builder now REFUSES a requested reduction by name
// - honoured, refused with its condition named, or demoted with the
// disclosure, never silently substituted - and carries the term counters
// later work is sized from. The refusal fires before any tensor work, so the
// controls - the same call with the fields cleared - are the only expensive
// legs here.
//
// The qfmm leg is NOT here: its refusal, the same shape, is covered by the
// qfmm_fock_build tests.
...
TEST(SymmetryParityRefusalTest, RiFullFockRefusesARequestedReduction) {
    ...
    // which of its two pointers to clear ("refused with its condition named").
    ...
    // the number later work is aimed by.
...
TEST(SymmetryParityRefusalTest, TheChunkSinkReproducesTheMonolithicCounters) {
    ...
    // The measured number later work is aimed by: printed, not only asserted.
