// The auxiliary-pair density screen's structure and budget cells: the existing
// density bounds applied to the auxiliary-pair index, retaining the per-aux
// surviving-row structure. The named acceptance fixture - benzene/def2-TZVP -
// is its own file (ri_aux_screen_benzene_test.cpp), because the basis fixtures
// define `kSto3gCarbon` twice and one TU cannot include both; the instrument
// they share is ri_aux_screen_fixture.hpp.
//
...
//     builder's screened sweep over the FULL grid reproduces the module's dense
//     contraction entry points to round-off, so "screened against unscreened"
//     is a comparison of the same code with and without a dropped set, not of
//     two implementations.
//   - TheScreenStaysInsideThePresetBudgetWhereItEngages: the screen's own
//     added acceptance, which is NOT optional. The screen is a truncation, and
//     on an approximated-exchange path the truncation error does not shrink
//     with the RI error, so the converged energy must not move past the
//     preset's own J/K target when the screen engages. A cell that only checked
//     "it got faster" would miss exactly the failure mode the screen exists
//     to avoid.
//
// The screen's THRESHOLD value is a pending ruling: the screening-induced
// converged-energy move must stay at or below a tenth of the measured
// auxiliary-fit error on the same fixture, and no default number is fixed yet.
...
// a pre-registered constant: the value that becomes a preset default is still
// to be ruled, and this file would then switch to the mapper that carries it.
...
// The three presets the budget cell walks, in the preset ladder's own order.
...
TEST(RiAuxScreenTest, TheScreenDropsTheCellsItsGateNames) {
    ...
    // positive bound product can fall under - must reproduce the module's dense
    // contraction composition. That is what licenses reading every later
    ...
    std::cout << "[ri_jk screen] " << label << " (aux " << kAuxName << ", threshold " << threshold
              ...
              << "dense contraction composition " << sweepDeviation << "\n";
    ...
    EXPECT_LT(sweepDeviation, 1e-12)
        << "the screened sweep over the full grid must reproduce the dense contraction "
           "composition to round-off - a deviation here is a defect in the block walk (the "
           "mirrored lower triangle), not a screening effect";
}

// ---------------------------------------------------------------------------
// The added acceptance: the truncation stays inside the preset's own
// budget when the screen engages.
// ---------------------------------------------------------------------------
...
TEST(RiAuxScreenTest, TheScreenStaysInsideThePresetBudgetWhereItEngages) {
    ...
    // BenzeneDef2TzvpScreenedEnergyStaysWithinOneHundredthKcalPerMol, and its
    // recorded `benzene_def2tzvp.kNormal.energy_move_ratio` is that ratio. Two
    // facts keep that from being a defect the cells hide:
    ...
    // That trade belongs to the threshold ruling, so this cell pins the
    // CONTRACT that holds - a truncation that never reaches
    // the approximation's own error class - and records the ratio the ruling
