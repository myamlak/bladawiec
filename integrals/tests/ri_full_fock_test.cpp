// The composed full-RI Fock builder tests (ri_full_fock.hpp).
//
// The cells, and what each one is for:
//   - CreateRefusesTheRungsItDoesNotHaveAndNamesTheLadder: the refusal contract
//     for the option surface the memory ladder opened - the recompute rung's
//     knob (forceLightRung) refused by name with the rung that DOES run named
//     beside it, and a budget that holds no rung refused with the ladder in
//     its ruled ORDER (batched/blocked before recompute, disk last). Each
...
//     ladder's second rung (ri_full_fock.hpp RiFullFockRung), the one the
//     owner's rule puts BEFORE any per-iteration recompute. A rung selected
//     by memory must change nothing else: B, F and the term counters are
...
//   - TheBatchedTraversalIsReachedAndReproducesTheChain: the batched
//     traversal's WIRING, which is the thing a capability can be landed
//     without. The batched traversal and the three-call chain agree to
//     round-off at every width, so no comparison of their OUTPUT can tell a
//     builder that reaches the traversal from one that still runs the chain -
//     the cell therefore reads the per-call record, which is written from the
//     branch that ran, and it fails if that branch is not the batched one. Both
//     widths, the default, and the one combination Create refuses.
//   - BuildFockEvaluatesNoQuartetsIsAMeasurement: the COMPOSITION'S DEFINING
...
//     the shipped ri_j_link path (the error class the comparison is taken in).
//     Records the energy move, the iteration counts (which the acceptance
//     requires to be essentially unchanged) and the aux K-fit error class the
//     composition realizes. It runs on TWO fixtures, so the numbers land on the
//     contraction's own baseline rather than beside it: H2O/STO-3G, where the
//     in-tree pin anchors the instrument, and water/def2-SVP, the contraction's
//     acceptance fixture and the one that measured the worst RI-K/RI-J error
//     ratio (5.82x). The recorded fixture numbers for the same two shapes are
//     3.524e-4 and 8.515e-5 Eh of aux K-fit exchange error.
//   - IsJkOptimizedAuxAgreesWithTheVendoredListing: the JK-fit predicate over
//     the resolved aux name, including the two doors it names (the explicit
//     -rifit override for a def2-* orbital base, and the future cc-pvdz-jkfit
//     that must lift automatically), and the cross-check that the predicate
//     agrees with the vendored data/basis listing.
//
...
// The composition's round-off class: the two exact identities the contraction
// anchored (J from B at 8.7e-14 relative, the occ form against the AO form at
// 4.4e-16) compose into this cell, so it is the same 1e-12 class, read
// relative to the compared magnitude exactly as ri_engine_test.cpp reads it.
...
// The SCF instrument's convergence threshold: the operating setting (1e-8
// energy, under the standing no-1e-10 rule). Both sides of every comparison
...
qcx::Result<RhfOutcome> RunRhf(
    ...
    // INSTRUMENT/PRODUCTION DIVERGENCE, corrected 2026-09-14. Loewdin's
...
TEST(RiFullFockTest, TheBlockedRungReproducesTheFastRungAndChargesItsBudget) {
    ...
    // decision about MEMORY and nothing else: B, F and the term counters are
    // compared between the two rungs directly, so a rung that quietly changed
...
// ---------------------------------------------------------------------------
// The batched traversal's wiring, reached through the builder.
// ---------------------------------------------------------------------------
...
TEST(RiFullFockTest, TheBatchedTraversalIsReachedAndReproducesTheChain) {
    ...
    // arithmetic in the chain's order - the batching suite's bit-identity arm.
    ...
    // the reading the batching landing pinned at the module level and this cell
    // reports through the builder.
...
// ---------------------------------------------------------------------------
// The composition's defining property: no nested direct-exchange call.
// ---------------------------------------------------------------------------
...
// the composed full-RI builder is the composition's own path.
//
...
void MeasureComposedScf(const qcx::molecule::Molecule& molecule,
    ...
    // error class the RI-K error is compared WITHIN (the same-fixture
    // RI-J error class), so it is the baseline the composition is measured
    // against - and it is already SCF-converged at 11 iterations on this
    // fixture.
    auto shipped =
    ...
    // The composed full-RI builder: the composition's own run path.
    auto builder =
    ...
    // The iteration-count acceptance: the count is essentially unchanged. The
    // two
    // shipped-like paths are compared against the exact anchor on the same
    // instrument; the bound is generous because this is a convergence-path
    // statement, not an accuracy one.
    ...
    // lands in is the RI-K-versus-RI-J error comparison's own subject, and the
    // composition does not re-open
    // it (the composition does not change the auxiliary fit, so the class the
    // contraction measured is the class this path still carries).
...
TEST(RiFullFockTest, ScfThroughTheComposedBuilderReachesTheReference) {
    ...
    // The contraction's acceptance fixture, and the one where it measured the
    // worst RI-K/RI-J error ratio (5.82x). It carries no in-tree energy pin - the
    // instrument's anchor is exercised by the STO-3G shape above - so this
...
// ---------------------------------------------------------------------------
// The JK-fit predicate, over the resolved aux name.
// ---------------------------------------------------------------------------

TEST(RiFullFockTest, IsJkOptimizedAuxAgreesWithTheVendoredListing) {
    // The predicate the ri_jk refusal is built on. The two doors it names are
    // both here: the explicit -rifit override for a def2-* orbital
    // base, and the future cc-pdvz-jkfit that must lift without a code change.
    ...
    // The auto-selection interaction, which is why the rule puts the check on
    // the RESOLVED NAME rather than on the requested kind: for a def2-* orbital
    ...
    // THE HAZARD LINE, THE TIERED-RULE INVERSION (2026-09-13). This assertion
    // used to read
    // EXPECT_FALSE, and it was an honest pin of the OLD rule: a cc-* orbital
    // base under kRiJk resolved to a J-ONLY fit, which the downstream refusal
    // then caught. That resolution IS the "silently using a J-fit for K"
    // substitution the tiered rule names as the real risk, so the old pin
    // documented a defect as though it were a contract. The tiered rule no
    // longer produces a J-fit for a JK request at all, and this asserts the
    // property that actually matters. Kept as the same test, inverted, rather
    // than deleted: the line it guards is the one the refusal is the second
    // line of defence for.
    ...
    // - which after the tiered rule is the explicit [basis].aux door, since the
    ...
    // The count moved 1 -> 2 and the refusals 4 -> 7 on 2026-09-13, when the
    ...
    // count: a JK fit the predicate REFUSED would be exactly the failure the
    // predicate exists to prevent, so it is checked by suffix on every refused
    // name.
    for (const std::string& name : refused)
    {
...
// ---------------------------------------------------------------------------
// The 2026-09-13 corpus addition, run end to end.
// ---------------------------------------------------------------------------
...
// 2026-09-13: a diffuse calculation could not go up a rung at all. This is the
// brief's own acceptance case - an aug-cc-pvtz RI-J run must resolve and
// converge - and resolution is asserted against the rule rather than a literal.
TEST(RiFullFockTest, AugCcpvtzRiJLinkResolvesAndConverges) {
    ...
    // the contract this cell pins. That is checkable without running an SCF.
    const auto auxName =
...
// The RI-K corridor the JKFIT addition was aimed at (the stated
// purpose behind the corpus addition), measured on the module's own instrument
// rather than
// asserted. Water/cc-pvdz, the composed full-RI SCF, against the EXACT direct
// SCF on the same fixture. Two auxes run through it:
//
...
// molecule cannot certify a rule, and that is exactly the caution its own
// figures carry. The bound below only refuses a move an order past the aux
// K-fit error class, which would mean the run is broken rather than merely
// fitted.
//
...
// ---------------------------------------------------------------------------
// The corpus gate for the families the corpus addition brought in, and the
// window it sits in.
// ---------------------------------------------------------------------------
...
// invisible to a directory check and fatal to every run, so the seven families
// added on 2026-09-13 are parsed and sized here.
