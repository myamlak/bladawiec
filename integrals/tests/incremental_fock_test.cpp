...
// every call down the full-rebuild path (the plan's easy-case trap).
...
// \param perElementScreening The per-element screening flag for both builders: on, the
// incremental delta passes rebuild the per-call pair-max vector per
// BuildFock call (the delta density's own maxima: a builder-level cache would
// screen the wrong density), and the envelope absorbs the filter's
...
TEST(IncrementalFockBuilderTest, MatchesFullRebuildWithinRoundoff) {
    if (IsFastOnlyMode())
    {
        GTEST_SKIP() << "full-sequence numerical pin - fast smoke subset";
    }
...
// then exercise the delta-density filter path: each BuildFock call - full and
...
TEST(IncrementalFockBuilderTest, RebuildTriggerFiresAfterMaxConsecutive) {
    if (IsFastOnlyMode())
    {
        GTEST_SKIP() << "rebuild-trigger counter pin - fast smoke subset";
    }
...
TEST(IncrementalFockBuilderTest, ResetRestartsTheStateMachine) {
    if (IsFastOnlyMode())
    {
        GTEST_SKIP() << "reset-state-machine pin - fast smoke subset";
    }
