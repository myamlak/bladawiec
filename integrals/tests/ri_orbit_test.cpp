// The orbit expansion the owner ruled on
// 2026-09-13 ("the expansion is worth doing, skip reading not"): RI-J's
// 3-center work is a (braPair, auxShell) task grid and the point group acts
// on both indices, so the fast path's single tensor pass can evaluate one
// REPRESENTATIVE per joint orbit and expand the rest of the orbit from that
// representative's block.
//
// What these tests hold down:
//   - the joint action's own contract (one group over two bases, both
//     shell-closed - the second being the piece the orbital-basis action alone
...
//     the cells the Create-time 3-center screen dropped stay exactly zero in
//     both;
//   - the BLOCK COUNT: the tensor-pass term counters fall to the
...
// cells (the Schwarz screening test), their orbits found by an INDEPENDENT
...
// shell's - the quantity the g3 term counter sums (AccumulateScreenedRiPass:
...
// option a record cannot mention is an option nobody can check (the
// disclosure the flag owes). True exactly under the condition the tables
// are built with; false when the flag is off and when the group is trivial
// (the mechanism is then the identity).
TEST(RiOrbitExpansionTest, BuilderNamesWhetherTheExpansionRan) {
    ...
    // SAME statement one step out, and the step that costs real work if
    // it is missed (the mechanism-value audit, 2026-09-13): a NON-C1 group
    // that acts trivially on every shell pair. Every element is a pure sign
    // flip - the molecular plane's mirror on a planar molecule, the audit's
    // hocl/Cs case - so every joint cell is its own orbit and the expansion
    // removes nothing while building its tables and charging them. The
    // verdict is the same as the order-1 case above, and an order test
    // (`groupOrder > 1`, what this predicate used to read) gets it wrong.
...
// allocation cannot drift apart (the never-under discipline). The term is
// zero whenever the mechanism is not engaged, which is why the default
// path's envelope is untouched.
...
TEST(RiOrbitExpansionTest, RepresentativesPartitionTheSurvivingGrid) {
    ...
    std::cout << "[ri-orbit] water/STO-3G + tiny aux: survivors " << census.survivors
...
// c8h18/def2-SVP + def2-universal-jfit, C2h - the numbers the default-ON
// ruling rests on - and THOSE FOUR NUMBERS ARE TRANSCRIBED FROM AN UNTRACKED
// THROWAWAY PROBE AND ARE REPRODUCED BY NO TEST IN THIS TREE. What is
...
TEST(RiOrbitExpansionTest, ExpansionTradesEvaluationsForCopiesAndAddsNoScatter) {
    ...
    std::cout << "[ri-orbit] accounting (fixed): survivors " << survivors << ", reps " << reps
...
// (value-equal) evaluation order - the action's landing measured that class of
// difference at ~1e-15/1e-16 per element. Measured here: this fixture is
...
TEST(RiOrbitExpansionTest, TensorMatchesThePlainWalkWithinTheMeasuredTolerance) {
         ...
         {qcx::integrals::AccuracyPreset::kNormal, qcx::integrals::AccuracyPreset::kLoose})
    {
        ...
        std::cout << "[ri-orbit] value neutrality ("
        ...
        // zero in both tensors.
        std::cout << "[ri-orbit]   dropped cells at this preset: " << droppedCells << '\n';
    }
}

// The block count: the tensor-pass term counters fall to the evaluated
...
TEST(RiOrbitExpansionTest, EvaluatedBlocksFallToTheRepresentatives) {
    ...
    std::cout << "[ri-orbit] block counts: g3 plain " << plainCounters.g3 << " expanded "
...
// same discipline the orbital-basis action gives its own flag.
...
// rung recomputes the grid per iteration (the amortized variant, a separate
// body of work), so the flag is REFUSED there rather than silently dropped.
