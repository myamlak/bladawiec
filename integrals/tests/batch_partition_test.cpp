...
// (BatchElementCount, EqualKetGroupStraddles, GpuSplitBatchIndex) - pinned
// 2026-09-04: the split index is deterministic and
// cost-based (the fp64 block element mass), lands at the nearest
// equal-ket group boundary (never inside a group), and degenerates to the
// empty suffix at the default zero device share (the CPU-only fallback)
// and the empty prefix at one. The fixture exercises real assembled
// batches (the byte-cap cuts of AssembleClassBatches land where they
