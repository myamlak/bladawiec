// The lean builder's Abelian point-group reduction: it reaches the lean path
// as a CLASSIFICATION, not as a contraction
...
// The second half is the ORBIT EXPANSION
...
// the orbit's members - the petite-list addition, whose tests and their own
// pins are at the end of this file.
//
...
// ===========================================================================
// The petite-list addition: the ORBIT EXPANSION.
...
//     in exact arithmetic (the machinery pins its own class path against the
//     plain path the same way, at 1e-12, fock_build_test.cpp:772). The delta
//     is a CEILING, never an equality
...
// fp64QuartetCount and their member sum must equal the plain build's. The
// work is a count, and the count is the family's own, not this test's model.
TEST(LeanPointGroupTest, OrbitExpansionCountsMatchTheMachineryClassDecomposition) {
    ...
    // The fixture's own numbers: water/STO-3G C2v at kTight has 120 screened
    // canonical cells covered by 76 ERI blocks (66 kept class pairs, 76 orbit
    // representatives, 120 member quartets). The assertions below are
    ...
    // THE STRONG-FORM ARGUMENT, measured in the counters. The orbit expansion
    // removes ERI EVALUATIONS and removes nothing at all from the
    // ACCUMULATION: `ContractBlock` runs once
    ...
    // remove its contraction. That invariance was reconstructed by hand before
    // the counter existed (8,811,481 either way on c8h18/def2-SVP); this pin
    // is it, read off two live builds. It is also why the ERI wall falls while
    // the contraction span does not.
...
TEST(LeanPointGroupTest, OrbitExpansionCountsOnTheInversionFixtureAndThePruningCountsOnce) {
    ...
    // them. 120 member quartets in 76 orbits.
