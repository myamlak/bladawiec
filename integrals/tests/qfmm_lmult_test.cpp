// The general-L_mult acceptance gate: on the C12 alkane fixture at every multipole order
...
//   3. decrease monotonically in theta at every fixed L_mult (the theta-
//      monotonicity property carried forward).
...
TEST(QfmmLMultTest, TranslationTablesReduceToCoulombAtMonopoleOrder) {
    // The bit-exact L_mult = 0 reduction's foundation (the gate on the
    ...
    // reproducing the monopole Coulomb expression exactly. In the sparse
...
TEST(QfmmLMultTest, FarFieldErrorDecreasesWithLMultAndTighterTheta) {
    // The two monotonicity properties, carried forward from the monopole
    // gate. The C12 alkane
    // fixture, the theta ladder {1.05, 0.85, 0.7} (loosest first) and the
    // L_mult ladder {0, 1, 2}. The near-field parts of the QFMM and the
    // direct build are bitwise equal (same options, same density, serial
    // pin), so the measured error is purely the far-field multipole
    // truncation - and it must shrink strictly in BOTH directions:
    ...
    //     (the theta-monotonicity property carried forward).
    if (IsFastOnlyMode())
    {
...
TEST(QfmmLMultTest, MonopoleBlockAtLMultOneMatchesTheOverlap) {
    // The optional self-consistency check: at lMult = 1 the
...
// The order-selection gate fixtures: an alkane chain's octree at a preset's extent
...
// geometry the adaptive order selection and the on-demand table run on (the
...
/// The source-radius ratio r / d the order gate reads for a node pair: the
/// larger of the two nodes' charge radii over the centre distance - the radius
/// MultipoleTruncationBound raises to (L + 1), the quantity SelectMultipoleOrders
/// compares against epsInt. It is NOT the box circumradius sqrt(3) * halfWidth
/// this used to build: that is the box's own slack, not the reach of the charge
/// the moments are of, and it over-states the source radius by the box's
/// diagonal - so a ratio built from it tested the selector against a number the
/// selector never used.
double FarPairRatio(const qcx::integrals::internal::QfmmTreeBuildResult& tree,
    ...
    return std::max(nodeA.radius, nodeB.radius) / std::sqrt(dx * dx + dy * dy + dz * dz);
}

TEST(QfmmLMultTest, SelectMultipoleOrdersPinsTheMinimalOrderAndTheCap) {
    // The adaptive selector's contract, on a
    // real C24 tree at theta 0.21 (the far-pair onset band, where the
    // source-radius ratios sit around 0.1 - the charge radius the bound reads,
    // not the box circumradius - making the per-interaction budget large
    // enough for the selector to genuinely lower orders below the cap): each
    // returned order is the MINIMAL L whose bound (r/d)^(L+1) fits epsInt
    // (asserted by the two-sided inequality, order+1 qualifies and order does
    // not - or the pair keeps the cap), no order exceeds the cap, the
    // tiny-epsInt limit saturates at the cap for every pair, and a generous
    // epsInt reaches L = 0 (bound of an order-0 pair is ratio <= 1, satisfied
    // by every well-separated pair).
...
TEST(QfmmLMultTest, SelectMultipoleOrdersIsMonotoneAndDeterministic) {
    ...
    // clause of the selector gate), and a looser per-interaction budget never
...
TEST(QfmmLMultTest, MomentTableAllocatesZeroBlocksForNearOnlyPairs) {
    // The memory gate (the on-demand moment table): on the C12
...
TEST(QfmmLMultTest, AdaptiveMomentTableContentMatchesTheFullTable) {
    // The content gate (the on-demand moment table): "bit-identity
...
TEST(QfmmLMultTest, AdaptiveBuilderMatchesFixedWhenTheOrdersSaturate) {
    // The selector gate's first clause: "at
