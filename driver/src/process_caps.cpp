...
// the attribution instrument: its watchdog samples
...
// problem (the defect of: one job per call, ~100 calls per
// process).
...
// tightest limit and the one the attribution instrument samples.
...
qcx::Result<AppliedProcessCaps> ApplyProcessCaps(const qcx::io::RunResourcesInput& resources) {
    ...
    // fails with error 50. Reusing the job in force also keeps the attribution
    ...
    // The attribution instrument's job-peak sampler binds the applied
    ...
    // applied in-process and NOTHING enforces it here - the predictive
    // model that once stood in for the missing limit was deleted on
    // 2026-09-17. The record says so honestly instead of pretending a cap
    // exists.
    (void)resources;
    return AppliedProcessCaps{false,
