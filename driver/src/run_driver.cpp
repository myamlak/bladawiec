...
// definitions sit at the end of this file (the clauses and the NOCV charge
// are not predictions and stay).
...
struct EriStoreHandle {
    ...
    /// one string `resources_resolved.eri_store.demoted_reason` carries (a demotion names its cause, and the cause is the store's own words,
...
// trailing members carry the ri_j budget path: the workspace
...
struct WiredFockBuilder {
    ...
    /// record's mode_record.forced_disk (schema 18) - the one fact that
    ...
    /// (schema 26): the ENGINE's own answer
    ...
    /// them to the RI-J builder (schema 26). Distinct from
    ...
    /// The composed-QFMM model the Coulomb half ran (schema 29), set at the
    ...
    // The disk route's scratch-store path: set only when the disk rung engaged. The store's HDF5 file
    ...
    // The Kohn-Sham seam's companions: the energy seam
    // needs J[D] and the functional's own contribution alongside the
...
// The certified-bound collector (schema 14): the run's reduction over
...
public:
    ...
    /// True when the final observed call ran the global budget
...
// re-derived as the pre-gate arm and the reservation half was
...
// default's split point is GONE with the ladder: the boundary is
...
// molecule-scoped count are equal by construction (the filtered parse's
// own contract); a directory-wide parse would over-count elements the
...
struct UhfRun {
    ...
    std::optional<qcx::integrals::FockModeInfo> modeInfo;
    std::optional<qcx::integrals::FockModeInfo> exchangeModeInfo;
    /// The composed-QFMM model the Coulomb half ran (schema 29): the
    ...
    /// twins: the builder whose per-run term counters the
    ...
    /// expansion's request/outcome pair (schema 26: whether this run wired
    ...
    bool orbitExpansionEngaged = false;
    bool orbitExpansionWired = false;
    /// The composed full-RI family's Create-time rung decision (schema 30),
...
qcx::Result<UhfRun> RunDirectUhfScf(
    ...
    // ResolvedBuilderSelection::leanMember (the size ladder). Before
...
// v1; every other builder has no aux). The checkpoint
...
// rule chose, or nothing when it owes none (the notice's text is
// qcx::integrals::AuxSelectionNotice and its record home is
// RunApproximation::auxNotice, schema 25).
//
// READ AT THE SAME RESOLUTION POINT as the name above, and from the same two
// inputs, so it cannot describe a selection the wiring did not make. The
...
// build stops. The failure it prevents is SILENT: a device number reaching a
// CPU builder
...
// Renaming it to `computeProfile` would be an open decision, not this
// function's to take; this function is the one place where the two names
// meet, and the field's own documentation carries the same reading.
...
// enumerator a BUILD FAILURE - so a change that adds a MethodType enumerator
...
qcx::Result<ScfPath> ResolveScfPath(qcx::io::MethodType method) {
    ...
    // The Kohn-Sham families. The composition that folds the
...
// The [method] word that names the run path the classifier resolved (schema 34)
...
// the resolution can retarget - the D-A7 device-less gpu fallback, which lands
...
std::string FamilyNameForRefusal(const qcx::io::RunInput& input) {
    ...
    return "the no-fock_builder size ladder's own tier for this run (the direct family's "
           "lean member at nBasis <= 1000, ri_j_link to 2000, "
...
qcx::Result<void> ValidateCombination(const qcx::io::RunInput& input) {
    // The builder is optional: absent means the size ladder
    // (the direct family's lean member at nBasis <= 1000,
    ...
    // first clause was "no full-RI exchange path exists". The full-RI exchange
    // builder answered that clause by existence (integrals RiFullFockBuilder, the composed
    ...
    // whitelist named the direct family alone at first, took the RI-J
    // link beside it, then the composed QFMM builder, and took THIS family
    // too, once the pair's bare
    ...
    // be unreachable behind this one, and driver tests pin both; each later
    // admission moved the cell the pin sits on with its own reason.
    ...
    // The lean spelling on a UHF run needs no rule of its own (the UHF
    ...
    // The builder-combination whitelist for the Kohn-Sham lanes. A Kohn-Sham Fock is not a Hartree-Fock Fock with an extra term
    ...
    //   - ri_jk: OPENED, on this rule's own terms. The composed
    ...
    //   - qfmm: OPENED. It was named here as "the composed builder
    ...
    //     by name, a separate gap, which the runner carries
    ...
    //     rung request it cannot serve (the no-silent-substitution rule).
    ...
    // accounting for the seam" to OPENED (MakeRiFullKsHalf), and
    ...
    // defect this whole guard exists to prevent. Refused by name until the
    ...
    // the defect class this rule exists for - an unwired request running
    ...
    //
    // The extent model, the separation mode and
    ...
    // rather than a request to refuse or to drop. What they
    // get instead is the record: the run's `qfmm_model` block (schema 29)
    ...
    //
    // This rule does NOT extend to method.ri_tensor_mode's rung word: whether the
    // ladder word ("disk") should refuse on a family with no disk rung, where
    // the sibling force key demotes, is an open question rather than a
    // judgement applied here.
    //
    // The FORCE is a separate key and demotes: [diagnostics] force_disk_ri
    // keeps the posture the retired "forced_disk" word carried -
    // DEMOTE with the record stating both sides (resources_resolved.
    // ri_tensor_mode carries requested/resolved/outcome/reason, schema 19;
    // a key that cannot work is demoted to the judged best choice). That
    // disclosure replaced a by-name refusal, once
    ...
    // key's refusals that would survive - no runnable choice, or a
    ...
    // system's judgement, and an absent knob keeps the engine's
    // preset default.
...
// epoch; the join tool estimates the launch offset). The
...
// split-probe tail (the phase-accounting instrument) is the lean path's
...
// one per ContractBlock call - per assembled class task, plus
...
// 8,811,481. The
// window probe adds the per-window spread columns when the seam handed a
// per-window record: window_count=<n>, window_wall_max_ms /
// window_wall_min_ms / window_wall_mean_ms over the call's row windows -
// the per-window reading of the parallelism decomposition. The windows
...
// the region's wall. The kernel-span probe subdivides that kernel time in the SAME row:
...
// and the stats parser is presence-driven (an absent column is
...
public:
    ...
    // certified lane on the recompute path) plus xvol, and the
    ...
    // gate reads off a one-call run, where the cumulative wall column
    // has no delta to give). The per-window
    ...
    void RecordLean(const qcx::integrals::FockBuildStats& stats, std::size_t exchangeOccupied) {
        ...
        // The split-probe tail: the two lean
        ...
        // The kernel-span probe: the three disjoint
...
// either way (measured: exit 0, 0 parseable rows, no diagnostic, and a
// timing limb that silently could not be measured). The warning uses the
// qcx: warning: form the selection divergence uses, so a caller can grep
...
// D-A9 Q*O_occ calibration volume on the recorded lines. Zero when no
...
// quantity the driver discarded before schema 14. Only the direct
...
// The composed full-RI exchange seam. The
...
// exactly for ANY block C_occ - the identity verified at
// 4.4e-16, and the one RiFullFockBuilder's own header states. The exchange half
...
//
// What that buys, and why it is the smallest reading of an open choice: a
...
// and is left as a known open item rather than taken here.
//
...
qcx::scf::FockBuilderFn MakeLeanRhfFockBuilder(const qcx::integrals::LeanDirectFockBuilder& builder,
    ...
    return [builder, statsSink, exchangeOccupied](
        ...
        qcx::integrals::FockBuildStats callStats;
        // The window probe: the seam's own per-window record, refilled
        // by the stats-carrying call (one entry per row window) and read by
...
// machinery member and its lean member. The seam hands over
...
// three BuildFock calls with their K-channel occupancies: the J-only call
// contracts no exchange (xvol 0), the
...
// The lean member's UHF seam: the same assembly over two
...
// (schema 34): the same six numbers, in the record's own shape.
...
// subtracted per channel - the MakeDirectUhfFockBuilder math). The
// lambda is only the Eigen <-> tensor boundary: the scf seam speaks Eigen
// matrices, the integrals builder host-canonical tensors. Unlike
// MakeDirectUhfFockBuilder there is no per-call stats wrapper here, so the
// per-spin exchange occupancies are not needed and a traced composed-QFMM
// UHF run lands zero rows in scf_trace.stats - the RHF arm's QFMM row
// family (MakeQfmmRhfFockBuilder / RecordQfmm) has no UHF counterpart yet,
// because RunQfmmUhfScf owns no stats stream of its own (its traceFile
// feeds the scf loop's own trace lines only). Known and named, never a
// silent zero: see DriverRunTest.TraceEnabledRunsEmitTheRowFamilyOfTheirBuilder.
...
// re-verifies it against the workspace budget's remaining bytes (the
...
struct WiredUhfBuilders {
    ...
    /// runner's two arms (the Hartree-Fock arm and the Kohn-Sham one),
    ...
    // per-run counters the record publishes on an instrumented
    ...
    /// (ri_engine.hpp RiTermCounters, and schema 3.8 says in as many words
...
qcx::Result<UhfRun> RunUhfScfWithBuilder(
    ...
    //
    // Measured on O2/STO-3G at 1.2075 A (2.2818443 Bohr), at the default
    // settings and after the comparison case was cleared of a unit error: the
    ...
    // Our good defaults are used rather than an
    ...
    return UhfRun{std::move(*scf),
                  ...
                  std::move(wired.modeInfo),
                  std::move(wired.exchangeModeInfo),
...
//
// Two arms, one per member of the direct family (the UHF seam): the
...
qcx::Result<UhfRun> RunDirectUhfScf(
    ...
    // The lean arm (the UHF seam): the SAME member choice the RHF
    ...
    // size ladder is the absent key at nBasis <= 1000, an explicit
    ...
    // so leanMember is false by construction (the explicit
    ...
    // The certified fp32 lane's REQUEST (the [method] force_certified_lane
    // key): both halves carry it for the same
    ...
    // it against the budget's remaining bytes (the sure-fit gate). A
    ...
    // The class-aware seam: the same reduction the RHF branch
...
qcx::Result<UhfRun> RunQfmmUhfScf(
    ...
    // The adaptive seam, wired like the direct runner above: the
    ...
    if (IsKohnShamPath(*scfPath))
    {
        ...
        // RHF run on this family states (schema 29).
...
qcx::Result<UhfRun> RunRiJLinkUhfScf(
    ...
    // run that dropped it would be a silent substitution
    ...
    // exactly as on the RHF leg: the driver grants the cap as the
    ...
    // The D-A9 counter carrier: the run flow reads the run's accumulated term
...
// effect and its JK-fit quality refusal, the cap-minus-base workspace budget the
...
//     unrestricted run's record has to know.
//
...
qcx::Result<UhfRun> RunRiJkUhfScf(
    ...
    // and the JK-fit aux quality), the budget and its rung, the point-group carrier and the
    ...
    // The JK-fit aux check at the site that RESOLVES the name (the restricted arm's
    ...
    // The budget path (the restricted arm's, verbatim): the driver
    ...
    // carries - its D-A9 counters are NOT published through the ri_j_link
...
qcx::Result<std::optional<qcx::io::RunFukui>> BuildFukuiBlock(
    ...
    // Convergence gates (bug-hunt drv-2, the prop-2 discipline): the
...
qcx::Result<std::optional<qcx::io::RunNocv>> BuildNocvBlock(
    ...
    // The NOCV dense-tensor charge: the
    ...
    // refusal through the driver's own reinstatement ladder in the
    // user-facing memory_cap_gib terms, so no NOCV
    ...
    if (!eri.has_value())
    {
        ...
        // standing NOCV clause composed onto it (the same seam pattern
        // the ri_j and direct branches use): the engine text alone names
...
// The opt-in density-at-nuclei block: the
...
// The opt-in Bader QTAIM block: the (3,-1)
...
// The symmetry guard's action word (schema 32), and with it the PRESENCE rule of
...
// The symmetry-blocking disclosure of one spin channel (schema 32): the guard's
...
// The workspace_budget record: the cap-minus-base budget the
...
// The engine's Create-time mode record: mirrors the integrals
...
// That is the misreport class this guard forbids, in its sharpest form - the wrong
...
std::optional<qcx::io::RunModeRecord> MakeRunModeRecord(
    ...
    // accuracy presets, not a new lowercase vocabulary. kDisk is the driver's disk
...
// The composed full-RI builder's Create-time rung record (schema 30): the
...
// The composed-QFMM builder's model record (schema 29): mirrors the
...
// it. They are the whole disclosure: the default is the pair
...
// The requested-vs-ran pairing of the RI tensor mode (schema 19, the
...
// or [diagnostics] force_disk_ri (schema 22) - on the paths with no mode
...
// that replaced method.ri_tensor_mode = "forced_disk", so it carries exactly
// the posture the displaced word carried (cbee51cf: the request DEMOTES,
// with the record stating both sides,
...
std::optional<qcx::io::RunRiTensorMode> MakeRiTensorModeRecord(
    ...
    {
        // An omitted key is the DEFAULT request, and a default is
...
// The ri_chunk_bytes request record (schema 23): the disclosure half of the
// key split. The key is a pure SIZE HINT and is read
...
// the run proceeds and the hint is never read. Dropping it is deliberate
// (refusing a size hint on a family with no chunking to size is
...
// remedy, and the key stays non-fatal.
std::optional<qcx::io::RunRiChunkBytes> MakeRiChunkBytesRecord(
    ...
    if (!input.method.riChunkBytes.has_value())
    {
        ...
        // (the schema-19 null-honesty rule the sibling block follows).
        return std::nullopt;
    }
...
// `EngineDecoratorFactory`): the closure the builder calls ONCE at Create with the two raw
...
// text stays on the handle for the record to disclose (honour, else demote
// to the best runnable choice with the demotion disclosed). Returning the
// engines unchanged rather than an Error is the whole point - a store that
// cannot open costs the run a cache, it
...
// The eri_store request record (schema 33): the outcome of
...
// substitution this record exists to forbid.
//
...
// reduction) and on the LightPath (whose miss-assembly reads the full
...
// The orbit-expansion request record (schema 26): the outcome of
// method.ri_orbit_expansion, the mechanism's only driver-side key. The
...
std::optional<qcx::io::RunRiOrbitExpansion> MakeRiOrbitExpansionRecord(
    ...
    if (!input.method.riOrbitExpansion.has_value())
    {
        ...
        // (the schema-19 null-honesty rule the sibling blocks follow).
        return std::nullopt;
    }
...
// One unique scratch-store path for the disk route's chunked tensor:
// the OS temp dir plus a per-process
...
// The lean selector this file used to carry (IsLeanDirectSelection: an
// ABSENT fock_builder at nBasis <= 1000, or the explicit lean word) is
// GONE with the size ladder, and it is not replaced
...
// The no-key ladder's tier runnability for one run: the two facts the
// resolution's demotion reads, stated once
...
//     route around a refusal it would make for the same request. This
...
//   - no auxiliary basis the wiring can resolve. Measured as live under the
//     earlier rule, which refused every
//     orbital name outside def2-* and cc-* (the constraint that left
//     ri_j_link unestablished at the tightest tier). That rule was
//     replaced with a quality tier that always
...
// qfmm is runnable on the Kohn-Sham lanes (the composition
...
LadderTierRunnability LadderRunnabilityFor(const qcx::io::RunInput& input) {
    ...
    //
    // The qfmm tier left this branch with its composition, so
...
//     the driver does not seed - a gap deliberately NOT
...
// The lean member's name in the run record (schema 15): the same word the
...
// The ri_jk family's member word. It is
...
// approximated (schema 25; the notice-only block, RunApproximation::exchange).
...
// (the record must not need inference).
std::string BuilderMemberName(qcx::io::BuilderKind kind, bool lean) {
    ...
    // The one family whose member word is not its own family word.
...
// The ladder's ONE exception as a fact for the resolution: a Kohn-Sham run whose
...
// The pre-gate setup admission (the above-ceiling crash class): the
// decision the run's own gates make
...
//
// This consult runs the SAME admission the wiring would apply, one step
...
//
// The consult has TWO arms, and the second is the crash class's own answer.
//
...
// sits BEHIND it. The earlier admission model's removal retired the arms that
...
// ResolvedBuilderSelection::leanMember): the consult sizes the admission of
// the member that will actually run, so under the size ladder a run
...
qcx::Result<double> CheckPreGateSetupAdmission(const qcx::io::RunInput& input,
    ...
    // envelope is computable here. Both method types reach it (the UHF
    ...
    if (kind == qcx::io::BuilderKind::kDirect && leanMember)
    {
        ...
        // Consulting on a disengaged envelope would admit a run the builder's
    ...
    // alone at first, and the gap that left was MEASURED on the
...
// Create-time estimate: it sizes the cap-minus-base workspace
...
qcx::Result<WiredFockBuilder> WireRhfFockBuilder(
    ...
    // size ladder a run above the 1000 boundary can be sent to the
    ...
    // The certified fp32 lane's REQUEST (the [method]
    // force_certified_lane key): the ONE resolution point in
    ...
    // The certified-bound global budget enforcement:
    ...
    // machinery at every size (explicit direct = the existing
    ...
    // A point-group classification IS handed to the lean
    ...
    // when the lean arm held no classification at all; the
    ...
    if (kind == qcx::io::BuilderKind::kDirect && leanMember)
    {
        ...
        // legitimises. Measured on the same fixture, 8,811,481 screened
        // cells collapse to 2,379,972 ERI blocks - 0.2701 by count, 0.2873
        ...
        // CONTRACTION span RISES (~1.29x there). The net is
    ...
    if (kind == qcx::io::BuilderKind::kQfmm)
    {
        ...
        // The QFMM schema knobs reach QfmmOptions here: an absent key keeps
        // the engine default (the preset theta/order, leaf size 8, the
        // engine's crossover constant); the io
        ...
        // The adaptive seam, wired like the direct branch below:
        ...
        // The composed RHF builder (the RIJCOSX-style combination point):
        // F = H + 2J_QFMM(rho) - K(rho) -
        // the Coulomb half through the QFMM near/far split, the exchange
        // half through the exchange-only direct builder. It replaces the
        // exchange-less QfmmJBuilder (a J-only SCF).
        auto builder =
            qcx::integrals::QfmmHfFockBuilder::Create(molecule, basis, coreTensor, qfmmOptions);
        ...
        // The Kohn-Sham arm of this family. It replaces the
        ...
        // from a wiring defect in the trace.
        ...
        // The composed Coulomb half's own model record (schema 29): the
    ...
    else if (kind == qcx::io::BuilderKind::kRiJk)
    {
        ...
        // a full-RI exchange path - the defect the JK-fit check exists to
        // refuse.
        std::optional<std::string> auxName;
        ...
        // The JK-fit aux check, at the site that RESOLVES the name - the builder
        // cannot apply it, because a parsed BasisSet carries no name. This is the
        // refusal the validate pass and the run
        // to agree about, and it is spelled from the predicate's own message
        ...
        // The point-group reduction CARRIER: method.ri_orbit_expansion asked
    ...
    } else if (kind == qcx::io::BuilderKind::kRiJLink)
    {
        ...
        // estimate: the driver grants the cap as the workspace
        ...
        // The disk route's one construction site,
        // shared by the two admissions below so the wiring cannot
        ...
        // into the run record (mode_record.forced_disk, schema 18), which
        ...
        const auto engageDiskRung =
            ...
            // that is deliberate, not an omission: a pure SIZE HINT is DROPPED on a
            ...
            //
            // The DISCLOSURE half of the drop LANDS with this line as its
            // subject: the drop is recorded in resources_resolved.ri_chunk_bytes
            // (schema 23), filled by MakeRiChunkBytesRecord and named after the
            ...
            // dense, so 8 n^2 nAux is also the file's payload) and the budget
            // context the refusal left untouched. All other terms stay zero,
            // and with no budget
        ...
        // The orbit expansion, on the explicit key ONLY (schema 26).
        ...
        if (!builder.has_value())
        {
            // The disk rung's driver route: the explicit opt-in engages the storage-module
        ...
        wired.modeInfo = builder->ModeInfo();
        // The term-counter carrier: the run flow reads the run's
    ...
    // The adaptive seam, wired like the ri_j branch: the cap is the
    ...
    // The class-aware seam: the reduction extracted once - the
    ...
    if (isKs)
    {
        ...
        ksWired.exchangeModeInfo = std::move(exchangeModeInfo);
        ksWired.coulomb = std::move(seam->coulomb);
        ksWired.contribution = std::move(seam->contribution);
        return ksWired;
    }
...
qcx::Result<void> FillUhfRunOutcome(
    ...
    // The symmetry-blocking disclosure (schema 32): what this run did with
...
// the instrument THIS RunDriver call enabled is disabled on
...
qcx::Result<std::string> RunDriver(const qcx::io::RunInput& input) {
    ...
    const auto refuse = [](qcx::Error error) -> qcx::Result<std::string> {
    ...
    // the node/CPU/host/device bundle the D-A7 condition reads (an
    ...
    // The builder resolution: the no-builder
    ...
    // reads 31.2 while its host probe reads 2 - the measured inversion - and a
    // host number reaching a device builder
    ...
    // rename to `computeProfile` is an open decision, not taken here.
    ...
    // Every input the consult needs (the molecule, the parsed basis, the
    ...
    // the consult's bound charges one Schwarz sweep per basis in effect, and
    ...
    // aux error is the accurate one, and the consult must not pre-empt it -
    ...
    // retargeted D-A7 device-less gpu fallback: it lands on this machinery
    ...
    // The disk-tier ERI store (`method.eri_cache_store`, schema 33) on the
    ...
    // accepted and ignored: an explicit request the run drops is a
    ...
    // silent one is not (this is one increment of a larger seam).
    ...
    // The certified fp32 lane's REQUEST: the
    ...
    // the device arm did the same). On a machine whose probe
    ...
    // request ResolveCertifiedLane returns as written (the probe's
    ...
    //      (MixedPrecisionThreshold(kTight) is 0.0 by construction), so
    ...
    //      is ACCEPTED and runs vacuously where the lane is not engaged, but a
    //      request that names the LANE ITSELF cannot
    ...
    // The unit the geometry was READ under (schema 28): the parser's resolved
    ...
    // The run's own physics (schema 34). `method` is the word the
    ...
    // The within-family member name (schema 15): the family word above stays
    ...
    // The RESOLVED selection in the orthogonal axis vocabulary (schema 35), the
    // record's top-level `builder_axes` block.
    ...
    // The auxiliary-basis weak-region notice (schema 25): the disclosure
    ...
    // The exchange-approximation disclosure (schema 24). An ri_jk run's exchange
    ...
    if (selection->kind == qcx::io::BuilderKind::kRiJk)
    {
        ...
        // The approximation closes with its error
        ...
        // against (schema 31, RunExchangeError). This is the ONLY site that
    ...
    {
        // The notice-only arm (schema 25), and the arm
    ...
    // The certified fp32 lane's DECIDED record (schema 20): the
    ...
    if (isUnrestricted)
    {
        ...
        if (selection->kind == qcx::io::BuilderKind::kRiJLink)
        {
            ...
            // The requested-vs-ran pairing (schema 19) and its two sibling
            ...
            // The orbit-expansion request record (schema 26): the engine's own
            ...
            // The term_counters block, published on the same
        ...
        } else if (selection->kind == qcx::io::BuilderKind::kRiJk)
        {
            ...
            // (schema 30), the record the restricted leg publishes from its
            ...
            result.resourcesResolved.riChunkBytes =
                MakeRiChunkBytesRecord(input, selection->kind, run->modeInfo);
            // The orbit-expansion request record (schema 26): this arm carries
        ...
        } else if (selection->kind == qcx::io::BuilderKind::kQfmm)
        {
            ...
            // The requested-vs-ran pairing (schema 19): a UHF run has no
            ...
            result.resourcesResolved.riTensorMode =
                MakeRiTensorModeRecord(input, selection->kind, run->modeInfo);
            // The chunk-size hint's own disclosure (schema 23), filled from
            ...
            result.resourcesResolved.riChunkBytes =
                MakeRiChunkBytesRecord(input, selection->kind, run->modeInfo);
            // The orbit-expansion request record (schema 26): a UHF run has
    ...
    } else
    {
        ...
        // The certified-bound collector (schema 14): the same
        ...
        // The disk-tier ERI store's run-owned wiring (method.eri_cache_store,
        // schema 33): the direct family's machinery arm fills it, every other
        ...
        // The presence of the companion callbacks IS the
        ...
        // The ri_j budget-path audit: the cap-minus-base
        ...
        // The ri_jk builder's own rung decision (schema 30): the RI-K
        ...
        // The composed-QFMM model this run's Coulomb half ran (schema 29):
        ...
        // The requested-vs-ran pairing (schema 19): the one record that
        ...
        // The chunk-size hint's own disclosure (schema 23): the same one
        ...
        // The orbit-expansion request record (schema 26): the same shape for
        ...
        // The disk-tier ERI store's own disclosure (schema 33): the same
        ...
        // reads its stats - `integrals` cannot name `storage`). The
        ...
        // The term_counters block (the RI-J emission lane,
        ...
        // The certified mixed-precision bound block (schema 14): the
        ...
        //
        // Schema 16 adds the global budget enforcement's members to the
        ...
        // HfResult::density is the spin-summed D = 2 rho;
    ...
    return qcx::io::SerializeRunResultJson(result);
}
...
// The standing reinstatement clause of the ri_j budget path: the
// four options the driver's engine-composed refusals name, the light rung
// in the recompute slot and the disk-backed store always
...
std::string RiJReinstatementClause() {
    return " Reinstatement options: (1) raise memory_cap_gib in [resources]; "
           ...
           "method.ri_tensor_mode = \"disk\" (the disk rung, landed; it runs "
...
// follow are subset-sized. This is an ENGINE estimate, not a prediction:
// the analysis it gates needs the dense tensor and the estimate is the
// engine's own.
