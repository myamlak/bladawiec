...
// rulings of 2026-09-13: the no-builder default is a size LADDER - the
...
// where the policy is read (the owner's ruling: what is not measured "must
// say so wherever the policy is stated"). 1000 is the existing lean
...
// The ladder's rungs, in the order the ruling names them.
...
// The tier the size policy names for this count (the ruling's table, with
// its ONE exception - see the nonHybridKohnSham clause below).
//
// The ladder is the SAME for RHF, UHF, RKS and UKS (the owner's ruling,
// 2026-09-13: "all should have the same defaults (RHF/UHF/RKS/UKS)"), so the
...
// The ruling's own name for a tier: the text a demotion's reasoning and
...
// unrestricted legs through the unrestricted seam, and Kohn-Sham through the same
...
// run can wire (demote-with-disclosure arm). The demotion is stated
...
qcx::Result<ResolvedBuilderSelection> ResolveBuilderSelection(
    ...
    // member choice - the device-less fallback lands on the ladder's tier.
    ...
    // The device-less fallback GPU auto-fallback, retargeted to the ladder: an explicit
    ...
    // byte-for-byte (Ruling B - all old pins untouched).
    //
    ...
    // reasoning claimed the machinery - the silent substitution Ruling B
    // forbids, caught by the record test (the member name read "lean" for an
