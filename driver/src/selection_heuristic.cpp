...
// The stub reason of the non-single-node columns: MPI is not wired and
// nothing distributes a Fock build today.
...
// store. The seeds are the SelectionCostConstants fields; Package C's
// device-budget mirror measures the real terms.
...
// model on 2026-09-17: its trigger was the model's floor, so it had none.)
...
// deleted with the model on 2026-09-17: the job-object cap is the
...
CandidateEstimate Evaluate(const SelectionInput& input,
    ...
    if (!wired)
    {
        ...
        estimate.reason = "deliberately excluded from the ranking: ri_jk is an "
                          ...
                          "(owner ruling 2026-09-16) rather than proposed here";
        return estimate;
    }
...
// builder. The nullable RESOLUTION the disclosure rule cites as the shape to copy for a
// selector that must be able to answer "there is nothing to run here" - and,
// per the same record's standing obligation, a resolver whose switch lacks
...
std::optional<qcx::io::BuilderKind> BuilderKindOf(CostCandidate candidate) noexcept {
    ...
    // NOT because the builder is missing - it exists since it landed and runs on both
    // legs since 2026-09-16 - but because ri_jk is an APPROXIMATED-exchange
    // path: it is opted into by name and never proposed by the ranking (owner
    // ruling 2026-09-16). Removing this nullopt would let the heuristic route
...
SelectionRanking EstimateCandidates(const SelectionInput& input) noexcept {
    ...
    // memory model on 2026-09-17. A candidate is no longer excluded from
    ...
    // gpuSpeedup/(1+gpuSpeedup) - the batch-partition slice-B measurement replaces
    // the default zero gate with that value. At the default g = 0 the
    // suffix is empty and the row IS the CPU-only fallback - the k = 1 k=1
    // path - so it prices exactly the direct row (LightPath pricing
    // included, no overhead: nothing rendezvouses) and can never outrank
...
double GpuProbeFloorGiB(const SelectionCostConstants& constants) noexcept {
    ...
    //
    // The memory model's base term was added to this sum until 2026-09-17,
    // when the model was deleted; the term is gone with it. The gate is
