// Acceptance: RunDriver reproduces the
...
// bottom tier (the lean Schwarz-only member of the direct family).
// The explicit "direct" of kH2Toml stays the machinery at every size,
// so the two fixtures are the flag's true/false pair. The SCF
// thresholds are the OPERATING defaults (1e-8/1e-6): the standing rule
...
// side, and it is the whole of this file's coverage of the
// lean-word wiring: the above-ceiling discrimination (1210 basis functions)
// lives at the ladder's own edges in selection_resolution_test.cpp and
// selection_reporting_test.cpp, and a converged above-ceiling lean run stays
// ladder-harness coverage.
...
// kH2oToml with the certified-bound routing threshold switched from kTight's 0.0 to
...
// and 9.43e-8 away - 14x and 11x inside. The tight pair was a note about when
...
// lands on the same pinned ground state (the UHF landscape rule - the
...
// per-spin adapter was wired here. The SAD seed and the per-spin DIIS are
...
// kO2Toml's explicit 1e-10 pair: that pair is not a rung of the gate
// ladder. The default-gate
...
// fock_builder = "ri_jk", the family whose per-spin adapter is wired here.
...
// raised rather than the gate loosened (the SCF thresholds are not to be tuned
...
// The auto-lean mirror of kO2Toml (the UHF seam): the identical
...
// The composed-QFMM driver rows' [method] retune: a copy of a base
...
// geometry (MakeAlkaneSto3g - the shared sweep fixture, so every text
...
// certified-bound routing threshold 1e-10) with the OPERATING SCF
// thresholds (1e-8/1e-6 - not the sibling chain helper's 1e-10 gate). The
...
// from a dead one. Measured on this fixture family (2026-09-11):
// methane (9 basis functions) routes zero quartets at
...
// The enforcement fixture with the lane FORCED on (schema 21): the same
...
// key alone runs vacuously (a permitted no-op, and the reading
...
// The alkane chain TOML of the chain pins: an n-carbon STO-3G RHF
// text over the shared fixture geometry, the [basis] sto-3g name resolving
// from the compiled-in basis data.
//
// THE SCF GATE IS THE OPERATING DEFAULT (1e-8 energy / 1e-6 density), not the
// 1e-10/1e-10 pair these rows were once hardened to: the standing rule bans a
// 1e-10 energy or density threshold without raising it first, and
// 1e-10/1e-10 is four orders below the 2e-4 band the kLoose rows validate
...
// The method-word comparison pair: ONE molecule block (H2 at 1.4 bohr -
...
// every run takes the operating defaults (1e-8 energy / 1e-6 density). The only
...
TEST(DriverPinTest, H2DirectRhfReproducesThePin) {
    ...
    // Schema 13's lean flag and schema 15's member name. This run spells
    // fock_builder = "direct" explicitly, so it takes the machinery branch
    // at every size: the flag stays false and the member name
    ...
    // The coefficients block (schema 11) is a RETIRED PRODUCER: the candidate
    // ranking that filled it died with the lean flip, so no driver path sets
    // `coefficients` and the serializer's
    ...
    // The populations/moments block: the H2 STO-3G pins of populations_test.cpp /
...
// (schema 13). The same H2 fixture with no [method].fock_builder key: at
...
// potential into the Fock build. Until this wiring the dispatch REFUSED both
...
// fixture, at the operating [scf] defaults (no [scf] block is written). A
...
TEST(DriverPinTest, RksSlaterH2RunsTheKohnShamPathOnBothBuilderMembers) {
    ...
    // branch at every size - a different integral engine behind
    // the same halves.
...
// ---------------------------------------------------------------------------
// The record names the run's OWN PHYSICS (schema 34) - which method
...
// not a formatting one: one defect was that a record named its Fock builder
// and no physics (two runs differing by the whole RKS-vs-RHF difference were
// otherwise indistinguishable in the artifact a user keeps), and the other was
...
// The ladder's ONE exception, end to end: a
...
TEST(DriverPinTest, PureKohnShamOrderRunsThePromotedTier) {
    ...
    // promotion, no demotion - the unchanged record byte for byte. And it
...
//     the chain pins), and no Kohn-Sham cell here reaches that size.
TEST(DriverPinTest, RksQfmmMatchesTheDirectFamilyOnTheSameDeck) {
    ...
    // The QFMM model the Coulomb half ran (schema 29): a Kohn-Sham run on
...
//
// qfmm LEFT this list (the composition over the composed
...
// its unrestricted twin below. ri_jk LEFT it the same way
...
TEST(DriverPinTest, ThreadCapOneDegradesToTheD59SerialPath) {
    ...
    // the same cap-1 input run twice produces the same energy bits
...
TEST(DriverPinTest, H2QtaimEndToEnd) {
    ...
    // The unrequested block is absent from the document (the presence rule's
    // absent case), never null.
...
TEST(DriverPinTest, RhfGwhGuessMatchesTheCoreDefault) {
    ...
    // that same start. This guards the driver seam, not the SCF (the loop
...
TEST(DriverPinTest, H2oDirectRhfReproducesThePin) {
    ...
    // The moments/populations block through the driver path: the H2O dipole (multipoles_test
...
// The certified-bound observability (schema 14): the machinery
...
//
// A's and B's numerical readings are the fp32 LANE's, and the
...
// unconditional.
//
// The certified fp32 lane's expected state for this process: the driver probes
...
// The certified lane's DECIDED record (schema 20): the compute
...
// reached, and the no-CUDA fallback resolves an explicit "gpu" request to a CPU
...
TEST(DriverPinTest, CertifiedBoundReportsTheMachineryLaneAtKNormal) {
    ...
    // state is this run's own probe verdict
    ...
    if (CertifiedBoundLaneExpectedOn())
    {
        ...
        // (measured 2026-09-11: methane 0, butane 17056), so
...
// The ri_jk run path end to end: a run record whose
// selection.builder is "ri_jk". The request is EXPLICIT, so it resolves
// verbatim (the size ladder governs the ABSENT key only), wires the
// composed full-RI builder, and reports both the family word and the member
// word the family word cannot state (the contraction form, not the
// family restated).
//
...
// never reach a run - the aux-quality rule refuses it - so the run existing at
// all is part of what this row proves.
//
// FALSIFIABILITY: the request used to be refused by name at the top of
...
// fall back to echoing the family word - the drift this check exists to prevent.
TEST(DriverPinTest, RiJKWiresTheComposedFullRiBuilderEndToEnd) {
    ...
    // The approximation disclosure (schema 24), which is the OTHER half of what
    ...
    // The MEASURED-error disclosure (schema 31): the operative half of the
    // rule that closed this work, which is that it closes with its error
    ...
    // schema 25 describes is pinned by
...
// The weak-region aux notice AT THE RUN PATH (schema 25): the disclosure
...
//
// The defect this cell closes, named: until this wiring, `AuxSelectionNotice` had
...
//
// The fixture is STO-3G: a minimal basis is the region the aux measurement
// actually characterizes (6.99x the def2-svp control, the strongest residual),
// and it is a weak region for BOTH aux consumers,
...
TEST(DriverPinTest, TheAuxWeakRegionNoticeReachesTheRunRecord) {
    // Arm 1 - the exact-kernel notice block (schema 25), which is the arm that
    ...
    // no policy chose it and there is nothing to disclose: the disclosure
...
// The aux-quality rule at the run path: a JK request whose auxiliary basis IN EFFECT is
...
TEST(DriverPinTest, RiJkRefusesAnAuxThatIsNotAJkFit) {
    // The aux-quality rule at the RUN path. It has exactly ONE reachable door
    // since the aux basis rule became a quality tier (5b21a295): the aux
    // auto-selection now answers a JK request
...
// The certified-bound budget enforcement end to end (schema 16): the [method]
...
// The certified fp32 lane's FORCE-ON input (schema 21):
...
// SOURCE - the host-ratio lane forced DetectHostComputeProfile to 31.2, the
// device arm did the same for the device profile - so on a machine whose probe resolves
...
// The other half of the contract: THE PROBE STILL DECIDES THE DEFAULT. The
...
// The same key closes the OTHER half of the counterfactual. The
// record notes that the same binary with the probe return forced to
...
// off, which is a permitted no-op and says nothing about the fall-back the budget is
// there to force.
//
...
// The energy the lean arm delivered BEFORE the UHF seam engaged:
...
// SUPERSEDED as a pin: with the seam engaged the same
...
TEST(DriverPinTest, O2UhfSadDirectReproducesThePins) {
    ...
    // The populations block through the driver path: each oxygen carries one unpaired
    ...
    // The explicit family word keeps the machinery member: the
    // UHF seam is reached by the lean selection alone, so the record
...
// low-order bits - varies run to run above a team of one. This fixture's
// gate is 1e-10 and the serial path converges at 9.756e-11, 2.4% under it,
// so a reordered accumulation flips the record's "converged" flag: measured
// 2026-09-16 at this machine's resolved 5-thread team, 6 of 10 runs reported
// it, against 20 of 20 bit-identical at one thread. Loosening the gate to
// settle that is NOT available: at 1e-8/1e-6 the spin_squared diagnostic
// lands 1.3-2.6e-5 from the pinned 0.753257, outside its 1e-5 assertion.
// One thread removes the ordering freedom, which is the whole of the fix.
...
TEST(DriverPinTest, OhUhfMatchesTheExternalReferenceAndStaysUnderTheRohfBound) {
    ...
    // The pin and its band, both MEASURED at the gate declared above. The
    // 1e-10 gate's record (0.753257, the pyscf reference at conv_tol 1e-12
    // quoted above) is not the pin any more: at 1e-8/1e-6 the density fixed
    // point is a different one, ~1e-5 away. Measured at the standard gate,
    // thread_cap = 1, one value per CI leg that runs it (2026-09-19):
    //   0.75323082129312269  linux-arm64 gcc Release
    //   0.75323082129312269  macos arm64
    //   0.75323082340785552  linux-x86 gcc Release
    //   0.75323082340785552  macos x64
    //   0.75323082340785552  windows-msvc Release
    //   0.75323082340785552  linux-sanitizers, both asan and ubsan
    //   0.75323082366141048  windows-arm64 msvc Release
    // so the legs disagree in the ninth decimal (spread 2.4e-9; the two gcc
    // arm64 legs, and separately the two apple arm64 legs, bit for bit). The
    // pin is the CENTRE of that spread at eight decimals, deliberately not
    // one leg's bit pattern, and the band is 1e-7: 42x the observed spread,
    // 100x tighter than the ~1e-5 the gate move is worth, and 3.2e4x tighter
    // than the distance to the ROHF bound (0.750000). The check is therefore
    // WEAKER than the 1e-15 record it replaces - a spin error below ~1e-7 no
    // longer fails here - and that is what the looser gate costs; the ROHF
    // bound above is the assertion that still catches a wrong solution, at
    // any gate.
    EXPECT_NEAR(*ohSpin, 0.75323082, 1e-7);
...
// The UHF seam, end to end and through the run record: the O2 triplet
...
TEST(DriverPinTest, O2UhfAutoLeanWritesTheLeanMember) {
    ...
    // ORDERING, not an absolute band: the
...
TEST(DriverPinTest, O2UhfComposedQfmmReproducesTheDirectUhfPin) {
    ...
    // The composed-UHF model disclosure (schema 29): the UHF runner carries
    ...
    // here as the negative surface-ball buffer with theta 0.0: at kTight the
    // absent-theta default keeps the exact near-field-only rung rather than
    // silently acquiring a live far field, and the record is where that shows
    // up in the document.
    EXPECT_NE(result->find("\"qfmm_model\""), std::string::npos);
    EXPECT_NE(result->find("\"extent_model\": \"kProductBall\""), std::string::npos);
    EXPECT_NE(result->find("\"separation_mode\": \"kSurfaceBall\""), std::string::npos);
    EXPECT_NE(result->find("\"separation_k\": -1.0"), std::string::npos);
    ...
    // The populations block through the driver path, asserted like the direct pin
...
// wired here, MakeRiJLinkUhfFockBuilder over RiJkFockBuilder's two exposed
...
// Before this wiring the request never reached a run at all (the UHF
...
// wired here, MakeRiFullUhfFockBuilder over
...
// anywhere to write. Before this wiring the request never reached a run at all
...
TEST(DriverPinTest, UhfRiJkWiresThePerSpinAdapterEndToEnd) {
    ...
    // RESOLVED kind (schema 24), never from the SCF leg - so this run owes it
    ...
    // disclosure (schema 31) rides the same block - and its fixture is a
    // CLOSED-SHELL water cell, which is the open item recorded here rather
    ...
    EXPECT_NEAR(std::abs(mullikenSpin->values[0] - 1.0),
                std::abs(mullikenSpin->values[1] - 1.0),
                1e-12)
...
TEST(DriverPinTest, UhfRiJkMatchesTheRestrictedLegOnAClosedShellDeck) {
    ...
    // MEASURED on this fixture - see the printed line
    // above for the value it was taken at. The band is the runs' own energy gate
    // (the operating default, 1e-8), not
...
TEST(DriverErrorTest, RejectsUnwiredCombinations) {
    ...
    // wired in v1 (no full-RI exchange path exists)". The composed full-RI
    // builder and its run path answered that message's first clause, so ri_jk
    // is no longer an UNWIRED combination and does not belong in
    // this test at all. Its replacements are all in DriverPinTest: the run
    // (RiJKWiresTheComposedFullRiBuilderEndToEnd), the aux-quality refusal
    ...
    // those whitelists never had to cover this kind
    ...
    // commit: the universal aux fallback (5b21a295) removed the "no
    ...
    // (never a dropped rung request), for the explicit family word;
...
force_disk_ri = true
    ...
    // energy under a DFT label. The composition landed
    ...
    // The lean spelling is no longer a UHF rejection (the UHF seam
...
type = "sad"
    ...
    // path builds in v1 (gwh is wired for RHF; sad stays UHF-only); the
...
TEST(DriverErrorTest, RksRiJkRunsTheComposedFullRiCompositionAndDisclosesIt) {
    // The ri_jk opt-in's last REFUSED cell became a RUN, and this
    // row is that cell's new home - the same displacement this test's UHF arm
    // went through when its own composition landed, and by the same rule: a refusal cell
...
accuracy = "kNormal"
    ...
    // The family and the member word (the contraction form, not the
    ...
    // it (schema 24) and states the error it is to be read against (schema 31),
    // and a consumer must never read this energy as one built from exact
    // quartets. The engine's own rung decision rides beside it (schema 30).
...
accuracy = "kNormal"
    ...
    // refuses: it classifies and runs. The pin therefore
...
// The REACH test for the Kohn-Sham dispatch, and the evidence that licenses
// the removal of
...
// earlier dispatch produced exactly that, an HF energy under a DFT label.
...
//
// Positive control (the red half): with
// ResolveScfPath's two Kohn-Sham cases reverted to kRestricted/kUnrestricted -
// the exact earlier dispatch, which selected its method by equality - all
...
accuracy = "kNormal"
    ...
    // Leg 3 - the combination whitelist, the other refusal that exists only because
    ...
    // on its own.
    //
    // qfmm was the family named here, and ri_jk after it, and
...
TEST(DriverErrorTest, GpuSplitRequestIsRefusedByNameNotRunAsDirect) {
    ...
    // lane's artifacts): the call returned a COMPLETE document whose
...
TEST(DriverErrorTest, TightCapRefusesBeforeThePreGateSetupRamp) {
    // The above-ceiling crash class, closed, on the above-ceiling fixture:
    // at C50H102/def2-SVP the driver's shared setup ramp
    ...
    // pre-gate lane's measurement at 1210 basis functions), so a cap under
    ...
    //
    // 0.50 GiB is INSIDE that band, and it is DERIVED rather than inherited.
    // The admission does not read this host's own plan any more: it takes the
    // row-window plan the ceiling can afford - the widest that clears it, down
    // to the one-window floor (ChooseCeilingPlan) - and refuses only when no
    // plan fits. So the smallest envelope ANY plan can achieve is the
    // one-window plan's, and at this fixture (1210 basis functions, 183921
    // canonical pairs) that floor reads 1,301,211,224 B = 1.212 GiB - the
    // number the two arm64 CI legs printed for required_peak_bytes
    // (2026-09-19, teams of 3 and 4, identical to the byte) - against
    // 1,298,234,592 B = 1.209 GiB on the 5-thread development host, 0.23 %
    // apart, so the floor is a property of the fixture and the plan rather
    // than of the host. Its parts: 49 x 8n^2 = 573,927,200 B of SCF matrix
    // state and DIIS history at n = 1210, the retained shell-pair state, one
    // wave slot's 8n^2 = 11,712,800 B accumulator, one live window's
    // 43,368,672 B of scratch, that window's retained store (windowStoreBytes,
    // 192 MiB) and the pair store's own materialization (skeleton plus one
    // kPairChunkBytes prep chunk). Both plan-derived terms are at their
    // minimum at one window - measured exact on this tree's own code for
    // window counts 1..5 (2026-09-19, the 5-thread development host):
    // +11,712,800 B per extra wave slot and +244,695,264 B per extra live
    // window. The host's own plan, by contrast, is charged per host:
    // 1,849,165,752 B at 3 threads (macos arm64), 2,117,286,616 B at 4
    // (linux-arm64 gcc Release) and 2,382,430,848 B = 2.219 GiB at 5 - so no
    // host can charge less than the floor, whatever its core count.
    //
    // A plan fits under a cap C while its estimate stays at or under
    // C x kRefusalSlack (1.5, the constant the refusal text names), so the
    // smallest cap that admits ANY plan is the floor / 1.5 = 0.808 GiB. At
    // 0.50 GiB the product is 805,306,368 B - 62 % of the floor and 7.2 %
    // under that boundary - so the refusal at this cap is genuine
    // infeasibility on every host, never this machine's plan meeting a cap,
    // and the child's own refusal line carries the number it decided on
    // (required_peak_bytes, the selected plan's total), so the reading is
    // checkable after the fact rather than inferred.
    //
    // 0.50 GiB is also the value the sanitizer leg has already carried this
    // child under: at this cap the refusal lands in 0.52 s on
    // linux-sanitizers asan, 0.37 s on ubsan and 0.50 s on macos x64
    // (2026-09-19, CI run 35451596080), so an instrumented child reaches the
    // admission well inside the bound twenty lines down - which is why this
    // fixture did not go lower.
    // And it is the test's whole point: the ramp cannot fit it at this size
    // at all - its
    // pair data alone is 441,172,808 B and its measured commit is 582.1 MiB -
    // so a run that reaches a refusal here has provably decided before the
    // ramp allocated anything: no timing or allocation-order claim is needed,
    // the arithmetic is the proof. Before the fix this exact invocation
    // exited 0xC0000409 (3221226505) after 1.7-2.8 s with the marker never
    // written; now both legs refuse in ~0.2 s together and the refusal names
    // the placement (the pre-gate setup admission's clause).
    //
    ...
    constexpr double kWithinTheRampCrashBandGiB = 0.50;
    // Bounded for the same reason every cap fixture here is: a cap that
    // admitted a leg would put this child into a real SCF, and the harness's
    // own 2400 s ceiling then reports nothing about which leg and which cap
    // did it. A stopped child is -2 and this ASSERT prints the marker.
    constexpr int kCapChildWaitMs = 300000;
    const int exitCode = qcx::testing::RunCapChild(
        kWithinTheRampCrashBandGiB, marker, "lean_above_ceiling", kCapChildWaitMs);
    ...
    // and the ramp is what actually stops the run here.
    ...
    // ladder (its own edges are pinned by selection_resolution_test.cpp and
    // selection_reporting_test.cpp, in the routine selection), and this leg is
...
// The key split on the four QFMM tuning knobs: a
...
// while the document asked for a screening it never got - the defect
...
//
// Two boundaries are this row's own, because the rule turns on both:
//   - the key PRESENT refuses and the key ABSENT runs (an absent
...
//     ladder's, never a family name: an absent
...
TEST(DriverErrorTest, QfmmKnobsOnAnotherFamilyAreRefusedByName) {
    ...
    // fock_builder resolves to at this fixture size (the
...
crossover_basis_function_count = 1000
    ...
    for (const auto& test : cases)
    {
        ...
        EXPECT_NE(result.error().message.find(test.messageFragment), std::string::npos)
            << "message: " << result.error().message;
        // The ladder's spelling of the absent-key case:
        // this assertion pinned "the direct family (no fock_builder was
        // given" until the ladder landed (afca4a2e), and that
...
theta = 0.5
    ...
    // Leg 4 - the ABSENT key, the other half of the rule: the same fixture
...
accuracy = "kNormal"
    ...
    // fock_builder = "qfmm" is NOT refused. The numbers are the passthrough
    // pin's (H2QfmmRhfWithSchemaKnobsRunsToConvergence above);
...
TEST(DriverRunTest, TheQfmmRunRecordDisclosesTheModelThatRan) {
    // The disclosure the default flip created (schema 29).
    ...
    // These legs are the two states a reader must be able to tell apart: the
    // flipped default and the retired pair an explicit theta still selects.
    //
    ...
    // test the surface ball at its default buffer; theta reads 0.0 because
    // the width test is not what separated the tree - the engine records the
    // parameter of the test that RAN and zeroes the other one.
...
accuracy = "kNormal"
    ...
    EXPECT_NE(defaultRun->find("\"separation_mode\": \"kSurfaceBall\""), std::string::npos);
    EXPECT_NE(defaultRun->find("\"separation_k\": 1.0"), std::string::npos);
    EXPECT_NE(defaultRun->find("\"theta\": 0.0"), std::string::npos);

    // Leg 2 - the record follows the BUILD, not the default. An explicit
    // theta > 0 still selects the retired centre-to-width test (the historic
    // contract: no written accuracy control is silently ignored by the flip),
    // so the same fixture with theta = 0.5 must come back naming that test
    // with its own parameter, and separation_k must read the 0.0 of the test
    // that did not run. A record hard-wired to the default pair passes leg 1
    // and fails here.
...
accuracy = "kNormal"
    ...
    // explicit theta moved the separation test and must NOT have moved the
    // extent model: the same product-ball geometry ran under both tests, and
    ...
    // because it is not obvious: theta < 0 does not select the width word. It
    // resolves the empty far field by driving the SURFACE test's buffer
    // negative (the form that also carries every absent-theta run at a preset
    // whose own rung is the gate), so the record reads kSurfaceBall with a
    // negative separation_k and theta 0.0. A reader who took a negative theta
    // to mean "the width test ran" would misread this build.
...
theta = -1.0
    ...
    EXPECT_NE(gate->find("\"separation_mode\": \"kSurfaceBall\""), std::string::npos);
    EXPECT_NE(gate->find("\"separation_k\": -1.0"), std::string::npos);
...
TEST(DriverPinTest, H2QfmmRhfWithSchemaKnobsRunsToConvergence) {
    ...
    // kQfmm branch wires the composed builder (F = H + 2J_QFMM - K),
...
use_diis = true
    ...
    // The composed-QFMM RHF run joined the budget path: the
    // default 16.0 GiB cap becomes a workspace budget of the cap MINUS the
...
TEST(DriverPinTest, H2oQfmmComposedRhfReproducesThePinAtAllPresets) {
    // The composed-RHF H2O gate: the qfmm
...
TEST(DriverPinTest, C6ChainQfmmRhfMatchesDirectInsideThePresetBudgets) {
    // The chain pins' kTight rung, on the smallest fixture that carries
    // it (a correctness claim is
    ...
    // at this fixture (44 functions) on this tree 2026-09-18, at the operating
    ...
    // gate, and qfmm_model.separation_k says so (-1.0 = nothing well
    // separated, the far field empty). A run whose far field was live would
    // carry a positive k here, and this row's comparison would no longer be
    // the split pass' alone - which is the reading the row depends on.
    EXPECT_NE(qfmmResult->find("\"separation_k\": -1.0"), std::string::npos)
        << "the composed run must record the degenerate gate";
...
TEST(DriverPinTest, C12ChainQfmmRhfMatchesDirectInsideThePresetBudgets) {
    // The chain pins on C12H26/STO-3G. The QFMM J-half budgets
    // {1e-5, 1e-8} Eh for {kLoose, kTight} (the kTight rung re-derived
    // from 1e-9 - the ladder is
    ...
    // The kLoose row therefore compares the
    ...
    // class). The kLoose rung's far field is live on this fixture (14 far
    // pairs at the (0.45, 1e-6) winner - the CSV record; asserted at the
    // builder level in
    // QfmmHfBuildTest.C24KeepsItsFarAliveRungsAliveAtCreateTime). The
    // kNormal rung is the VACUOUS C12 tooth (all-near, bit-exact by
    // construction), so its pin lives on C24
    // (C24ChainQfmmRhfMatchesDirectInsideThePresetBudgets).
    //
    // WHAT THIS FIXTURE IS FOR, and why it is not smaller (a correctness
    // claim is asserted on the
    ...
    // class grows with the basis. Measured on this tree, each at the operating
    ...
    // measured exchange-class budget 1e-4 (the 1e-5 record bounds the
    ...
    // LIVE in the run whose number the rows below read (qfmm_model.
    // separation_k = 1.0, the surface-ball buffer; -1.0 would be the
    // degenerate gate). Without it the composed run would be a near-field
    // build and the kLoose row would assert nothing about the multipoles.
    EXPECT_NE(qfmmKlooseResult->find("\"separation_k\": 1.0"), std::string::npos)
...
TEST(DriverPinTest, C24ChainQfmmRhfMatchesDirectInsideThePresetBudgets) {
    // The C24H50/STO-3G half of the chain pins. C24 carries the
    // NON-vacuous kNormal rung (499 far pairs at the (0.3, 1e-8) winner -
    // the C12 tooth is all-near there), the strongest kLoose
    // exercise (1997 far pairs), and the kTight degenerate rung. Budgets
    // and references follow the C12 test's, with two
    ...
    // single tight constant serves both fixtures. The composed side is the more
    // accurate one at both: it reproduces the kLoose direct to the QFMM
    // scale at C12 (3.2e-8) and beats it by 1.9972e-5 at C24 - the
    // direct's own screened-J far-region drop, the pairs the composed
    // path's multipole far field keeps (the ladder-sweep CSV's C24-kLoose J-half
    // record, 3.84e-7 at a fixed reference density, is NOT an SCF-level
    // bound: the converged density shifts under the exchange half's
    // screening and carries the SCF deviation up to the class). The
    // kLoose rows therefore assert the C24 exchange-class budget 2e-4
    // (1.5x the measured composed deviation), the discriminator band on
    // the kLoose direct's own class, and the asymmetry. Second, the
    // kNormal row CAN use the same-preset direct as its reference (sound
    // at this preset: the K halves are identical code at an equal preset,
    // and the CSV's C24-kNormal J-half error is 1.9e-9, over an order
    // under the 1e-7 budget).
    //
    // WHY THIS FIXTURE HAS NO SMALLER STAND-IN, measured on this tree
    // 2026-09-18. A far-field row is only a test
    ...
    // default and the far field's effect on THIS fixture are printed
    // by the runs below.
    //
    ...
    // C12's), the discriminator band on the kLoose direct's own class
    // (keeps the budget's basis live-checked on every gate run, as on
    // C12), and the asymmetry assertion: the composed run must sit
    // strictly closer to near-exact than the kLoose direct itself does -
    // its margin 1.9972e-5 is the direct's screened-J far-region drop,
    // which the composed path's multipole far field keeps (1997 live far
    // pairs, per QfmmHfBuildTest.C24KeepsItsFarAliveRungsAliveAtCreateTime).
    const auto qfmmKlooseToml = AlkaneChainToml(*molecule, "qfmm", "kLoose");
    auto qfmmKlooseResult = RunInputText(qfmmKlooseToml.c_str());
    ASSERT_TRUE(qfmmKlooseResult.has_value()) << qfmmKlooseResult.error().message;
    EXPECT_NE(qfmmKlooseResult->find("\"converged\": true"), std::string::npos);
    ...
    // The kLoose row's rung, from the run's own record: the far field must be
    // LIVE (qfmm_model.separation_k = 1.0, the surface-ball buffer; -1.0 is
    // the degenerate gate the kTight row runs). The discriminated rows below
    // are read as a statement about the composed path's far field, so a
    // degenerate run must fail this rather than quietly pass them.
    EXPECT_NE(qfmmKlooseResult->find("\"separation_k\": 1.0"), std::string::npos)
        << "the kLoose composed run must record a live far field";
    ...
    EXPECT_LT(klooseNearExactDeviation, 2e-4) << "preset kLoose";
    EXPECT_LT(klooseNearExactDeviation, directLooseDeviation)
        << "preset kLoose (the composed side must beat the kLoose direct's own deviation)";
    ...
    // asymmetry the composed side wins by 1.9972e-5), and the CSV J-half
    // record is not an in-run measurement - so the line prints the bin
    // and the composed deviation only.
    {
        qcx::driver::QfmmAccuracyRunRow bandRow;
    ...
    // LIVE in the run that produced the number below (qfmm_model.
    // separation_k = 1.0, the surface-ball buffer; -1.0 would be the
    // degenerate gate the kTight row runs, and a run that degenerated here
    // would compare two near-field builds and prove nothing about the far
    // field). The differential evidence that this fixture is the smallest
    // one where that live far field MOVES the answer - the far field's own
    // contribution to the converged energy - is in this test's header note.
    EXPECT_NE(qfmmKnormalResult->find("\"separation_k\": 1.0"), std::string::npos)
...
TEST(DriverErrorTest, GpuWithoutCudaFallsBackToTheHeuristicPick) {
    // The no-CUDA fallback: an explicit "gpu" without a CUDA device no longer refuses the
...
accuracy = "kNormal"
    ...
    // That refusal is gone BY DESIGN - the aux rule became a quality tier that
    // ALWAYS
    ...
    //
    // Note the attribution, because it is not this file's ladder: the aux rule is
    ...
    //
    // PINNED SINCE THE NOTICE'S PRODUCER LANDED. This leg used to
    // say the NAME of the resolved aux was NOT pinned and could not be:
    // "The record plumbing for that (AuxSelectionNotice) is open and unlanded,
    // and the warnings the rule asks
    // for are structurally blocked, so nothing on this run's face says which fit
    // was chosen." That sentence was true when it was written and is false now,
    ...
    // basis, the strongest residual in the measurement the aux rule rests
...
accuracy = "kLoose"
    ...
    // The budget path: the default 16.0 GiB cap becomes
    ...
    // engine's enumerator vocabulary.
    EXPECT_NE(result->find("\"mode_record\""), std::string::npos);
    EXPECT_NE(result->find("\"mode\": \"kFastPath\""), std::string::npos);
...
// The symmetry-blocking disclosure (schema 32) at the driver level: the
// run record's own account of the guard's decision, per spin channel. The
// fixture is a measured probe
// (build/windows-msvc/probe/h2-3.5.toml, verbatim): stretched H2/STO-3G from
...
// on: O2/STO-3G read as a SINGLET (multiplicity 1, so both spins start from
...
//
// It is the word a reader must tell apart
...
type = "core"
    ...
    // information, so the labeling blocks are absent for the same C1 cause.
    EXPECT_EQ(result->find("\"symmetry\""), std::string::npos);
    EXPECT_EQ(result->find("\"symmetry_beta\""), std::string::npos);
}

// The disk-rung knob through the error surface: the io layer's vocabulary
// rejections run
...
// table ([diagnostics] force_disk_ri) and
...
// The other side of the key split:
...
// the pedantic reading the rule rejects, and the alternative it forbids is
// not refusal but silence about a key the run did use differently.
//
// Both halves of that rule are landed. The DISCLOSURE is
// resources_resolved.ri_chunk_bytes (schema 23): the size that was asked for,
...
// is the state this comment used to describe as OWED, and the paragraph
// here is rewritten rather than deleted because a reader who finds the
// earlier text quoted in an older commit should see where it landed. The
...
//
// What this row pins is therefore the PROCEED half - the half the rule
...
ri_chunk_bytes = 134217728
    ...
    // The record half (schema 23): the document names the key, what it asked
...
// The orbit-expansion key on a family with no 3c task grid (schema 26): the
...
ri_orbit_expansion = true
    ...
    // The record half (schema 26): the document names the key, what it asked
...
// The requested-vs-ran pairing at the driver (schema 19): a request
...
// by-name refusal this wiring landed: its assertions
...
TEST(DriverRunTest, RiTensorModeRequestsOnAnotherFamilyAreRecordedNotDropped) {
    ...
    // word): the request asserted which
...
// nAux = 71, so the disk record's modeled dense payload (the
// reconciliation target) is 8 * 49 * 71 = 27832 bytes. The 0.1 GiB cap comes
// from direct probes (2026-09-03): the driver's base-term
...
TEST(DriverRunTest, RiTensorModeDiskEngagesTheDiskRungAfterTheLaddersRefusal) {
    ...
    // follows the record's own outcome, one branch each, rather than the plan.
...
use_diis = true
    ...
    // The request/ran pairing (schema 19) names this case as what it is: a
...
use_diis = true
    ...
    // ... and the request/ran pairing (schema 19; schema 22 names the KEY
...
//
// THE CONTRACT FLIPPED, and this cell's own
...
TEST(DriverErrorTest, OverCapDirectRunProceedsUnderTheRealCap) {
    ...
    // load-time footprint also exceeds the tiny cap (the same load-time wedge class). The
...
// The ri_j budget path: the driver grants the engine a
...
//
// THE CONTRACT FLIPPED: the withheld budget is
...
// of the model stopping it at zero. Measured under the new contract: this
// run COMPLETES under the 0.045 GiB cap (exit 0, "completed"), which is also
// the sharpest evidence for the change at this fixture - the model's 0.056
...
TEST(DriverErrorTest, RiJProceedsWhenTheCapDoesNotClearTheBase) {
    ...
    // universal-J aux carries f shells. Under the new contract the over-cap
    ...
    // (the earlier refusal at cap_child.cpp shielded these shells, which is
...
// The orbit-expansion request ON THE ri_jk FAMILY (the mechanism key):
// the driver CARRIES the request to the
// composed full-RI builder, which refuses it BY NAME.
//
// The side is decided by what the key NAMES (the key split):
// method.ri_orbit_expansion names a MECHANISM - a point-group
...
// counted but never reduced - accepted-and-silently-ignored is the
...
ri_orbit_expansion = true
    ...
    // The SAME request on the UNRESTRICTED leg (the other runner), which
...
ri_orbit_expansion = true
    ...
    if (qcx::integrals::SupportsL(3))
    {
        ...
        // reader state of the schema-26 rule, here on the family this row
        // wires the carrier for.
        EXPECT_EQ(control->find("ri_orbit_expansion"), std::string::npos);
    }
}

// The ri_jk budget path (the RI-K memory ladder's driver half): the driver
// grants the engine a workspace budget and the engine's
...
//
// THE CONTRACT FLIPPED (the ri_jk half of the
...
// engine's own rung decision, with the OS enforcing the cap (the same
...
// the cap the family was refused at - the case, in miniature.
//
...
TEST(DriverErrorTest, RiJkProceedsWhenTheCapDoesNotClearTheBase) {
    ...
    // Debug exe's does too (the same load-time wedge class). The advisory
...
// The rung half of the same budget path (schema 30): a cap that leaves a grant
...
// The pair-class admission gate (the pair-class root fix,
// with the materialization accounting): the class path's
...
TEST(DriverPinTest, H2oDirectRhfFukuiSumRules) {
    ...
    // budget under kTight (the drv-2 gate surfaces it; the species SCF is
    // formally not converged at budget end though converged for every
    // observable), so the budget is raised - the same discipline as the
    // NOCV cc-pVDZ fragment test.
...
TEST(DriverErrorTest, FukuiRejectsUnconvergedSpeciesScf) {
    // The tiny-maxIterations case: the driver runs the charged Fukui
    // species' UHF SCFs itself,
...
force_certified_lane = true
    ...
    // The ABSENT key is not this test's case: it is the default (the probe
...
TEST(DriverErrorTest, ForcedCertifiedLaneRejectedAtKTight) {
    ...
    // would be the silent no-op the rule forbids: the key would read as honoured
...
// The analysis-charge refusal relocated into the cap-child family:
// this was the one-process suite's ONLY in-process
...
TEST(DriverErrorTest, RefusesNocvDenseTensorWhenTheCapCannotFitIt) {
    ...
    // load-time footprint also exceeds the tiny cap (the same load-time wedge class). The refusal
...
TEST(DriverPinTest, H2MoldenEndToEnd) {
    ...
    // the written rows alone matches the recorded 0.354892073081 row.
...
// The [memory_instrument] driver opt-in: an
...
// The instrument's refusal record, driven end to end. The instrument opens at
// run entry, but every admission gate
...
// non-event. That is the MEASURED refused-run shape, not a hypothesis: the
// trace is 163814 bytes / 1186 lines / 1149
...
// The single-Fock-call timing seam (the per-call
// wall): the lean
...
TEST(DriverRunTest, LeanStatsRowCarriesTheSingleCallWall) {
    ...
    // The row family's SHAPE, count-insensitively: the run's Fock calls are
    // the rows of a family numbered
    ...
    // taken from. The family's LENGTH is deliberately not pinned: the
    // exit-path recompute builds the Fock of the density the run
...
TEST(DriverRunTest, TraceEnabledRunsEmitTheRowFamilyOfTheirBuilder) {
    ...
    // The kernel-span probe: the lean row carries the
    ...
    if (qcx::integrals::SupportsL(3))
    {
        ...
        // The SAME family on the unrestricted leg (the per-spin adapter
        // wired here): one SCF iteration makes THREE calls - the J-only half
        ...
        // adapter wired here): its call contracts BOTH halves at once,
    ...
    // The UHF lean seam: the SAME lean row family on the
    ...
    EXPECT_GE(countRows(qfmmStats), 2u)
        << "the composed QFMM seam recorded no per-call rows; stats: "
        << qfmmStats;
...
// ---------------------------------------------------------------------------
// The run-restart acceptance: the driver
...
// run_driver.cpp) through the same Molecule::Create. The
...
TEST(DriverRestartTest, ConvergedExitWritesTheCheckpointOnlyWhenAsked) {
    ...
    std::filesystem::remove(alsoWritten);
#else
    // The widening above goes through the storage module's store, and a build
    // without that module refuses the checkpoint route by name - so on this
    // configuration there is nothing further to assert. The half above is not
    // skipped with it: that a converged exit writes NOTHING by default is
    // checkable whether or not the store is compiled in, and it is the half
    // this configuration can still prove.
    GTEST_SKIP() << "this half of the case needs the storage module (QCX_ENABLE_IO=OFF)";
#endif
}
