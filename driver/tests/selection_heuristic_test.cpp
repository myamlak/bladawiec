...
// the estimator returns a total order over the candidates, is monotone in
// nBasis for each candidate, and never selects a candidate with a budget
// below its admission floor - plus the priority behavior (GPU presence
// first, then the budgets, then threads) on hand-computed fixtures.
...
// The anchor fixture: C80H162/STO-3G, n = 562, nPairs = 81003 (402
// shells). The capture was never committed (no git object, not
// gitignored) and the 5717 ms row it records is a pre-correction timing
// the repo disowns - the corrected same-fixture row is 2478 ms, a 2.31x
// gap. The fixture's SIZES are still what the pins below measure against;
// only the cited TIMING is disqualified.
...
TEST(SelectionHeuristicTest, NoCandidateIsSelectableBelowItsDeviceBudget) {
    // This cell changed when the memory model was deleted: it used to
    // assert that a 0.3 GiB cap made EVERY CPU candidate unselectable,
    // because the model's admission floor (~1.5 GiB at n = 562) exceeded
    // the cap. The estimator no longer refuses a candidate for its modeled
    // size - the job-object cap is the admission and the engine's
    // Create-time estimates decide the rung - so at 0.3 GiB the CPU
    // candidates are selectable and only the DEVICE budget excludes the GPU
    // family. Old expectation: bestSelectable empty and every candidate
    // refused. New: the CPU family is selectable, the device family is not.
    SelectionInput input;
    input.nBasis = kAlkaneN;
    input.nPairs = kAlkanePairs;
    input.effectiveThreads = 12;
...
TEST(SelectionHeuristicTest, CostsAreMonotoneInNBasis) {
    // The monotonicity gate: monotone in nBasis for every candidate. The
...
TEST(SelectionHeuristicTest, NonSingleNodeColumnsAreStubs) {
    // Beyond one node every column reports "not selectable" - distributed
...
TEST(SelectionHeuristicTest, RiJkIsDeliberatelyExcludedFromTheRanking) {
    // NOT "unwired": ri_jk's builder exists and runs on both legs. It is
    // the only candidate the RANKING excludes, and that is deliberate
    // rather than a missing path - it is an approximated-exchange builder,
    // so it is opted into by name and never proposed for a user who did not
    // ask for one. gpu_split is the other candidate that once reported
    // unwired; it has an io kind now and the zero device share wires the
    // CPU-only fallback, so ri_jk is alone here for a different reason than
    // it used to be.
    SelectionInput input;
    input.nBasis = kAlkaneN;
    input.nPairs = kAlkanePairs;
...
TEST(SelectionHeuristicTest, GpuSplitAtTheDefaultZeroFractionIsTheCpuOnlyFallback) {
    // The zero-device-fraction gate pin: at the default zero device
    // fraction the gpu_split row IS the CPU-only fallback - the k = 1 path -
    // so it is selectable at exactly the direct row's cost, needs no
    // device, and ranks immediately after direct (the exact tie breaks by
    // the fixed candidate order): the pick can never be gpu_split by
    // construction.
    SelectionInput input;
    input.nBasis = kAlkaneN;
    input.nPairs = kAlkanePairs;
    ...
    // row undercuts direct - QFMM at this scale - which is not what this
    // row pins.
    input.deviceBudgetGiB = 0.0;
...
TEST(SelectionHeuristicTest, EngagedGpuSplitIsPricedByTheDeviceShare) {
    // The cost semantics once engaged: at a nonzero device fraction
...
TEST(SelectionHeuristicTest, EngagedGpuSplitPricingFollowsTheDeviceShare) {
    // The pricing sweep on the live direct row X (the LightPath-inclusive
    // directRow both rows are pushed with): every engaged row prices
    // max((1 - g) x X, g x X / gpuSpeedup) x (1 +
    ...
    // The pick vocabulary names the kind; the execution behind the pick is
    // not wired here.
    SelectionInput input;
    input.nBasis = kAlkaneN;
    input.nPairs = kAlkanePairs;
    ...
    for (const double share : {0.1, 0.5, 0.9})
    {
        ...
        // share. The flip was first measured under the memory-model
        // correction, which moved this fixture's direct floor 8.5533 ->
        // 1.8548 GiB and so took the 16 GiB budget off the LightPath
        // threshold. The floor and the penalty are BOTH gone with the model
        // (see SelectionCostConstants), so no cap reproduces the penalised
        // row now. Pinned as measured behaviour.
        if (share > 0.5)
        {
...
// which claimed completeness while asserting one candidate is NOT wired. ri_jk's
// nullopt is DELIBERATE (an approximated-exchange path is opted into by name,
// never chosen by the ranking), not a statement that its builder is missing.
...
TEST(SelectionHeuristicTest, GpuProbeFloorIsTheRuntimeCommitAlone) {
    // The probe-gate side of the admission rule: the driver gates
    ...
    //
    // The floor used to be the model's base term at n = 0 (0.056 GiB) PLUS
    // this runtime commit (0.35), i.e. 0.406 GiB. That term is gone with the
    // model, so the floor is the runtime's measured commit alone - a real,
    // small loosening of a safety gate, recorded rather than hidden.
