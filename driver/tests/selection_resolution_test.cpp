// The builder resolution (selection_resolution.hpp) under the owner's
// ruling of 2026-09-13 (~20:35): the no-builder default is a size LADDER -
// the direct family's lean (Schwarz-only) member at nBasis <= 1000,
// ri_j_link up to 2000, qfmm above - and direct (the budgeted machinery),
// both disk routes and gpu run only as explicit opt-ins. A tier this run
// cannot WIRE is DEMOTED with disclosure (the record's reasoning plus the
...
// setup ramp.
//
// THE ONE EXCEPTION (the owner's ruling 2026-09-13, refining the ladder):
// the order above is the SAME for RHF, UHF, RKS and UKS, and the single
// exception is a NON-HYBRID Kohn-Sham run, which puts ri_j_link BEFORE the
// lean member (a pure functional needs J and no K). The key is the
// functional's character, never the method - hybrid Kohn-Sham keeps the
// shared order. The exception tests state both sides of that key.
//
...
// from the lean boundary (it has history); 2000 is a CHOSEN value with no
...
// covered no aux for the orbital basis - the measured constraint, and the
// LIVE one when it was measured. DATED: the ruling of 2026-09-13
// replaced that rule with a quality tier that always produces a default, so
// the driver no longer produces these words for any orbital name the parser
// admits. The ladder arm is still the contract - the reason is the caller's
// to state, and a rule that refuses again must be demoted rather than run -
// so the test injects the text rather than invoking the rule.
...
// live words, and as of 2026-09-16 it is not any of them: qfmm was the last
...
TEST(SelectionResolutionTest, AutoAtOrBelowTheLeanBoundaryPicksTheLeanMember) {
    // The ladder's floor, unchanged from the lean boundary: an absent key
    // at nBasis <= 1000 wires the direct family's lean member, silently
...
TEST(SelectionResolutionTest, NonHybridKohnShamPromotesRiJAboveLean) {
    // The ladder's ONE exception (the owner's ruling 2026-09-13, refining the
    // same day's ladder): a NON-HYBRID Kohn-Sham run puts ri_j_link before
...
TEST(SelectionResolutionTest, ReasoningStatesWhichBoundaryIsMeasured) {
    // The owner's requirement, pinned as text: the reasoning must say that
    // the 2000 boundary is CHOSEN and unmeasured, and that 1000 is the
    // boundary with history. A future edit that quietly promotes the chosen
...
TEST(SelectionResolutionTest, MissingAuxDemotesTheMiddleTierToTheLeanMember) {
    // The measured constraint, in its dated form: ri_j_link needs an
    // auxiliary basis, and when the pre-ruling rule covered no aux for the
    // orbital name a no-key run in the middle tier could not enter it. The
    // The aux-auto rule (owner's ruling of 2026-09-13) made the aux rule a quality tier
    // always resolves, so this fixture injects the reason instead of
    // provoking it - what is under test is the ladder's contract, not the
    // aux rule's coverage: a tier this run cannot wire DEMOTES to the tier
    // below, disclosed in the reasoning AND carried on the warning, never
    // silently and never by refusing a run that named no builder.
...
TEST(SelectionResolutionTest, TheWalkReachesTheFloorAndNamesEveryTierItPassed) {
    ...
    // not merely that the run ended up small. DATED: this read "the
    // Kohn-Sham lanes supply such a run only while BOTH reasons exist there",
    // and as of 2026-09-16 they supply NO run of that shape at all - a
    // Kohn-Sham run wires qfmm and ri_j_link both, and only the ri_j_link
    // disk RUNGS are left, which are a rung rather than a tier. The fixture
    // below is therefore synthetic on both members, which is what it always
    // was: the reason is the caller's to state, and what this row pins is the
    // walk's own behaviour - two stated reasons, two tiers named, the floor
...
TEST(SelectionResolutionTest, UhfAboveTheLeanBoundaryDemotesWhenTheMiddleTierIsUnwired) {
    ...
    // demoted to the lean member - the unrestricted seam wires it - rather than
...
TEST(SelectionResolutionTest, AnExplicitRequestIsNeverDemoted) {
    // The disclosure rule: the ladder governs the ABSENT case. An explicit ri_j_link on a
...
TEST(SelectionResolutionTest, ExplicitDirectKeepsTheMachineryMemberAtEverySize) {
    // Ruling B: an explicit machinery spelling stays byte-stable, and that
...
TEST(SelectionResolutionTest, ExplicitDirectRequestIsHonoredVerbatimAndSilently) {
    // An explicit builder wins verbatim (Ruling B - the machinery family for
    // an explicit "direct"); the record still reports the pick the run would
...
TEST(SelectionResolutionTest, TheTierWordInMemoryResolvesExactlyLikeTheFamilyWordDirect) {
    // The two axes of the builder vocabulary (the owner's ruling 2026-09-15):
    // `direct` names the FAMILY and runs its in-memory tier; `in_memory` names
    // that TIER. One request in two spellings, so both must reach ONE
...
TEST(SelectionResolutionTest, DeviceLessGpuRequestFallsBackToTheLadderWithALoudWarning) {
    // The retargeted the device-less fallback: an explicit "gpu" without a device
