...
// the path disengaged - scanned across the whole serialized document).
...
// leg, and its exit code is 1 when any leg refused. Each leg also writes one
// "[qcx-cap] prework ..." line BEFORE it does any work, so a leg that never
// returns - killed at the cap, or stopped when the parent's wait bound
// expires - is still identifiable in the marker.
...
// The same guard cap_child_harness.hpp defines for the parent-side TUs: the
// windows min/max macros break the std::min inside cpu_backend.hpp's
// DefaultOmpTeamSize (C2589) if anything pulls windows.h in first.
#ifndef NOMINMAX
#define NOMINMAX
#endif
...
// The above-ceiling TOML: C50H102
...
// OPERATING defaults (1e-8 / 1e-6 - no fixture runs at 1e-10).
...
// The above-ceiling fixture (1210 basis functions), driven by
// DriverErrorTest.TightCapRefusesBeforeThePreGateSetupRamp: above the
// 1000-function LEAN boundary the ABSENT key resolves to the ri_j_link tier
// under the size ladder (def2-SVP carries a bundled universal-J aux,
...
// member - each refusal NAMES the family its spelling reached, which is what
// tells the three legs apart in the marker.
...
// leg is the third spelling of this size (explicit direct = the
...
// the ladder opened: until the family joined the pre-gate setup consult,
...
// The word the prework line's two envelope fields carry: this child cannot
// measure them. EstimateLeanEnvelope needs a parsed basis set, and the
// bundled data root is a PRIVATE compile definition of the libraries that
// load it (io, driver) - this child links those libraries, never their
// definitions. Saying so beats an invented number.
constexpr const char* kEnvelopeUnavailable = "unavailable";
...
int RunLeanAboveCeiling(const char* markerPath, double capGib) {
    ...
    // exactly the evidence that death destroys. Reachable now that an
    // over-cap leg goes PAST the pre-gate
    ...
    // The leg's prework line, one per leg, written BEFORE the leg does any
    // work: a leg that is killed at the cap, or that is still running when
    // the parent stops waiting, is then identifiable from the marker alone -
    // an admitted leg writes nothing else until it finishes.
    //
    // The prefix is a namespace of its own on purpose. The parent reads a
    // leg's line by searching for "<legName>: ", so the leg name here is
    // followed by a space and a field and never by ": " - the new line is
    // not findable by that search and cannot be mistaken for the leg's own
    // completion or refusal.
    const auto recordPrework = [&recordLeg, capGib](const char* legName) {
        std::ostringstream line;
        line << "[qcx-cap] prework " << legName << " team=" << qcx::backend::DefaultOmpTeamSize()
             << " cap_gib=" << capGib << " auto_peak_bytes=" << kEnvelopeUnavailable
             << " plan_peak_bytes=" << kEnvelopeUnavailable;
        recordLeg(line.str());
    };

    for (const Leg& leg : legs)
    {
        recordPrework(leg.name);
...
// The process-cap verdict probe (the capped-child pin):
// applies the cap the caller gave it and records what the memory
...
int RunCapVerdictProbe(const char* markerPath, double capGib) {
    ...
    // deleted with the model: the cap IS the admission now,
...
int main(int argc, char** argv) {
    ...
    // that shared this probe is deleted).
    ...
    } else if (fixture == "ri_jk")
    {
        ...
        // (0.056 GiB at n = 24), so the older driver refused here
        // before any builder work. The driver no longer refuses on that
        // prediction: the leg proceeds on the legacy null-budget path with
        // the job-object cap as the only limit.
        ...
        // so the refusal a pin may still assert here is the retired behaviour.
        toml = "[molecule]\n"
               "charge = 0\n"
               "multiplicity = 1\n"
               "atoms = [\n"
    ...
    {
        // The ri_jk fixture's UNRESTRICTED mirror (the UHF arm): the
    ...
    {
        // The NOCV analysis-refusal fixture: C12H26 STO-3G (n = 86, nPairs = 1953,
...
memory_cap_gib = )" +
    ...
    const auto writeToken = [&marker, &json](const char* key) {
        const std::string prefix = std::string("\"") + key + "\": ";
        const std::size_t pos = json.find(prefix);
    ...
    // The RI-K rung's observation surface (schema 30, the ri_jk fixtures
    ...
    // The disk rung's observation surface:
