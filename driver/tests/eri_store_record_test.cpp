// The eri_store request record (schema 33), end to end through the driver: the
...
//   - REFUSED BY NAME: the unrestricted legs, which the key's own wiring
...
std::string TempStorePath(std::string_view tag) {
    ...
    const std::string path = TomlPath(std::filesystem::temp_directory_path().string()) +
                             "/qcx-eri-store-record-" + std::string(tag) + "-" +
...
TEST(EriStoreRecordTest, AStoreThatCannotOpenDemotesWithTheStoresOwnError) {
    ...
    // every platform). The fact this row pins: a
    ...
    // invocation; the exclusion is what makes this
...
TEST(EriStoreRecordTest, TheUnrestrictedLegsRefuseTheKeyByName) {
    // The wiring boundary, pinned as a REFUSAL rather than a silence: an
    // explicit request the run drops is a contract violation whether or not a
...
eri_cache_store = ")" + path +
    ...
    ASSERT_FALSE(run.has_value()) << "the unrestricted leg ran with the key silently dropped: "
                                  << *run;
    EXPECT_NE(run.error().message.find("not yet wired on the UHF path"),
              std::string::npos)
        << run.error().message;
