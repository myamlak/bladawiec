// The selection record the driver reports in resources_resolved (the size
// ladder) - the wired builder, the pick the record reports, the override
// flag and the warning - through the full driver path (TOML -> RunInput
// -> resolution -> JSON). The no-builder default is the size ladder: the direct family
...
// actually wired in builder_member (schema 15 - the field the consistency
...
// QcxHasCuda macro on purpose (the GPU fallback pair).
//
// The fixtures' [scf] block names the OPERATING DEFAULT (energy 1e-8,
// density 1e-6). It used to pin 1e-10/1e-10 - a convention inherited when the
// block was copied, never a property these
...
// count. 1e-10/1e-12 is retired outright, so the inherited gate went with it.
// [method].accuracy is untouched: that preset selects SCREENING budgets, not
// the gate.
...
// kH2AutoToml with the builder pinned to gpu: the GPU fallback probe.
...
// driver's selection values carry escaped quotes (the fallback warning names
...
TEST(SelectionReportingTest, AutoRunReportsTheSelectionBlock) {
    ...
    // The size ladder: the no-builder run at H2 (2 basis functions,
    // the lean tier) wires the direct family lean member - the record
    // keeps the family name "direct" and names the member (schema 15) - the
    ...
    // The chosen boundary says so where the policy is read: a reader of this
    // record must not take 2000 for a measured crossover.
    ...
    // the pin rides the io constant so later bumps - the ri-j
    // counters (9), the disk rung's mode_record rows (10) - cannot
...
TEST(SelectionReportingTest, ExplicitDirectRequestIsRecordedSilently) {
    ...
    // The explicit request is honored verbatim (an explicit
...
TEST(SelectionReportingTest, DeviceLessGpuRequestFallsBackAndReportsIt) {
    // The device-less GPU fallback through the driver path: an explicit
...
TEST(SelectionReportingTest, CudaGpuRequestWiresGpuAndStaysSilent) {
    // The CUDA side of the fallback pair: with a device present, the explicit
...
TEST(SelectionReportingTest, UhfAutoRunWiresTheLeanMember) {
    // UHF rides the same ladder as RHF, lean member included: the no-builder
    // O2 sto-3g run at nBasis <= 1000 wires the direct family's lean member,
    // and the record keeps the family name "direct" while naming the member
    // (schema 15) - the same shape the RHF auto run reports. The record
...
// The schema-15 drift pin. `builder_member` and `builder` are two records
...
// (schema 13, inside the model's audit block) - was deleted with the model.
// The member name is now the record's only answer, which is what the
// record's own note said it was for.
TEST(SelectionReportingTest, BuilderMemberAgreesWithTheFamilyWord) {
    ...
    // explicit family word (which keeps the machinery).
    ...
    for (const auto& [toml, expectedMember] : fixtures)
    {
        ...
        ASSERT_TRUE(member.has_value())
            << "every resolved run must name the member it wired (schema 15)";
        ...
        // the record's answer (never infer it).
        if (*member == "lean")
        {
            EXPECT_EQ(JsonString(*result, "\"explicit_builder\":", selection), std::nullopt);
        }
