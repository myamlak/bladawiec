...
/// it there. The predictive model that once stood in for the missing
/// limit was deleted on 2026-09-17, so a Linux run is bounded by the
/// machine alone.
///
...
/// Is the run's memory cap a REAL limit right now (the 2026-09-17 owner
/// ruling)? True when an OS-enforced process-memory limit at or below
...
/// limit is the verdict. (Until 2026-09-17 this verdict decided whether
/// the predictive model's refusal stood; the model is deleted and the
/// reader is now the cap machinery's own record and tests.)
...
/// metric the cap and the attribution instrument use (QueryInformationJobObject,
/// JobObjectExtendedLimitInformation.PeakProcessMemoryUsed) - the ACTUAL
/// number the prediction is checked against at run end (the 2026-09-17
/// owner ruling's continuous check). Fail-open: nullopt when the read
