...
/// \file
/// The builder resolution (the size LADDER that
/// replaced the earlier lean-direct flip): turns the parsed
...
/// | n > 2000 | qfmm |
///
/// The 1000 boundary is the existing lean ceiling,
...
///
/// THE ONE EXCEPTION, and what it is keyed on: the ladder is the SAME for
...
///
/// The ladder demotes rather than refusing (the demote-with-
...
/// an auxiliary basis the wiring cannot resolve for ri_j_link (the
/// auto rule is a quality tier that always
...
/// method (RHF, both unrestricted legs through the UHF seam, and
...
/// GPU auto-fallback survives, retargeted: an explicit "gpu" with no
...
/// The lean tier's ceiling: the no-key
/// ladder's bottom boundary, nBasis <= 1000 - the existing lean
/// ceiling, carried over unchanged. It has history behind it (that
...
/// The ri_j_link tier's ceiling: the no-key
...
/// says where qfmm's crossover begins, and the value is stated as chosen
/// ("the cutoffs are lean <= 1000, ri_j_link to 2000, qfmm
/// above", with both cutoffs unmeasured). A reader must not be
...
/// record the driver copies into resources_resolved (the pick, its reasoning, and the override/fallback flags).
/// \ingroup qcx-driver
struct ResolvedBuilderSelection {
    ...
    /// disagree about which member runs (before the ladder they
...
/// not re-derive (the ladder itself reads the topology only for the GPU
/// fallback condition).
/// \ingroup qcx-driver
struct SelectionResolutionInput {
    ...
    ///
    /// The aux half of this field carries a note: it was
    /// measured as the LIVE reason (the earlier rule refused every
    /// orbital name outside def2-* and cc-*), and that rule was
