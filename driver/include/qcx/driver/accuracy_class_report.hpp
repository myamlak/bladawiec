...
// The size-class accuracy report of the composed QFMM runs: the honest
// semantics split. The QFMM
// Coulomb half keeps its per-preset constant budgets relative to exact
// Coulomb ({1e-5, 1e-7, 1e-8} Eh, the accuracy record with the kTight rung re-derived
// from 1e-9 by the 2026-09-13 ruling), but the composed run's exchange
...
// a-posteriori (the accuracy record records the measured class: 2.29e-5 at C12's 86
