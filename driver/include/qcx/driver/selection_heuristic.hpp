...
/// the bar, refined by the recorded crossover measurements when they
...
/// it today. ri_jk stays OUT of the ranking DELIBERATELY (owner ruling
/// 2026-09-16): its builder exists and runs on both legs, and the
/// 2026-09-16 unrestricted wiring, but it is an APPROXIMATED-exchange path
...
/// BY NAME and never chosen for the user. The nullopt is the ruling, not a
/// leftover; while
/// gpu_split IS wireable since the batch-partition slice-A scaffold: its io kind
...
/// behind the gpuSplitDeviceFraction) is the flag-gated batch-partition slice-C
/// work. The non-single-node columns are stubs: nodeCount > 1 reports
/// every candidate "not selectable": MPI is not wired and nothing
/// distributes a Fock build today.
///
...
/// partition candidate - the batch-partition verdict, 2026-09-04) deliberately has
...
/// the inter-iteration pipelining shape: that shape names the
/// INTER-ITERATION CPU/GPU split (SCF-dependency-capped, lagging the
/// density would change the physics), while gpu_split is the
...
enum class CostCandidate {
    ...
    kRiJk, ///< Full RI exchange - IMPLEMENTED and running on both legs since 2026-09-16;
    ...
    /// Intra-build CPU+GPU batch partition . Selectable in the
    /// slice-A scaffold: the io kind exists and the default zero device
    /// share wires the CPU-only fallback (the k = 1 path); the
    /// partition execution itself is the flag-gated slice-C work.
    kGpuSplit,
};

/// The builder kind of a candidate, for the driver to wire a pick through
/// the io vocabulary. Only kRiJk maps to nullopt, and that is a RULING rather
/// than a state of the builder: ri_jk exists (`RiFullFockBuilder`)
/// and runs on both legs since 2026-09-16, but it is an approximated-exchange
/// path and stays out of the automatic ranking so it is never chosen for a user
/// who did not name it (owner ruling 2026-09-16). kGpuSplit maps to io::BuilderKind::kGpuSplit -
/// the pick vocabulary names it today (its kind exists and reports), even though no execution path
/// consumes it until the flag-gated batch-partition slice C.
...
/// The seeded cost-table constants (the measured
...
/// comment; the calibration step re-measures the timed ones on a
/// clean-machine slot.
///
...
/// (2026-09-17): the hook's trigger was the model's direct-family floor,
/// so with the model gone it had no trigger, and the removal ruling
/// forbids reconstructing a model-derived floor here. Nothing prices the
...
struct SelectionCostConstants {
    ...
    /// (re-anchoring is a measurement, tracked separately): the capture the
    /// seed cited (2026-08-29) was NEVER COMMITTED - no git object, not
    /// gitignored, a lost scratch capture rather than a fabricated citation -
    /// and its 5717 ms is the PRE-correction alkane two-level row, already
    /// disowned in-repo as superseded history, i.e. a timing of the defective
    /// J-mode gate that the corrected capture replaced. The corrected
    /// same-fixture, same-day row is 2478 ms, a 2.31x gap. The seed's team
    /// factor (12.0) was uncorroborated, and it goes with the superseded
    /// value it scaled.
    ...
    /// cheaper RI-J tensor build. Provisional - the append-only crossover measures
    /// the real ratio.
    double riJkShareOfDirect = 0.65;
    /// The ri_j_link tensor build + contraction constant, in seconds x
    /// threads per (n^2 x nAux). The tensor work is anchored on the append-only
    ...
    /// Recorded 2026-08-29 (p4-the append-only-harness): ~40 s per create on the small
    ...
    /// PATH itself is no longer pending: both legs run it.
    ...
    /// recorded yet; the append-only crossover calibrates it.
    ...
    /// overhead below the crossover. Provisional - the append-only crossover
    /// measures the real curve.
    double qfmmVsDirect = 0.7;
    /// The GPU candidate's speedup over the CPU team (the fp32-certified
    /// device lane vs the fp64 team). Provisional - the Package-B
    /// device-profile probe measures the throughput ratio and the append-only the
    ...
    /// statics, screening tables - they are built once per run and then
    /// only read). Provisional; the device-budget mirror measures it.
    ...
    /// pair (the pair store + Schwarz neighbor CSR; the adaptive-memory
    /// lane measured the direct stores at 7-45 MB across the append-only fixtures,
    ...
    /// (the Package A CUDA gate, reference T1000, cap-child bisect): a
    ...
    /// before the device probe may run (the driver's calibration probe gate, see
    ...
    /// access violation, never a clean CUDA error. Package C's
    /// device-budget mirror measures it properly. Provisional.
    ///
    /// The C3 memory model's base term was added to this floor until
    /// 2026-09-17, when the model was deleted - the floor is now this
    ...
    /// exactly the direct cost. Provisional; the batch-partition slice-B
    /// measurement replaces it (and gpuSpeedup) with the recorded medians.
    ...
    /// group). The default ZERO is the slice-A gate: with the share at
    ...
    /// the exact tie. The slice-B measurement replaces the gate with
...
struct SelectionInput {
    ...
    /// admission floor is the C3 memory model's modeled peak).
    ...
    /// The node count (1 today: nothing distributes a Fock build, so every
    /// candidate reports "not selectable" beyond one node).
...
struct SelectionRanking {
    ...
    /// then refuses through the C3 admission gate with its full ladder
...
///
/// The memory model's base term was part of this sum until 2026-09-17,
/// when the model was deleted; the floor is consequently lower by that
