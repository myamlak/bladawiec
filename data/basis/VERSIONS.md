...
Each file opens with a comment credit block above its `BASIS` line, written
by the fetch tool out of the pinned exchange data rather than by hand: the
exchange's own provenance block (resource, version, basis set, and the
revision the tables were imported from) followed by the published reference
or references the exchange records for that element's data. References are
per element because the exchange records them that way. `#` starts a comment
line in NWChem, so the block is invisible to the parser and the numbers are
untouched by it.
...
The five auxiliary families were added 2026-08-16 for the RI-J/RIJCOSX
work. BSE 0.12 names the universal Weigend
...
The def2-QZVP family was added 2026-08-28 (the benzene/def2-QZVP
symmetry benchmark, 522 AO) and completed to the full
H-Rn element range in the same session; `tools/fetch_basis_data.py`
FAMILIES carries it for future refreshes.

The six families in the last block above were added 2026-09-13, each
against a named gap in
...
- **aug-cc-pvtz + aug-cc-pvtz-rifit** - the aug-cc family stopped at aug-cc-pVDZ,
  ...
  matched J fit the selection rule already computes from the orbital name.
...
- BSE data is distributed under the **BSD-3-Clause license** — the project's
  own statement over the `basis_set_exchange` package that carries the
  basis-set tables (its `LICENSE`), not a line its web service publishes.
- The BSE citation is [Pritchard2019], in `CITATION.bib`.
- Individual basis families carry their own original references (STO-3G:
  ...
  reproduced on the BSE website per family; each vendored file now states the
  references for its own element in the credit block above its `BASIS` line,
  so the citation travels with the numbers instead of only with this record.
  Below that block the file is the data, unmodified apart from the wrapping
  header/END.
