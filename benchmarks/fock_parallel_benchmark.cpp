// The chunking-state acceptance benchmark: wall time of a direct Fock build
// on the certified mixed-precision lane
// (kLoose carries the most permissive fp32 gate), plus an exact (%.17g)
// Fock-matrix dump for the bit-level before/after checks across the
// two-pass -> merged -> aligned chunking states. Plain main() program, not
// a google-benchmark run (same shape as preset_sweep.cpp): run it with
// identical flags in each build state and diff the dumps. The min
// iteration time is the robust number to compare - the mean includes the
// first-call warmup.
//
...
// regime where 5.3's boundary false sharing shows up at all.
