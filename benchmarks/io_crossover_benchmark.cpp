// The read-vs-recompute crossover measurement for the disk-integral store:
// per class of the H2O/cc-pVDZ disk-integral surface, wall time of serving
// the whole class from the store vs recomputing it through the underlying
// engine. This is the measurement, not an assumption - the crossover is
// recorded per class, never assumed.
//
...
// matters. Plain main() program, not a google-benchmark run: the crossover
...
//
// The RI-J leg measures the SAME crossover for the disk rung's chunk stream:
// per BuildFock iteration, wall time of serving every chunk of the chunk
// plan from the chunked store (read + FNV-1a-64 checksum + dataset open,
// twice per iteration - the two-pass shape of DiskRiFockBuilder) vs
// recomputing the same chunks through the chunked BuildRiTensorChunk entry
// point. The two sides differ ONLY in the chunk source; the common two-GEMM
// base and the direct-exchange half of the build cancel (identical code on
// both rungs) and are not timed. The checksum column measures the FNV-1a-64
...
// of the reinstatement path are the clean-machine closing-milestone runs
// (the disk builder's tolerance gate - the in-memory monolithic reference
// is the cap-broken profile).
...
void RunSystem(const std::string& name,
    ...
    // the file under it (a regression that stopped this leg
...
// The RI-J leg of the crossover: per-iteration wall time of the disk rung's
// chunk stream vs the recompute rung's, over the SAME chunk plan. One
// measured iteration replays the two-pass shape of the streamed builder
// (disk_ri_fock_build.hpp): pass 1 reads every chunk, pass 2 reads every
// chunk again (the real per-iteration volume is 2x; the second pass hits the
// OS page cache, exactly as here). The store molecule is taken by value (the
// builder Create moves it in); the loads verify against `molecule`, an
// identical-geometry fixture.
void RunRiJLeg(std::string_view name,
    ...
    // warm-up, and its chunk plan is what the reps replay. Any stale file
    // from an earlier run is removed first (the store is append-only and
    // refuses to clobber).
    const std::filesystem::path storePath =
...
int main(int argc, char** argv) {
    ...
    // The RI-J leg: the streaming-vs-recompute crossover instrument. The
    // H2O/def2-SVP rows smoke the leg - the C24H50/def2-SVP and C60 rows
    // of the reinstatement path are the clean-machine closing-milestone
    // runs (the disk builder's tolerance gate). Two chunk modes per
