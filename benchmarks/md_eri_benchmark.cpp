// The MD engine throughput benchmarks: the
// pure (6,6)-class workload (i-shell pairs - the peak-efficiency target,
// >= 30% of the machine's FMA peak) and the all-class shellset
// spread. Reported as integrals/second; the (6,6) lane also prints the
// achieved fraction of the machine's FMA peak (the class's per-integral
// flop model is below).
//
// The pair tables are per-molecule-basis constants (a Fock
// build constructs them once and reuses them). ComputeEriBatch rebuilds
// them on every call - correct for tests, but the (i,i) l = 12 pair-table
// fold dominates the pass time at class (6,6) (measured 2026-08-22: ~320 ms
// of the ~325 ms pass). This benchmark therefore drives the internal batch
// API directly: BuildPairData + AssembleClassBatches once in the fixture,
// RunBatches per iteration - the same split the Fock builder and the CUDA
// host setup already use (fock_build.cpp Create).
...
// The machine's measured FP64 peak: numpy/OpenBLAS
// dgemm, n = 2048, 2026-08-23 - 90.6 GFLOPS. The theoretical all-core
// number is ~394 GFLOPS (6 x 2 FMA x 256-bit x ~4.1 GHz); the measured
// number is the acceptance reference ("vs the machine's measured FP64
// peak").
...
void BenchMdEriClass66(benchmark::State& state) {
    ...
    std::printf("[class (6,6)] %.3e integrals/s -> %.2f GFLOPS = %.2f%% of the %.0f GFLOPS "
                "FMA peak (flop model: %.0f flops/integral)\n",
                rate,
                gflops,
