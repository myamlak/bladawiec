// The integral-tensor caching measurement: the question is whether the
// survived-quartet set of the DIRECT Fock path recurs across SCF iterations
// and what fraction of the run's ERI wall time a compute-once-on-first-
// occurrence value cache would eliminate. That cache is implemented
// (EriBatchCache in DirectJkFockBuilder) - this
// harness measures BOTH: the recurrence analysis on the uncached pass, and
// the implemented tier on the cached pass (--cache).
//
...
// RAM. The cached pass sizes the cache from a fresh
// DetectHostMemory() probe, a conservative fraction of the available
// bytes; the builder clamps to half) and reports the effective budget and
// the fp64/fp32 payload split.
//
// Plain main() like preset_sweep; local-only like the other benchmarks.
// Sizes: C20H42 STO-3G (142 functions) by default, C80H162 STO-3G (562
// functions, the 500+ benchmark floor) with --large.
...
std::optional<RunTotals> RunOne(std::string_view label, std::size_t carbonCount, bool useCache) {
    ...
    if (useCache)
    {
        // The sizing policy in action: a FRESH DetectHostMemory() probe
...
int main(int argc, char** argv) {
    ...
    if (cache)
    {
        ...
        // The before/after the benefit question asks for: the
