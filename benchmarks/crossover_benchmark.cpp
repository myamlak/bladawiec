// The qcx-internal crossover benchmark: per-iteration BuildFock wall time of
// the DirectJkFockBuilder / RiJkFockBuilder / QfmmJBuilder over the committed
// molecule family spanning ~7 -> ~840 basis functions, at every accuracy
// preset. The two outputs this exists for:
//
//   1. the direct-vs-RI-J crossover table, which feeds the default-algorithm
//      policy,
//   2. the QFMM-vs-RI-J crossover table - the first real measurement of the
//      RecommendQfmmOverRiJ threshold, landed as an unmeasured placeholder.
//
// Rows are recorded, not asserted. The one thing that IS asserted is the
// physics guard: H2O/STO-3G at kTight through the FockBuilderFn seam must
// reproduce the pinned energy -74.96292827 (+-1e-5) or the benchmark refuses
// to produce tables (a timing benchmark whose fixture construction silently
// regressed the physics would produce beautiful meaningless numbers).
//
// Fairness notes: the RI-J and QFMM rows carry their real production work -
// the RI-J row includes the direct-exchange build (K is not part of the RI
// approximation), the QFMM row is Coulomb-only (the multipole far field has
// no exchange analog) and its density-weighted screening runs at the
// fixture's diagonal-0.5 rho. OMP_NUM_THREADS is read from the environment
// and recorded, exactly like the SCF driver.
//
// The far-field-liveness flag (the vacuous-ladder trap: a dead far field
// degenerates every preset to the exact direct build, so the accuracy ladder
// stops measuring anything): the QFMM rows must report whether the octree's
// far field was live (far-pair count). The count comes from
// QfmmJBuilder::FarFieldPairCount() (integrals/, qfmm_fock_build.{hpp,cpp});
// the liveness column prints the real count on every row, and a zero count
// on a chain fixture is the vacuous-ladder alarm (MarkQfmmLiveness below).
//
// Local-only like every benchmark (QCX_BUILD_BENCHMARKS); plain main(), NOT a
...
//     aux counts) - no timing rows. Harness verification runs in this mode
//     so the harness produces no timing claims; the timed runs belong to
//     the separately gated clean-machine sweep.
...
//     builder a FRESH --memory-cap-gib budget (with the CWA sequence the
//     kNormal/kTight big-fixture Creates are refused on the budget the
//     earlier presets already consumed - a per-preset invocation isolates
//     the cell's own admission; additive, absent = today's 3-preset
//     sequence).
//   --memory-cap-gib N: wire ONE WorkspaceBudget of N GiB into every builder
//     Create: the fixed per-fixture 3-preset x 3-builder sequence then
//     charges it cumulatively (CWA - Release never restores), so later
//     Creates see cap minus committedSoFar. That deterministic ordering is
//     the retention regression surface the budget acceptance runs. Absent =
//     legacy: no budget, every Create admits exactly as before.
...
// The physics-guard pin (the harness self-test pins the same value,
// tools/bench/tests/test_harness.py).
...
struct Fixture {
    ...
    CpuTensor2 density; // The spatial rho (diagonal 0.5).
    std::size_t nBasis = 0;
    std::size_t nAux = 0;
};
...
// The budget wiring: when --memory-cap-gib is given, ONE WorkspaceBudget at
// the cap rides the whole process - every builder Create below receives it
// through the options seam (FockBuildOptions::workspaceBudget and the
// RI-J/QFMM twins). The fixed per-fixture 3-preset x 3-builder sequence
// charges it cumulatively (CWA - Release never restores Remaining), so a
// later Create sees cap minus committedSoFar: that deterministic ordering is
// the retention regression surface the budget acceptance runs. Unset =
// legacy: no budget, every Create admits exactly as today.
...
// The physics guard: H2O/STO-3G at kTight through the FockBuilderFn seam must
// reproduce the pinned energy. Returns false when the run failed or missed
// the pin - the caller then refuses to time.
...
std::optional<double> TimeBuildFock(const Builder& builder,
    ...
    // separately gated sweep runs are the numbers.
...
void RunFixture(const Fixture& fixture) {
    ...
    for (std::size_t p = 0; p < 3; ++p)
    {
        ...
        // One live builder at a time (the no-paging memory rule, user
        // directive 2026-08-29): each builder is created, measured, and
...
int main(int argc, char** argv) {
    ...
    // Line-flush stdout (the buffered-death lesson, 2026-09-03): redirected
    // stdout is fully buffered, so a cap death (fail-fast exit 3221226505)
    // discards every row since the last flush - unitbuf keeps the per-row
    // output observable up to the dying Create. Rows print once per
    // fixture/preset, so the flush cost is unmeasurable.
    ...
    // The committed molecule family plus the C12H26 chain, smallest to
    // largest. H2O/STO-3G is ALWAYS first - the physics guard indexes
    // gFixtures[0]. push_back, not list-init: the Result<unique_ptr> values
    // are move-only (initializer_list requires copyability).
    ...
    std::cout << "The crossover benchmark: OMP_NUM_THREADS="
