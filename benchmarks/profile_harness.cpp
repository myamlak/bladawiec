// The profiling harness for the direct J/K and RI-J Fock paths: a
// representative production-sized run to attach a real profiler (Windows:
// wpr CPU sampling, VS CPU profiler) to, before picking a path to redesign.
// Plain main() program like the accuracy-preset sweep - not a
...
// STO-3G (562 basis functions - past the 500-function floor benchmarks are
// sized to), Create once + repeated BuildFock. This phase exercises the
// direct-path profiling candidate: the MdQuartetTask task lists (screening,
// class-batch assembly, contraction).
//
...
// Create exercises BuildRiTensor (the RI profiling candidate: the n^2 x nAux
...
//
// Flags (the single entry point for every measurement this harness takes):
...
/// Process-wide CPU time (kernel + user), milliseconds - the wall-vs-CPU
/// attribution. Windows: GetProcessTimes (100 ns units; MSVC's std::clock is
/// not process-CPU-correct - the Eigen lapack precedent). Linux/WSL:
/// getrusage - the perf attribution lane is gcc-side, and one helper keeps
/// both harness builds on a single code path.
...
bool ParseRunOptions(int argc, char** argv, RunOptions& options) {
    for (int i = 1; i < argc; ++i)
    {
        ...
        if (arg == "--iterations" && i + 1 < argc)
        {
            ...
            if (value < 1)
            {
                std::fprintf(stderr, "profile harness: --iterations must be >= 1\n");
                return false;
            }
        ...
        } else if (arg == "--chunks" && i + 1 < argc)
        {
            ...
            if (value < 0)
            {
                std::fprintf(stderr, "profile harness: --chunks must be >= 0\n");
                return false;
            }
...
/// The machine-state banner: every timed number this harness prints is
/// interpreted against it, since the parallel primitives run on the policy
/// team size rather than the machine's full core count. The runner records
/// the power plan and idle-state checks in the run log.
void PrintBanner(const RunOptions& options) {
    ...
    std::printf("profile harness banner\n");
    std::printf(
...
int main(int argc, char** argv) {
    ...
    PrintBanner(options);
    std::printf("profile harness: phase A start\n");
    RunDirectPhase(options);
    std::printf("profile harness: phase A end\n");
    std::printf("profile harness: phase B start\n");
    RunRiPhase(options);
    std::printf("profile harness: phase B end\n");
    return 0;
}
