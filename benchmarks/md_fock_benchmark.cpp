// The direct-vs-dense Fock-build benchmark:
// DirectJkFockBuilder::BuildFock against the retained dense path
// (BuildEriTensorGeneral + a supermatrix contraction) on H2O/def2-SVP -
// the direct path must reach at least 2x the dense one at this size.
//
// Plus the per-element-filter protocol rows: the per-element filter
//'s before-number (single-level: usePerElementScreening=false, the legacy
// six-block gate) and after-number (two-level: default flag on) on H2O/def2-
// SVP and on the production fixture C80H162/STO-3G (562
// functions; no dense supermatrix leg at that size - the dense path would
// need ~800 GB). Each row reports one stats capture (total/eri/contract wall
// time, per-lane quartet counts, ERI cache hits, survived-quartet keys,
// element drops) before the timed loop.
//
// Plus the Tensor <-> Eigen conversion micro-benchmark:
// the Tensor <-> Eigen::MatrixXd boundary copies, comparing the old
...
//
// Plus the AccumulateBlock micro-benchmark:
// FockContractor::AccumulateBlock in isolation at a representative block
// size. The kernel is ONE template compiled twice
// (internal/fock_contract_kernel.hpp): the scalar copy (no /arch flag) and
// the /arch:AVX2 copy (auto-vectorization only - no hand intrinsics exist).
// Both copies register the same per-iteration batch range over the
// (d,d,d,d) fixture - BenchAccumBlockScalar (the (a) no-/arch baseline)
// and BenchAccumBlockAvx2 (the (b) /arch:AVX2 copy) each at 1 / 8 / 64
// kernel calls per iteration. Batch 1 is the original ~5 µs floor row (the
// floor-slot-6 record: MEASURED-INCONCLUSIVE at the timing-resolution
// floor); batches 8 and 64 are the larger fixture - they lift the per-
// iteration time into the 40-320 µs class where the timer read stops
// dominating, which is what the (a)/(b) rerun resolves the scalar-vs-AVX2
// question with. The (c) hand-intrinsics row is gated on that ratio (not
// written).
...
// One BuildFock with a stats capture, printed as the per-element-filter
// record row: the total/eri/contract wall-time split (screen + infrastructure
// is the total-minus-eri-minus-contract residual at k = 1 - there is no
// separate screenWallTime field), the serialized merge-chain time
// (FockBuildStats::mergeWallTime - a split of the contract phase,
// never a separate span), the per-lane quartet counts, the ERI cache hits,
...
// The mode line (the k = 5 team-size read): one line per
// alkane builder at fixture build, printing the Create-time mode record -
// the rung, the authorized k (concurrentSlots), the clamp-origin team read
// (defaultTeamSize; k == team means the fired k was team-clamped, below it
// budget-clamped), the fitted batch cap, and the charged estimate and
// reservation. Rows without a workspace budget carry no mode record and
// print nothing (the k = 1 legacy path needs no regime label).
...
// The per-element-filter protocol's per-configuration run: one stats capture
// record row) followed by the timed BuildFock loop. GoogleBenchmark invokes
// the function several times per registration, so the capture is guarded to
// fire once per label (the call sites pass string literals - stable
// addresses).
...
// The re-measurement workspace budget: the k > 1 slot count fires only on
// the budgeted fast path
// (k = min(max(1, remaining / cap), team)). The band must clear the
// budget decision's a-priori exclusion. Before that fix (a846d94) that
// gate was the all-survive pattern bound - AllSurvivePatternBytes(nPairs)
// = 8 * nPairs * (nPairs + 1) / 2 - and the alkane geometry of 402 shells
// (the NWChem SP carbon shell parses into two shells, basis_set.cpp
// ParseShellType returns {0,1}; 81,003 canonical pairs) bounded it at
// 26,246,268,048 B (24.44 GiB): the 12 GiB band of 0b9dc3e excluded the
// count unconditionally (dump-proven excluded=1, chunkPairs=6) and the
// all-survive-bound light rung produced the wall-serial 341-s run, so
// e08e29d raised the band to 25 GiB. The exclusion now counts the
// Schwarz-screened neighbor pattern itself (CountSchwarzSurvivingPairs,
// fock_build.cpp decision site): the ~4.7M real rows (~38 MB) admit any
// band above them, and the band returns to 12 GiB (the timed
// re-measurement is still owed). The null-budget rows (H2O fixture)
// keep the legacy k = 1 path - the legacy pins stay intact by construction.
...
// The C80H162/STO-3G fixture (562 functions, 402 shells, 81,003
// canonical shell pairs - the SP-shell split of the NWChem carbon basis;
// see the kAlkaneWorkspaceBudgetBytes narrative). No dense supermatrix path
// at this size (n^4 storage would be ~800 GB) - the alkane leg is
// direct-only.
void PrepareAlkaneFixture(benchmark::State& state) {
    ...
    // The re-measurement budgets: both
    // alkane builders run the budgeted fast path, where the
    // k > 1 concurrent-slot batch loop fires. Each builder needs its
    // OWN band (12 GiB, kAlkaneWorkspaceBudgetBytes - the counted-pattern
    // exclusion gate): the Create-time fixed point clamps
    // its cap until the k = team charge fits the band and the Reserve is
    // monotone, so a shared budget would leave the second builder nothing
    // and refuse its Create. The budget outlives its Create call (the
    // seam contract); the builder captures the decision, not the pointer.
    ...
    qcx::integrals::FockBuildOptions directOptions;
    directOptions.accuracy = qcx::integrals::AccuracyPreset::kNormal;
    // The re-measurement cap clamp: the
    // fixed point at the default 512 MB cap would hold up to ~24 GiB live
    // (slots x cap-scale arenas plus stores) - over the 16 GiB benchmark
    // ceiling. 256 MB caps keep the live peak near ~6 GiB while the
    // k > 1 concurrent-slot loop still fires (k = 5 was measured at the
    // 25 GiB band of e08e29d; the 12 GiB return awaits its
    // own timed run), so the throughput question is answered against the
    // 65 s k = 1 baseline.
    ...
    // The mode lines: the fired k and the clamp-origin team read per
    // alkane builder (the k = 5 team-size read; the fixture's early
    // return above keeps this one-shot).
...
// intrinsics). Running BOTH copies on one binary gives the (a) vs
// (b) comparison at every batch; the AVX2 row skips itself on CPUs
// without AVX2 (FockAvx2Available - the runtime dispatch gate). The
...
// floor of the floor-slot-6 record; batches 8 and 64 are the larger
// fixture, lifting the per-iteration time to ~40 µs / ~320 µs so the
// (a)/(b) rerun resolves the scalar-vs-AVX2 question that the floor row
// left MEASURED-INCONCLUSIVE.
// ---------------------------------------------------------------------------
...
// iteration (the benchmark range is the batch - the "larger fixture" of
// the floor-slot-6 record). The pass rotates through the fixture's block
// pool: the per-chunk sequence of kernel calls a real Fock build
// performs over one chunk's quartets, each with its own ERI block. A
// sink read of the accumulated Fock matrix keeps the work live.
...
// the 50-500 µs class the rerun needs. Both copies share the range,
// so the (a)/(b) ratio reads at every batch on one binary.
...
// wraps the tensor's host buffer zero-copy and lets
// Eigen perform the relayout internally. All three variants must produce
// bit-identical output - checked once per size in the fixture.
// ---------------------------------------------------------------------------
