// The QFMM separation-counts instrument. Counts ONLY - no wall clock, no SCF,
// no timed pass: the tree
...
// point: the near field covers 24,123,496,017 of the
...
// calibration, or the fixtures is a question for counts, not timings - the
// architecture decision is gated behind exactly this number.
//
...
// Every block is printed and flushed as it is produced (the ordering
// lesson): a fixture that dies of allocation still leaves the earlier
// fixtures' counts on stdout.
...
/// (kNormal 0.3, kLoose 0.45), the sweep's upper anchors (0.7, 1.05) and
/// the classical FMM regime (1.5 / 2.0 = the one-box buffer rule / 3.0).
...
/// The corrected-geometry theta rungs and
/// the surface test's buffers (k in units of max(rA, rB); 0 = touching).
...
/// The pair-extent decomposition over the fixture's
...
bool RunFixture(const FixtureSpec& spec, bool verifyLiteral, Output& output) {
    ...
    for (const std::size_t cap : kLeafCaps)
    {
        ...
        // The recorded cell: the baseline, now with the weighted work.
...
bool RunErrorLimb(const FixtureSpec& spec, Output& output) {
    ...
    // The fixture's physical-shaped probe density (the lesson: a
    // diagonal-only density zeroes whole screened classes).
    ...
    // The RETIRED model is pinned EXPLICITLY: the defaults are the corrected
    // ones, so the "recorded" cells of
...
int main(int argc, char** argv) {
    ...
    std::printf("QFMM separation counts - counts only, no timings\n");
