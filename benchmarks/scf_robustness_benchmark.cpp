...
// joint-system DIIS evidence - per row: converged flag, iterations, total
...
// scf/tests/robustness_uhf_test.cpp). Plain main()
// program; run standalone on an idle machine - the full
...
// seam, direct coulomb/exchange builders, the coulomb+exchange-H assembly) runs under
// {plain, CDIIS, CDIIS + robust gate} - the premature-convergence case
// lives there.
...
void PrintRow(const std::string& label, const qcx::scf::UhfResult& result, bool shifted) {
    ...
    if (shifted)
    {
        // The energy of the shifted runs: the level-shift baseline.
...
int main() {
    ...
    // alpha+beta error vectors (niedoida's joint form), the per-spin-vs-
    // joint comparison on every seed. The two-phase cap is not ported to
    // the joint form, so no twoPhase+joint row exists.
    auto joint = diis8;
    ...
    std::cout << "SCF robustness harness: O2/STO-3G triplet, seed x accelerator x gate\n";
    ...
    // the UhfFockBuilderFn seam with the coulomb+exchange-H assembly
    // fock_sigma = coulomb + exchangeSigma - H (the double-H trap; kTight
    // for the fp64 pin, the direct_uhf_test.cpp convention).
