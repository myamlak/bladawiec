// The RI-J benchmark:
// RiJkFockBuilder::BuildFock against the direct J/K builder on
// H2O/def2-SVP with def2-universal-jfit - the RI path must reach at least
// 3x the direct one at this size.
...
struct Fixture {
    ...
    // metric-factorization path (the one-shot
    // cost of the RI-J metric solve, reported separately from the per-iteration
    // BuildFock wall time). Molecule is move-only, hence the heap copies.
...
void PrepareFixture(benchmark::State& state) {
    ...
    // The fixture keeps the Create() inputs alive (what the loop
    // re-Creates from) alongside the ready-built builders; the builders are
...
// The one-shot Create() cost (aux metric build + the
// factorization the epsilon floor protects), isolated from the
