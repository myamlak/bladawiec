// The CUDA MD lane throughput benchmarks: per-variant rates on representative
// classes (kV0: L <= 4, kV1: 5..8, kV2: L >= 9) plus the CPU-vs-GPU comparison
// on the (6,6) peak class. Record-only - no thresholds: the numbers feed the
// acceptance-target comparison and the peak-efficiency tracking (the
// per-integral flop model). The suite skips without a CUDA device (the
// engine's Create probes the device count), so CUDA builds without a GPU
// stay green.
//
// Plus the host-vs-device split profile: the GpuJkFockBuilder::BuildFock wall
// vs the device-timeline span of the launches inside that call, per iteration -
// CUDA events on the default stream bracketing each device pass (the
// corrected-method instrument, replacing the invalid isolation-engine
// decomposition) plus the DirectJkFockBuilder host-only wall leg -
// the measurement that decides whether overlapping host-side screening over
...
// The Fock-build comparison: the same screened workload - the s/d chain at
// kLoose with the density gate active - through DirectJkFockBuilder (CPU) and
// GpuJkFockBuilder (GPU). Record-only, the acceptance is explicit ("speedups
// recorded, not asserted as a hard gate - hardware-dependent"): the two
// builds/sec numbers give the speedup by division; no threshold is asserted
// anywhere.
...
// (a protocol flaw - the isolated second engine's ComputeBatch
...
// full build. The split prints once per invocation.
