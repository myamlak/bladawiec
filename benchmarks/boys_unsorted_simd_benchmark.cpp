// The unsorted-SIMD
// lane. The engine-realistic sorted lanes live in boys_benchmark.cpp; this
...
//
// Ported from the design-study ChebSimdMixed (C:\Users\myaml\dev\boys-bench,
// read-only reference) onto the shipped qcx tables (qcx::integrals::detail::
...
//
// Custom main(): --self-check runs the verifier (max
// |out - BoysSingle(8, x)| over the benchmark inputs, budget 5.5e-14) and
// exits; the default mode runs the runs-log protocol (warmup + 3 passes,
// min/median/max, median = paper cell).
...
// e^{-x} on [0, 30]: degree-4 Taylor table (niedoida-style), rows padded to
// 8 doubles so the gathers can use the legal scale 8. Mirrors the shipped
// construction in integrals/src/boys_simd.cpp (the ~192 KB gather table the
...
public:
    ...
    // Taylor sum sum_k (z - x)^k / k! (niedoida's _c convention): evaluate
