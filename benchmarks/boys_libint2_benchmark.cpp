...
// libint2's FmEval_Chebyshev7<double> ladder on the SAME (n, x) input
// streams at the SAME accuracy target. Custom main(): --self-check first
// (the accuracy gate), then the timing mode
// on [uniform|molecular] - one warmup pass per side, then 3 recorded
// passes, min/median/max per side, median = the paper cell.
//
// Comparator pin: libint2 v2.13.1 ==
...
//
// The streams are the benchmark streams, generated verbatim like
...
// self-check asserts max |qcx - libint2| over each stream against the
...
// own targets add it the same way); in-source defines stay limited to the
// sanctioned set (this is the only one).
...
// into libint2/ (the fetched tree is mapped to that layout).
...
// findstr anchors as the Boys benchmarks do: "self-check:", "| PASS".
...
// The machine-state banner (PROTOCOL identity-block fields the exe can know
// itself): the CPU id, the logical-processor count, and the compile-time
