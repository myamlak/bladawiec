...
// re-derivation at the per-preset
// extents. For each (fixture, preset-tau, theta, L) cell the
// QFMM J-energy error vs the direct ground truth is measured together
// with the far-field pair count (the vacuous-ladder alarm) and the
// cost proxy nFarPairs x (L+1)^2 (the M2L blocks per interaction - the
// pick metric). The sweep is fp64-serial and deterministic - one run
// per cell, no timing (an interference-free status; the
// optional wall-time confirmation is a separate mode).
//
// Grid: theta x {1.05, 0.85, 0.7, 0.55, 0.45, 0.35, 0.3} (the incumbent
// anchors IN the grid - the incumbent ladder and the fallback rungs),
// L x {0..kQfmmMaxLMult} (0/1 include the incumbent cells (1.05, 0) and
// (0.85, 1) so they reproduce in-run), presets {kLoose, kNormal, kTight}
// (the per-preset extent taus 1e-6 / 1e-8 / 1e-10, printed per row from
// QfmmExtentForPreset) = 63 cells per preset per fixture once the L cap
// lands at 8 (28 cells today - the current cap is 3; the grid self-adapts
// via kQfmmMaxLMult, and the fit-input CSV is RE-TAKEN after the cap
// lands). The original "L x {2..10}" became {0..8} with the cap
// reconciliation.
//
// Measurement convention (the qfmm_fock_build_test.cpp
// PresetLadderDecreasesStrictlyAndStaysInBudget convention, so the
// gate reproduces the incumbent pins IN-RUN): the QFMM side is the
...
// the extent and the near-field screening; theta and
...
// D_ij (F_qfmm - F_direct)_ij| in Eh - the budget metric - plus the
// relative Frobenius error. The density is the fixture's PhysicalDensity
// (seed 20260817, diagonal 0.5 + U(-0.25, 0.25), U(-0.25, 0.25)
// everywhere else, symmetrized), fixed per fixture.
//
// Fixtures (the protocol's table): H2O/STO-3G first (the all-near
// degenerate control - every cell must give error ~0), then C12H26 /
// C24H50 / STO-3G (the budget-tooth and production-pin chains);
// C24H50/def2-SVP is the optional leg behind --def2-svp (the wide-
// exponent-spread probe). C60 is OUT of the pack entirely (the 16 GiB
// cap + compact-3D far-dead degeneracy, protocol constraint section).
//
...
// The CSV rows go to stdout (unitbuf - progress must survive a crash) and, with
// --out, to FILE as well (the run's evidence). One row per cell:
...
// Eh (the protocol renders 6 decimals: 0.452467 / 0.129461 / 0.000144728;
// the sweep is fp64-serial deterministic, so the full-precision values
// reproduce bit-identically and the +-1e-9 is the same-bits tolerance)
// - and every H2O cell < 1e-9. Prints PASS/FAIL and exits nonzero on a
// mismatch (the protocol: stop, reconcile before any further run).
...
// The theta rungs of the sweep grid: the incumbent
// anchors {1.05, 0.85, 0.7} IN the grid, then the design rungs
// {0.55, 0.45, 0.35, 0.3} - 7 rungs x (L 0..kQfmmMaxLMult) = 63 cells
// per preset once the cap lands at 8 (28 today).
constexpr std::array<double, 7> kSweptThetas = {1.05, 0.85, 0.7, 0.55, 0.45, 0.35, 0.3};

// The presets whose extent taus {1e-6, 1e-8, 1e-10} are swept (the rung
// mapping). tau is FIXED per preset - not a sweep knob.
...
std::unique_ptr<ChainFixture> MakeChainFixture(std::size_t carbonCount) {
    ...
    // The core matrix is materialized BEFORE the moves (the ordering
    // lesson: the aggregate's clauses evaluate left-to-right, so an
    // in-place ToMatrix(*core) after std::move(*core) would read the
    // moved-from tensor).
...
// the data directory (the sweep's pattern).
...
// test.cpp:134 EnergyError - the budget metric, Eh): 0.5 * |sum_ij
...
bool RunCell(const ChainFixture& fixture,
    ...
    // The cost proxy of the pick: the M2L blocks per interaction.
    record.costProxy = static_cast<double>(record.farPairs) * (lMult + 1) * (lMult + 1);
    record.ok = true;
    return true;
}

// The PASS gate's captured values: the three incumbent cells of C12 and
    ...
    for (std::size_t p = 0; p < kSweptPresets.size(); ++p)
    {
        ...
        for (const double theta : kSweptThetas)
        {
            for (int lMult = 0; lMult <= qcx::integrals::internal::kQfmmMaxLMult; ++lMult)
            {
                ...
                // Scientific with 10 significant digits: the PASS gate
                ...
                // The PASS gate captures: the incumbent cells of C12 and the
...
// The PASS gate, ENFORCED: the three
// incumbent cells must reproduce the re-measured values at full
// precision and every H2O cell must stay below 1e-9. Returns 0 on
// pass; prints the failing rows and returns 1 on mismatch (the
// protocol: stop, reconcile before any further run).
int CheckS1Pins(const S1Pins& pins) {
    bool ok = true;
    const auto check = [&ok](const char* cell, std::optional<double> measured, double pin) {
        if (!measured.has_value())
        {
            std::cerr << "gate FAIL: " << cell << " was never measured\n";
            ok = false;
        } else if (std::abs(*measured - pin) > 1e-9)
        {
            std::cerr << "gate FAIL: " << cell << " " << std::scientific << std::setprecision(10)
                      << *measured << " vs pin " << pin << " (diff > 1e-9)\n";
            ok = false;
        }
    };
    ...
    {
        std::cerr << "gate FAIL: the H2O control's largest error " << std::scientific
    ...
    if (ok)
    {
        std::cout << "gate PASS: the incumbent C12 pins and the H2O control reproduce\n";
    }
...
int main(int argc, char** argv) {
    ...
    // per-cell progress must survive a crash (the sweep's lesson).
    ...
    // The gate, after the C12 leg:
    // the three incumbent cells must reproduce their re-measured values
    // and the H2O control must stay ~0. FAIL -> stop; reconcile before
    // any further run.
