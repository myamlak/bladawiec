// The pair-data construction benchmarks: the per-pair MD table path that
// used to heap-allocate per
// primitive pair and now runs on fixed-size std::array storage.
//
//   - BuildPairData: the whole per-pair pass - geometry, the per-axis E
//     tables (PerAxisETable: std::vector before, std::array<double,
...
//     per allocation, two per fold, before the array storage.
//
// The i-chain fixture is the same synthetic two-center i-shell basis as
// md_eri_benchmark.cpp (the same batch API), with a three-primitive I shell so
// each (6,6) pair exercises nine (3 x 3) primitive-table builds and nine
// folds.
// The 2026-08-22 measurement ("the (i,i) l = 12 pair-table fold dominates
// the class (6,6) pass time") was made with the vector fold allocations;
...
// allocation, identical in both forms - only the per-primitive-pair
// table/fold storage changed).
...
// nHerm = Hermite3DCount(12) = 455). The allocating form gave every fold two
// 2.7 MB std::vectors (e3d, mid); the fixed form keeps both as static
// thread_local fixed arrays. The output vector is reused across iterations (caller-side,
