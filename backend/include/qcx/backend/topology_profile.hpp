...
/// \file
/// The node's topology profile: the single input the algorithm-selection
/// heuristic's cost table, the device budgets, and the precision-ladder
/// referee cadence consume. One bundle probed once at driver start - nodes,
/// cores per node (the P/E split of DetectCpuTopology), devices per node with
/// VRAM each, and the interconnect class.
///
/// Staged reality: only the single-node class is
/// implemented. MPI is not implemented and multi-GPU has no plan yet, so
/// nodeCount is always 1 and interconnect is always kUnknown today; the
...
/// until MPI lands; every non-single-node candidate
...
struct TopologyProfile {
    /// The node count: the MPI world size when MPI is linked, else 1.
    /// Always 1 today - MPI is not implemented.
...
/// (the D-A9 probe gate; the runtime DLLs are load-time, so a CUDA-linked
