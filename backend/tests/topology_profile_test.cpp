// The topology-profile probe: the single-node bundle is consistent, and the device sweep
...
TEST(TopologyProfileTest, ProbeReportsSingleNode) {
    ...
    // The single-node class: MPI is not implemented, so the bundle is one
    // node with an unknown interconnect by construction.
