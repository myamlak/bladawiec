...
WHY THIS EXISTS, measured. The documentation gate is: zero warnings, and
`EXTRACT_ALL` must stay NO. Its coverage is exactly the Doxyfile INPUT list.
That list once named 13 of the 14 DAG modules: `scf/include` was absent, so the
gate reported the same zero warnings whether scf was documented or not - a whole
module outside the gate with the gate green over it (3908e583). The Doxyfile
comment states the rule; nothing enforced it, and the second, hand-typed copy of
a list whose home is tools/check_dag.py is what drifted. This check is that
enforcement.
...
3. The gate can still fail: WARN_AS_ERROR must be declared and must not be NO.
   ...
   a red tree - the defect that setting was adopted to close.
...
def mode_errors(text: str) -> list[str]:
    ...
    if mode.upper() == "NO":
        return [
            ...
            " qcx-docs succeeds over a red tree - the defect this setting was"
            " adopted to close. Use a mode that fails the build."
        ]
