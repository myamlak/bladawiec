...
Method (all conventions mirror the detection machinery so the runtime
stage can recompute the same class keys from the molecule-frame elements):
  ...
  - Correlation: subduction to the realizable Abelian subgroup H
    ...
    correlation columns index the Abelian block order directly;
...
def emit_header(tables):
    ...
    a("/// columns index the Abelian block order directly.")
...
def clang_format(path):
    ...
    The pre-commit hook blocks staged C++ that is not clang-format-clean,
    so the generated header must be emitted in the repo's canonical format.
    """
    import shutil
    import subprocess
