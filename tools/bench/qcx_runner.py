...
Writes the run-input TOML for `qcx run` from a case manifest (Angstrom
coordinates, verbatim from the manifest - the single-copy fairness
guarantee), invokes the exe, parses the result JSON from
stdout (io/src/result_json.cpp), and measures wall (perf_counter) + child
CPU time (GetProcessTimes) around the subprocess.

Thread contract: OMP_NUM_THREADS is set in the child
environment and recorded as threads_set. The effective fp64-lane thread
count is capped at half the team by the fp64 lane's team-size policy and is
observable from outside the process, so the row carries threads_set plus a
policy note; the OMP_NUM_THREADS=1 point is the serial anchor that
...
# The accuracy-preset names the input schema accepts (literal PascalCase).
...
# way. The first table (2026-08-29) put the two legs EQUAL - 1e-8/1e-8,
...
def RunQcx(exe: pathlib.Path,
    ...
    threads_effective records threads_set (the fp64-lane cap is not observable
    from outside; see the module docstring). The memory gate
    amendment): the child launches only when the host has at least
    ...
    # The peak-memory guard layers read the job-object peak after child exit
    ...
    row = {
        ...
        "thread_note": "threads_set=OMP_NUM_THREADS; the fp64 Fock "
                       "lane is capped at half the team (not observable "
