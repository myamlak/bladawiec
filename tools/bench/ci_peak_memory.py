...
harvest' line for the caller to copy into the baseline file when an
accepted change moves the footprint (the committed-baseline
discipline of the reference pins).
...
# The first-cut alert rule: trip when the row exceeds
# max(1.5 x baseline peak, baseline + 512 MiB).
...
def Main(argv: list) -> int:
    ...
    print(f"perf-guard harvest: {case_path.stem} peak RSS {peak_kib} "
          ...
          f"this becomes the accepted footprint in the "
          f"same commit)")
    return 0
