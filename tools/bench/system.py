#!/usr/bin/env python3
"""Machine-state recording for the comparative benchmark harness:
power-plan recorded, effective threads, wall AND CPU). Windows-native
...
def ToWslPath(path: pathlib.Path) -> str:
    """Translate a Windows path to the /mnt/... path WSL sees (the plan
    §3 bridge pattern)."""
...
def PowerScheme() -> Optional[str]:
    """The active Windows power scheme, e.g. 'Balanced' (recorded for the
    machine-state block). Bytes mode: subprocess's text mode decodes
...
# The calibration cells' run records carry the machine-state covariates
# the fit's block/covariate terms consume (the CPU frequency, the
