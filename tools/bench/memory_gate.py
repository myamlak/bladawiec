...
Hard rule (user, 2026-08-29): no benchmark invocation may
...
def JobPeakMemoryKib(proc: subprocess.Popen) -> Optional[int]:
    ...
    JobObjectExtendedLimitInformation.PeakProcessMemoryUsed - the
    field of the job object). Fail-open: None when the job handle is
    absent or the read-out fails - the read-out is measurement for the
    peak-memory guard layers, while the process cap itself stays the
    fail-closed enforcer.
    Read after the child has exited (the peak is final then)."""
