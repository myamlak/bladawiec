...
def write_tables_header(path):
    with open(path, "w", newline="\n") as f:
        ...
        f.write("// masks, and the amplification constants. Validated by\n")
        ...
        # Certified amplification constants, symmetric in (LBra, LKet).
...
def format_header(path):
    ...
    if clang_format is not None:
        ...
        # pass). A single pass leaves the file differing from a fresh --check.
        for _ in range(4):
