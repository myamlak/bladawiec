#!/usr/bin/env python3
"""Generate the QFMM translation-operator tables - "generate and verify,
don't hand-derive". Every identity the tables rest on is checked below,
and an unverified table is never emitted.
...
and do not ship an unverified translation operator; emission is refused):
  ...
  V4  The literal check:  a charge cloud's potential via
                          ...
                          acceptance suite on C12). The local
...
QFMM_MAX_L = 8  # kQfmmMaxLMult (the cap; L=10 measured ~30-36 MB header - escalation documented separately)
...
def check(identity, lhs, rhs, tolerance):
    ...
    deviation exactly as the emission rule demands."""
    global PASS
    diff = abs(lhs - rhs)
...
def verify(m2m, m2l, denom):
    ...
    # L=3-era and failed 31/162 draws at L = 8 by up to 42x (REVIEW LANE
    # 13, 2026-08-30).
    ...
    print("=== V4: the charge-cloud potential (the verify-before-emit check) ===")
    n_charges = 7
    R_cloud = mpf("0.5")
    center = rand_d()
    charges = []
    ...
    for _ in range(5):
        ...
        for L in range(QFMM_MAX_L + 1):
            ...
            # per-point decrease (monotonicity is the acceptance
            # suite's C12 check).
    ...
    for t in range(MOMENT_COUNT):
        ...
        check(f"V7: <R_{l_t},{m_t}, R_{l_t},{m_t}>_S2 == N[{l_t}] "
              ...
              # from the L=3-era 1e-15 by REVIEW LANE 13 (2026-08-30) -
              # never loosened - and still catches measure or
              # normalization bugs, which fail at O(1e-3..1).
              mpf("1e-18"))
...
def write_header(path, m2m, m2l, denom):
    with open(path, "w", newline="\n", encoding="utf-8") as f:
        ...
        f.write("// The QFMM translation-operator tables - generated and verified,\n")
        f.write("// not hand-derived.\n")
