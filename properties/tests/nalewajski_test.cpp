...
// exercised on real converged RHF densities (properties links scf PRIVATE;
// the fragment SCFs run internally per distinct element).
//
...
// The indices of the atoms of the given element (the canonical atom order is
...
// pairing: the canonical atom order is a sort, so the tests never assume an
// index layout.
