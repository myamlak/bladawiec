...
// canonical atom order is a sort, so tests must never assume an index layout).
...
TEST(EddbTest, BenzeneSto3gSymmetryAndReferencePins) {
    ...
    // Grouping by element (the canonical atom order is a sort - the H's land
