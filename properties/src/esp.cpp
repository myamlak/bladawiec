...
qcx::Result<EspFitResult> AnalyzeEspCharges(const qcx::molecule::Molecule& molecule,
    ...
    // leaves the normal equations rank-deficient: a partial point set must
    // not silently produce garbage charges.
    ...
    // Sanity cap: no physically meaningful ESP fit exceeds a
