...
// matrix. Algorithms ported from the read-only niedoida reference repo
// (property_kit/mulliken_analysis.cpp, lowdin_analysis.cpp,
// mayer_analysis.cpp, gopinathan_jug_analysis.cpp); the traps found there
// are documented at each site below. The combined AnalyzePopulations path
// computes S and S^{1/2} ONCE and shares them across all four analyses
// (the S^{1/2}-sharing requirement).
...
// b < a triangle only, mirrored (niedoida's mayer_analysis.cpp
// micro-optimization). Free valence: the (PS_alpha - PS_beta) cross-
// product sum over the atom's own block - identically zero for RHF since
// the two spin densities coincide there (documented in the header).
MayerAnalysis ComputeMayer(const Eigen::MatrixXd& psAlpha,
    ...
    for (std::size_t a = 0; a < aoRanges.size(); ++a)
    {
        ...
        for (std::size_t b = 0; b < a; ++b)
        {
            ...
            // bug); the full-matrix pairing restores the true
...
// squared block sum over ALL pairs including the diagonal (niedoida's
// gopinathan_jug_analysis.cpp loops b over every atom - the self-term is a
// formula artifact, documented in the header).
