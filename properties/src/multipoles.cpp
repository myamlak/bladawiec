...
// the integrals builders' dipole/quadrupole matrices. Algorithm ported
// from the read-only niedoida reference repo
// (property_kit/multipole_moments.cpp); the trap found there - the upper-
// triangle-only fill that never writes M(l,k) - is fixed by the explicit
// symmetrization below (documented in the header).
...
qcx::Result<MultipoleMoments> AnalyzeMultipoles(const qcx::molecule::Molecule& molecule,
    ...
    // symmetrization (the fix for niedoida's upper-triangle-only fill:
    // a caller reading the lower half must get the real value, not a stale
    // zero).
    result.quadrupole =
