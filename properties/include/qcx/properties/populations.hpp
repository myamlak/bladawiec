...
/// atom's functions (the "overlap population" row sums). The per-AO gross
/// orbital populations are the full diagonal of P_sigma S.
...
/// Lowdin populations ([Lowdin1950]): with P_ort = S^{1/2} P S^{1/2} (the
/// FORWARD square root - distinct from scf's S^{-1/2} symmetric
/// orthogonalization), the atomic population of atom a is the sum of the
/// P_ort diagonal over the atom's functions.
...
/// Mayer bond orders and valences ([Mayer1983]). With PS_sigma = P_sigma S:
...
/// Gopinathan-Jug bond orders ([Gopinathan1983]): B(a,b) = sum_{i in a,
/// j in b} [P_ort_alpha(i,j)^2 + P_ort_beta(i,j)^2] over ALL pairs including
/// the a == b diagonal. The self-term is a mathematical artifact of the
/// formula's definition, not a "bond to itself"; it is included in the
/// returned matrix by construction, and consumers that mean a bond-order
/// matrix proper (e.g. the EDDB stage) drop the diagonal themselves.
