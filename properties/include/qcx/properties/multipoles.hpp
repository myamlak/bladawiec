...
/// quadrupole, ported from the read-only niedoida reference repo
/// (property_kit/multipole_moments.cpp - algorithms and traps only, never
/// code). With the total density P = P_alpha + P_beta:
...
///
/// Origin convention (matching niedoida's multipole moments and the
/// integrals builders): all moments are relative to the coordinate origin
/// AS GIVEN - the caller is responsible for recentering the molecule first
/// if a center-of-mass or center-of-charge dipole is wanted. Units are
/// atomic: e*a0 for the dipole, e*a0^2 for the quadrupole.
///
...
///
/// Symmetrization fix: niedoida's reference fills only the upper
/// triangle of M and never writes the lower one; every qcx caller reads
/// complete matrix types, so the returned quadrupole is explicitly
/// symmetrized instead of carrying the partial fill forward.
...
struct MultipoleMoments {
    ...
    Eigen::Matrix3d quadrupole; ///< Traceless quadrupole Theta = 3M - Tr(M) I,
                                ///< e*a0^2, symmetric (the explicit symmetrization).
};
