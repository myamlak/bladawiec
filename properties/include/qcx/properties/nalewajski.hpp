...
/// reference: each atom is represented by its isolated-atom SCF (niedoida's
/// ROHF with AverageOccupations; here the qcx UHF equivalent - the alpha
/// channel of the isolated atom, whose highest partially filled shell is
/// degenerate so any spanning set yields the same averaged density). The
/// per-atom valence contributions are averaged over all configurations of
/// the degenerate shell (the OccGenerator combination enumeration of the
/// reference implementation), always with the closed-shell core states
/// doubly occupied.
///
...
/// as implemented by the reference) yields the bond orders
...
/// valences are negative; the reference implementation works with the
/// squares). The config averaging is invariant under rotations of the
/// degenerate-shell spanning set (v_i_a + v_c_a = 1/2 <||Delta||_F^2>_cfg),
/// so the bond orders depend only on the shell, not on the SCF's arbitrary
/// choice of spanning orbitals within it.
///
...
/// construction the SAD guess uses. The Aufbau state classification of the
/// reference implementation covers Z <= 19 ("less than 20 shells"); heavier
/// elements are kUnimplemented.

/// The Nalewajski-Mrozek analysis result, mirroring the reference
/// implementation's outputs (niedoida's NalewajskiAnalysis).
